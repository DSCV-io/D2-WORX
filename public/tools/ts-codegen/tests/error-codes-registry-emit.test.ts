...
describe("discoverCatalogs — real contracts directory", () => {
  it("discovers exactly 2 public error-code specs (generic + auth; KC is private)", () => {
  ...
  it("excludes the keycustodian catalog from public contracts discovery", () => {
    // H2 / dual-values: KC error-codes live under private/contracts only;
    // public discoverCatalogs must never surface KEYCUSTODIAN_* rows.
    const { catalogs } = discoverCatalogs(contractsPath());
