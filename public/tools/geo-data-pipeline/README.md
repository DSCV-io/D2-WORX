...
TypeScript dev tool that pulls upstream geo reference data (CLDR, IANA tzdb, libphonenumber, datasets/\*, Wikidata SPARQL, debian/iso-codes), transforms it, and writes the JSON catalogs at `public/contracts/geo/` that codegen consumes.
...
Three-tier output layout at `public/contracts/geo/`:
...
2. **Missing-Wikidata-en log** — `tools/geo-data-pipeline/logs/missing-wikidata-en.json` (gitignored). Review each row's `debianFallbackName`; if a fallback is awkward, add an overlay entry at `public/contracts/geo/overlays/subdivisions.overlays.spec.json` with a clear `reason`.
...
committed `public/contracts/geo/*` specs, or operator refresh) plus any replace-trigger for future
