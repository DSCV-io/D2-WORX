 ...
 *
 *   nuget — monorepo-root global.json (sdk.version + sdk.rollForward) +
 *           Directory.Build.props (TargetFramework + LangVersion).
 *   npm   — root package.json devDependencies.typescript +
 *           public/packages/typescript/tsconfig.base.json (target + module).
 *
...
export function readToolchainPin(
  ...
  if (ecosystem === "nuget") {
    const globalJson = JSON.parse(read("global.json")) as {
      sdk?: { version?: string; rollForward?: string };
    };
    const buildProps = read("Directory.Build.props");
  ...
  const tsconfigBase = JSON.parse(
    read("public/packages/typescript/tsconfig.base.json"),
