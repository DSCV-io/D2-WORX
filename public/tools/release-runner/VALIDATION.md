...
| `runner.ts` | Full pipeline integration (bump engine → manifest editor → changelog editor) over temp-dir fixtures with synthetic commit histories and a fixed `today` date. Covers: dry-run (no writes), apply mode (npm version + CHANGELOG written), NuGet apply, single-package filter, multi-package apply, no-op (no qualifying commits), wire-breaking entry in changelog, and prerelease-labelled packages under both default and legacy-commit-type semantics. 18 tests in `tests/runner.test.ts`. | Promote to a real-repo integration test when the runner's `--apply` mode is first exercised on a real release branch. |
...
```
pnpm --filter release-runner exec tsx src/cli.ts --against nova --dry-run
```

Result (baseline was `nova` at the time of validation):

```
Release runner — DRY-RUN (baseline: nova)

  DcsvIo.D2.ErrorCodes.Registry (nuget)  0.1.0 → 0.2.0  [minor]
