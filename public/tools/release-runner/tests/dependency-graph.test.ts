...
function makeNpmPkg(
  ...
  return {
    ...
    manifestPath: `public/packages/typescript/${name.replace("@dcsv-io/d2-", "")}/package.json`,
    changelogPath: `public/packages/typescript/${name.replace("@dcsv-io/d2-", "")}/CHANGELOG.md`,
...
describe("buildDependentIndex — graph construction", () => {
  it("returns an empty map for a package with no dependencies", () => {
    const pkg = makeNpmPkg("@dcsv-io/d2-a");
    const index = buildDependentIndex([pkg]);
    expect(index.size).toBe(0);
  });

  it("resolves a single npm name→name edge correctly", () => {
    const pkgA = makeNpmPkg("@dcsv-io/d2-a");
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", ["@dcsv-io/d2-a"]); // B depends on A
    const index = buildDependentIndex([pkgA, pkgB]);
    expect(index.get("@dcsv-io/d2-a")).toEqual(new Set(["@dcsv-io/d2-b"]));
  });

  it("resolves a nuget path-basename→name edge correctly", () => {
    const pkgUtils = makeNugetPkg("DcsvIo.D2.Utilities");
    const pkgResult = makeNugetPkg("DcsvIo.D2.Result", ["DcsvIo.D2.Utilities"]);
    const index = buildDependentIndex([pkgUtils, pkgResult]);
    expect(index.get("DcsvIo.D2.Utilities")).toEqual(
      new Set(["DcsvIo.D2.Result"]),
    );
  });

  it("handles multiple dependents on one package", () => {
    const pkgA = makeNpmPkg("@dcsv-io/d2-a");
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", ["@dcsv-io/d2-a"]);
    const pkgC = makeNpmPkg("@dcsv-io/d2-c", ["@dcsv-io/d2-a"]);
    const index = buildDependentIndex([pkgA, pkgB, pkgC]);
    expect(index.get("@dcsv-io/d2-a")).toEqual(
      new Set(["@dcsv-io/d2-b", "@dcsv-io/d2-c"]),
    );
  });

  it("ignores a self-edge (package listing itself as a dependency)", () => {
    const pkg = makeNpmPkg("@dcsv-io/d2-a", ["@dcsv-io/d2-a"]); // self-edge
    const index = buildDependentIndex([pkg]);
    // self-edge is dropped — index should be empty or not map @dcsv-io/d2-a → @dcsv-io/d2-a
    expect(index.get("@dcsv-io/d2-a")).toBeUndefined();
  });

  it("a devDependency @dcsv-io/d2-* name produces the same edge as a regular dependency", () => {
    ...
    const pkgA = makeNpmPkg("@dcsv-io/d2-a");
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", ["@dcsv-io/d2-a"]); // simulates devDep edge
    const index = buildDependentIndex([pkgA, pkgB]);
    expect(index.get("@dcsv-io/d2-a")).toContain("@dcsv-io/d2-b");
  });
  ...
  it("ignores edges whose dependency name is not in the consumable set", () => {
    ...
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", [
      "@dcsv-io/d2-sourcegen-internal",
    ]); // not consumable
    const index = buildDependentIndex([pkgB]);
    // The non-consumable name appears as a key in the index but points to @dcsv-io/d2-b.
    // For propagation, this is harmless — @dcsv-io/d2-sourcegen-internal will never
    // be in directPlans (it has no descriptor), so its dependent set is never
    // traversed. Confirm the index has the entry but @dcsv-io/d2-b is only reachable
    // FROM a plan of @dcsv-io/d2-sourcegen-internal (which never exists in practice).
    expect(index.get("@dcsv-io/d2-sourcegen-internal")).toContain(
      "@dcsv-io/d2-b",
    );
  });
});
...
describe("topoSort — basic ordering", () => {
  ...
  it("single package with no deps → returned as-is", () => {
    ...
    expect(result[0]!.name).toBe("@dcsv-io/d2-a");
  });
  ...
  it("two packages with an edge: dep appears before dependent", () => {
    ...
    const indexA = result.findIndex((p) => p.name === "@dcsv-io/d2-a");
    const indexB = result.findIndex((p) => p.name === "@dcsv-io/d2-b");
  ...
  it("chain A → B → C: sorted as A, B, C", () => {
    const pkgA = makeNpmPkg("@dcsv-io/d2-a");
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", ["@dcsv-io/d2-a"]);
    const pkgC = makeNpmPkg("@dcsv-io/d2-c", ["@dcsv-io/d2-b"]);
    const result = topoSort([pkgC, pkgA, pkgB]); // shuffled input

    const names = result.map((p) => p.name);
    expect(names.indexOf("@dcsv-io/d2-a")).toBeLessThan(
      names.indexOf("@dcsv-io/d2-b"),
    );
    expect(names.indexOf("@dcsv-io/d2-b")).toBeLessThan(
      names.indexOf("@dcsv-io/d2-c"),
    );
  });
  ...
  it("diamond graph (A→B, A→C, B→D, C→D): A appears first, D appears last", () => {
    ...
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", ["@dcsv-io/d2-a"]);
    const pkgC = makeNpmPkg("@dcsv-io/d2-c", ["@dcsv-io/d2-a"]);
    const pkgD = makeNpmPkg("@dcsv-io/d2-d", [
      "@dcsv-io/d2-b",
      "@dcsv-io/d2-c",
    ]);
    ...
    const names = result.map((p) => p.name);
    expect(names[0]).toBe("@dcsv-io/d2-a"); // sole leaf
    expect(names[names.length - 1]).toBe("@dcsv-io/d2-d"); // sole root
    expect(names).toHaveLength(4);
  });

  it("non-consumable dep edge is ignored (edge to a name not in the inventory)", () => {
    // @dcsv-io/d2-b depends on @dcsv-io/d2-external which is not in the inventory.
    ...
    expect(result).toHaveLength(2);
    expect(result[0]!.name).toBe("@dcsv-io/d2-a");
    expect(result[1]!.name).toBe("@dcsv-io/d2-b");
  });

  it("throws when a dependency cycle exists", () => {
    const pkgA = makeNpmPkg("@dcsv-io/d2-a", ["@dcsv-io/d2-b"]);
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", ["@dcsv-io/d2-a"]);
  ...
  it("isolated packages (no edges) are sorted by name for determinism", () => {
    const pkgZ = makeNpmPkg("@dcsv-io/d2-z");
    const pkgA = makeNpmPkg("@dcsv-io/d2-a");
    const pkgM = makeNpmPkg("@dcsv-io/d2-m");
...
describe("propagateBumps — direct bump propagates to dependents", () => {
  it("directly-bumped dependency patch-bumps untouched dependents", () => {
    ...
    const planB = result.find((p) => p.pkg.name === "@dcsv-io/d2-b");
  ...
  it("directly-bumped plan has empty dependencyEntries (not a dependency-update)", () => {
    ...
    const result = propagateBumps([planA], [pkgA, pkgB]);
    const resultPlanA = result.find((p) => p.pkg.name === "@dcsv-io/d2-a");
  ...
  it("PATCH propagation on a 0.x package produces 0.x.y → 0.x.(y+1)", () => {
    const pkgA = makeNpmPkg("@dcsv-io/d2-a");
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", ["@dcsv-io/d2-a"], "0.3.7");
    const planA = makePlan(pkgA);
  ...
  it("no dependents means propagation returns only the direct plan", () => {
    const pkgA = makeNpmPkg("@dcsv-io/d2-a"); // no dependents
    const planA = makePlan(pkgA);

    const result = propagateBumps([planA], [pkgA]);

    expect(result).toHaveLength(1);
    expect(result[0]!.pkg.name).toBe("@dcsv-io/d2-a");
  });
  ...
  it("duplicate entries in directPlans are de-duped by the visited guard (no double seeding)", () => {
    ...
    const aPlans = result.filter((p) => p.pkg.name === "@dcsv-io/d2-a");
    ...
    const bPlans = result.filter((p) => p.pkg.name === "@dcsv-io/d2-b");
...
describe("propagateBumps — transitive chain (A → B → C)", () => {
  ...
  it("transitive chain: bumping A patch-bumps B and C", () => {
    ...
    const planB = result.find((p) => p.pkg.name === "@dcsv-io/d2-b");
    const planC = result.find((p) => p.pkg.name === "@dcsv-io/d2-c");
  ...
  it("a directly-bumped middle package (A,B direct; C transitive) still expands to C", () => {
    ...
    const resultC = result.find((p) => p.pkg.name === "@dcsv-io/d2-c");
    expect(resultC).toBeDefined();
    expect(resultC!.bump).toBe("patch");
    expect(resultC!.dependencyEntries).toContain("@dcsv-io/d2-b");

    // A and B keep their own direct (minor) plans — not overwritten.
    expect(result.find((p) => p.pkg.name === "@dcsv-io/d2-a")!.bump).toBe(
      "minor",
    );
    expect(result.find((p) => p.pkg.name === "@dcsv-io/d2-b")!.bump).toBe(
      "minor",
    );
  });
  ...
  it("B's dependencyEntries names A; C's dependencyEntries names B", () => {
    ...
    expect(planB!.dependencyEntries).toContain("@dcsv-io/d2-a");
    expect(planC!.dependencyEntries).toContain("@dcsv-io/d2-b");
  });
  ...
  it("deep chain (A → B → C → D) all get PATCH", () => {
    ...
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", ["@dcsv-io/d2-a"]);
    const pkgC = makeNpmPkg("@dcsv-io/d2-c", ["@dcsv-io/d2-b"]);
    const pkgD = makeNpmPkg("@dcsv-io/d2-d", ["@dcsv-io/d2-c"]);
    const planA = makePlan(pkgA);
    ...
    const planD = result.find((p) => p.pkg.name === "@dcsv-io/d2-d");
...
describe("propagateBumps — diamond dependency graph (A→B, A→C, B→D, C→D)", () => {
  ...
  it("diamond with B directly bumped: B, C, D all appear in result once", () => {
    ...
    expect(result.find((p) => p.pkg.name === "@dcsv-io/d2-c")?.bump).toBe(
      "patch",
    );
    expect(result.find((p) => p.pkg.name === "@dcsv-io/d2-d")?.bump).toBe(
      "patch",
    );
  });
});
...
describe("propagateBumps — already-directly-bumped dependents keep their bump", () => {
  it("B directly bumped minor: propagation from A does not overwrite B's bump", () => {
    ...
    const resultPlanB = result.find((p) => p.pkg.name === "@dcsv-io/d2-b");
...
describe("propagateBumps — phantom dependent queued from two directly-bumped parents", () => {
  // A and B are both directly bumped, and both have @dcsv-io/d2-phantom as a dependent
  // in the reverse-dependency index (but @dcsv-io/d2-phantom has no descriptor).
  ...
  it("phantom dependent (no descriptor) queued twice → terminates cleanly, phantom absent from result", () => {
    ...
    const pkgPhantomDependent = makeNpmPkg("@dcsv-io/d2-phantom", [
      "@dcsv-io/d2-a",
      "@dcsv-io/d2-b",
    ]);
    const planA = makePlan(pkgA, { bump: "minor", newVersion: "0.2.0" });
    ...
    expect(
      result.find((p) => p.pkg.name === "@dcsv-io/d2-phantom"),
    ).toBeDefined();
    expect(result.find((p) => p.pkg.name === "@dcsv-io/d2-phantom")?.bump).toBe(
      "patch",
    );
  ...
  it("truly-absent phantom (not in packages array) → queue visits it twice without crashing", () => {
    // A depends on B, and B's dep list includes @dcsv-io/d2-phantom (no descriptor).
    // A has @dcsv-io/d2-phantom in its dep list too → reverse index maps phantom to {A,B}.
    // Seeding queues phantom from A and again from B's propagation.
    const pkgA = makeNpmPkg("@dcsv-io/d2-a", ["@dcsv-io/d2-phantom"]);
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", ["@dcsv-io/d2-phantom"]);
    const planA = makePlan(pkgA, { bump: "minor", newVersion: "0.2.0" });
    const planB = makePlan(pkgB, { bump: "minor", newVersion: "0.2.0" });
    ...
    // Second dequeue: visited.has("@dcsv-io/d2-phantom")===true → skips (line 236).
...
describe("propagateBumps — non-consumable edges are not propagation targets", () => {
  it("a dependency name not in the package inventory produces no propagation target", () => {
    ...
    const pkgA = makeNpmPkg("@dcsv-io/d2-a"); // directly bumped
    // pkgB lists @dcsv-io/d2-sourcegen-internal as a dependency — that name has no descriptor.
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", [
      "@dcsv-io/d2-sourcegen-internal",
      "@dcsv-io/d2-a",
    ]);
    ...
    expect(result.find((p) => p.pkg.name === "@dcsv-io/d2-b")).toBeDefined();
    // No phantom plan for the non-consumable.
    expect(
      result.find((p) => p.pkg.name === "@dcsv-io/d2-sourcegen-internal"),
    ).toBeUndefined();
  });

  it("a non-@dcsv-io/d2- dependency name produces no propagation target", () => {
    // The loader filters out non-@dcsv-io/d2- npm deps. Model the case where that filter
    // already ran and the descriptor has no entry for 'typescript' etc.
    const pkgA = makeNpmPkg("@dcsv-io/d2-a");
    const pkgB = makeNpmPkg("@dcsv-io/d2-b", ["@dcsv-io/d2-a"]); // loader already excluded 'typescript'
    const planA = makePlan(pkgA);
...
describe("propagateBumps — cross-ecosystem propagation", () => {
  it("nuget package bumping propagates to its nuget dependents", () => {
    ...
    const planResult = result.find((p) => p.pkg.name === "DcsvIo.D2.Result");
    expect(planResult).toBeDefined();
    expect(planResult!.bump).toBe("patch");
    expect(planResult!.dependencyEntries).toContain("DcsvIo.D2.Utilities");
  });
});
...
describe("propagateBumps — output ordering", () => {
  it("direct plans appear first in the result array", () => {
    ...
    expect(result[0]!.pkg.name).toBe("@dcsv-io/d2-a");
    expect(result[1]!.pkg.name).toBe("@dcsv-io/d2-b");
  });

  it("propagated plans are sorted by name for determinism", () => {
    const pkgA = makeNpmPkg("@dcsv-io/d2-a");
    const pkgZ = makeNpmPkg("@dcsv-io/d2-z", ["@dcsv-io/d2-a"]);
    const pkgM = makeNpmPkg("@dcsv-io/d2-m", ["@dcsv-io/d2-a"]);
    const planA = makePlan(pkgA);

    const result = propagateBumps([planA], [pkgA, pkgZ, pkgM]);

    const propagatedNames = result.slice(1).map((p) => p.pkg.name);
    expect(propagatedNames).toEqual(["@dcsv-io/d2-m", "@dcsv-io/d2-z"]);
  });
});
