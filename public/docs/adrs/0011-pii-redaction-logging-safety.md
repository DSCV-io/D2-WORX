...
**4. Cross-language redaction intent is spec-driven from the same source.** On the TypeScript side, `sanitizedErrorRender` (`public/packages/typescript/logging/src/sanitized-error-render.ts`) mirrors `SanitizedExceptionRender` and never exposes `.message`; `@d2/logging`'s `ILogger` forbids passing `Error` objects directly. For data-type redaction, the spec-driven codegen decision (ADR-0002) generates `<TypeName>RedactPaths` constants (e.g. `IRequestContextRedactPaths`) that consumers pass to `setupLogger` as Pino `redactPaths`. The source of truth for which context fields carry `redact: true` is the same spec that drives both the .NET codegen and the TS constant — so .NET's `[RedactData]` posture and TS's Pino redaction derive from the same annotation, not independent per-runtime curation.
