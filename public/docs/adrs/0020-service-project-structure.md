...
The namespace is the folder path verbatim, **including** the `.App` / `.Infra` layer segment (`DcsvIo.D2.Private.Edge.KeyCustodian.App.Infrastructure.Persistence`, `DcsvIo.D2.Private.Edge.KeyCustodian.Infra.Persistence.Postgres`). The layer segment is not dropped via `RootNamespace` tricks. **WHY:** in a service the layer IS semantics — `.App.Infrastructure.Persistence` (a port) versus `.Infra.Persistence.Postgres` (an adapter) is a meaningful distinction the namespace should carry; collapsing it saves one segment but creates a rule every reader must memorize.

Global usings follow a two-tier frequency-driven policy. **Tier-1, service-project scope** (central `<Using>` items in the product hosts' `Directory.Build.targets` — private monorepo illustration, not required for public clone): `DcsvIo.D2.Result`, `DcsvIo.D2.Utilities.Extensions`/`.Attributes`/`.Enums`, `DcsvIo.D2.I18n`. Product service hosts reliably reference the full D2 runtime stack, so all five Tier-1 namespaces resolve with no per-project opt-out. **Shared libs are excluded** (`public/packages/dotnet/`) — `DcsvIo.D2.I18n` is split across three assemblies (`I18n.Abstractions`, `I18n.Keys`, `I18n` core), and the Tier-1 libs form a dependency chain among themselves; a blanket runtime-wide global would produce hard CS0246 errors in the libs that sit below the full stack. Shared libs keep explicit usings. The `netstandard2.0` condition in the targets file excludes source-gen shells under a service. **Tier-2, per project** (`GlobalUsings.cs`, frequency-driven): a project MAY globalize ANY namespace it legitimately references — including `Microsoft.EntityFrameworkCore`, `Microsoft.Extensions.DependencyInjection`, `Microsoft.Extensions.Options`, `System.Security.Cryptography`, or any vendor SDK — when that namespace is repeated across roughly ≥3 files in that project. The dependency law is enforced by `<ProjectReference>` edges in the csproj (compile-time); a global using is per-project and cannot leak the namespace across the layer boundary. Per-file usings remain for low-frequency (1–2 file) namespaces. **Global aliases are permitted** — `global using IClock = DcsvIo.D2.Time.IClock;` in every project that uses both NodaTime and `DcsvIo.D2.Time` resolves the `NodaTime.IClock` vs `DcsvIo.D2.Time.IClock` CS0104 ambiguity project-wide. Reference implementation: KeyCustodian `domain/GlobalUsings.cs` + `app/GlobalUsings.cs` + Edge `tests/GlobalUsings.cs`. **SA1200 exemption**: `GlobalUsings.cs` files are exempt from SA1200 (inside-namespace placement) via a `[**/GlobalUsings.cs]` section in `.editorconfig` — `global using` directives are top-level by C# language rule and cannot be nested inside a namespace. **Hard constraints regardless of frequency**: never duplicate a namespace already covered by SDK ImplicitUsings (`System`, `System.Collections.Generic`, `System.Threading`, `System.Threading.Tasks`, etc.); never duplicate a Tier-1 entry; never globalize in shared libs. **WHY frequency-driven:** the dependency law is enforced at the `<ProjectReference>` boundary, not at using-directive visibility — a domain project without an EF assembly reference cannot compile any EF type regardless of whether a global using appears in it. Frequency-driven globals eliminate the highest-repetition usings with no loss of signal; low-frequency usings stay explicit as a natural readability cue.
...
- The dependency law is compiler-enforced, not just documented: `<ProjectReference>` edges enforce the layer boundary at compile time — a domain project without an EF assembly reference cannot compile any EF type regardless of per-file or global usings, and the api-is-the-only-Infra-referrer rule means a layering violation fails the build.
...
- **Globalize usings repo-wide** (across all projects) to cut per-file noise. Rejected for shared libs — the Tier-1 dependency chain produces hard CS0246 errors in libs that sit below the full stack. Service projects use Tier-1 globals for the universally-referenced namespaces; per-project `GlobalUsings.cs` handles the rest frequency-driven (≥3-file threshold).
