...
- **.NET** — [`public/packages/dotnet/auth/protocol-audiences-source-gen/`](../../public/packages/dotnet/auth/protocol-audiences-source-gen/) (Roslyn source-gen → `WellKnownAudiences` constants in `DcsvIo.D2.Auth.Abstractions`; no README)
- **TypeScript** — [`tools/ts-codegen` › `protocol-audiences-emit.ts`](../../tools/ts-codegen/README.md) (→ `ProtocolAudiences` const-object in `@dcsv-io/d2-auth-abstractions`)
