...
| [`grpc/`](grpc/README.md)                                     | Faithful `D2Result` ↔ `D2ResultProto` gRPC response-envelope codec. `ToProto()` (server WRAP) / `ToD2Result<T>()` (client RE-MATERIALIZE) / `HandleAsync<T>()` (call wrapper with transport-fault fail-open) / `IsTransientGrpcException()`. Business failures travel as normal gRPC `OK` responses; `RpcException` is reserved for auth/transport-layer faults (`DcsvIo.D2.Auth.Grpc`). |
