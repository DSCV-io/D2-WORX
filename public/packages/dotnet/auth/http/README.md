...
HTTP-transport binding for [`DcsvIo.D2.Auth`](../core/README.md) — convention-based middleware that runs the JWT validation pipeline + session liveness check on inbound HTTP requests, emits RFC 7807 ProblemDetails on failure, and supports per-endpoint scope requirements via the ASP.NET endpoint-metadata pattern.

Lives in its own csproj (separate from `DcsvIo.D2.Auth`) so the AspNetCore framework reference is opt-in: worker / console / gRPC-only services that consume `DcsvIo.D2.Auth` for JWT validation in non-HTTP paths don't need to drag in `Microsoft.AspNetCore.App`. Sibling [`DcsvIo.D2.Auth.Grpc`](../grpc/README.md) holds the gRPC-transport binding under the same logic. The two transport-binding csprojs are siblings (no inter-csproj dep): each registers an identical scoped `IRequestContext` resolver lambda that reads from a shared `HttpContext.Items` slot, so a single dual-transport host wires both extensions and resolves `IRequestContext` correctly under either transport.
...
`D2ProblemDetailsExtensions` is a plain static class carrying the `ToProblemDetails` extension method body + runtime `TypeUriFor` + `MaterializeMessages` helpers. The wire-format catalog (`TYPE_URI_PREFIX`, `CONTENT_TYPE`, `EXTENSION_*` extension keys, `TITLE_*` per-status titles, + the `TitleFor` switch) lives in [`D2ProblemDetailsKeys`](../../problem-details/abstractions/README.md) (codegen-emitted into `DcsvIo.D2.ProblemDetails.Abstractions` from `contracts/problem-details/problem-details.spec.json`). The same spec drives the TS-side `@dcsv-io/d2-problem-details-abstractions` catalog (re-exported from `@dcsv-io/d2-headers` for compatibility) AND the path-B Customizer in `DcsvIo.D2.AspNetCore`, so the three emit paths produce byte-identical Shape A bodies for identical inputs.
...
| `Extensions["d2_error_code"]`   | `D2Result.ErrorCode` (one of the `AUTH_*` constants from `DcsvIo.D2.Auth.Errors.AuthErrorCodes`).                                                                                                                                                     |
...
| `DcsvIo.D2.Auth.Abstractions`                                             | `ISessionLivenessTracker` contract + `JwtClaimTypes` + `D2HttpContextItems.REQUEST_CONTEXT` (shared with sibling `DcsvIo.D2.Auth.Grpc`) + `IAmbientRequestScopeAccessor` port — this lib hosts the `IHttpContextAccessor`-backed adapter for it so the outbound lib stays framework-free. |
...
| `DcsvIo.D2.Time`                                                          | `IClock` / `SystemClock` — timestamps the Edge call-path entry `RequestOriginEdgeInboundMiddleware` starts the path with.                |
