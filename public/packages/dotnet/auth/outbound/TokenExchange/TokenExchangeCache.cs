...
internal sealed class TokenExchangeCache : IAsyncDisposable
{
    ...
    private readonly ConcurrentDictionary<Guid, ConcurrentDictionary<string, byte>> r_reverseIndex =
        new();
    ...
    /// <param name="ttl">
    /// TTL for the entry — derived from the OAuth response's <c>expires_in</c>.
    /// </param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>
    /// <c>Ok</c> on success; the underlying <see cref="ILocalCache"/> failure on cache error.
    /// </returns>
