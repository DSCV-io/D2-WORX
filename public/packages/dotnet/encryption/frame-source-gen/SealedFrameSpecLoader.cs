// -----------------------------------------------------------------------
// <copyright file="SealedFrameSpecLoader.cs" company="DCSV">
...
/// <summary>Parses <c>encryption-frame-sealed.spec.json</c>.</summary>
internal static class SealedFrameSpecLoader
{
    ...
    /// <summary>Parses raw JSON spec content into a <see cref="SealedFrameSpec"/>.</summary>
    /// <param name="path">Spec file path.</param>
    /// <param name="json">Raw JSON content.</param>
    /// <returns>A <see cref="LoadResult{TSpec}"/>.</returns>
    public static LoadResult<SealedFrameSpec> Load(string path, string json)
    {
        ...
        try
        {
            ...
            {
                return new LoadResult<SealedFrameSpec>(
                    Spec: null,
                    Diagnostic: EmitDiagnostics.SealedMalformedSpec(
                        fileName,
            ...
            return new LoadResult<SealedFrameSpec>(
                Spec: new SealedFrameSpec(version, fields.ToImmutable(), constraints!),
                Diagnostic: null);
        }
        catch (JsonException ex)
        {
            return new LoadResult<SealedFrameSpec>(
                Spec: null,
                Diagnostic: EmitDiagnostics.SealedMalformedSpec(fileName, ex.Message));
        }
    }
        ...
        JsonElement element, string fileName, int index)
    {
        ...
        {
            return (null, EmitDiagnostics.SealedMalformedSpec(
                fileName,
    ...
    private static (SealedFrameConstraints? Constraints, EmitDiagnostic? Diagnostic)
        ParseConstraints(JsonElement el, string fileName)
    {
        ...
        if (!TryReadInt(el, "ephPubLengthPrefixSize", out var prefixSize))
        {
            return (null, EmitDiagnostics.SealedMalformedSpec(
                fileName, "constraints missing required integer 'ephPubLengthPrefixSize'"));
        }

        if (!TryReadInt(el, "maxEphPubLength", out var maxEphPub))
        {
            return (null, EmitDiagnostics.SealedMalformedSpec(
                fileName, "constraints missing required integer 'maxEphPubLength'"));
        }
        ...
        return (new SealedFrameConstraints(
            minKid, maxKid, prefixSize, maxEphPub, nonce, tag, minFrame), null);
    }
