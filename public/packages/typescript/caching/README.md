...
- **Dual-runtime constants/semantics** (not a full behavior interop harness): `@dcsv-io/d2-contract-tests` `tests/caching-twin.parity.test.ts` + `fixtures/caching-twin/constants.json` (dual-runtime constants catalog; emitter `CachingTwinFixtureEmitter`). See [PARITY.md](../../../../docs/PARITY.md) caching stack row.
