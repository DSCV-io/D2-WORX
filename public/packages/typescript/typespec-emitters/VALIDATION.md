...
| ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `client/GetJwksInput.g.cs` (ns `D2.Edge.KeyCustodian.Client`)                                 | `emitCsharpDtos("getJwks", "D2.Edge.KeyCustodian.Client", "contracts/typespec/key-custodian/key-custodian.tsp", [], [], [])`                                                                                     | Byte-identical to `GET_JWKS_INPUT_FIXTURE`                                               | `tests/byte-parity.test.ts`            | `byteParity_GetJwksInput_CommittedFixtureIdentical`                                                                             |
| `client/GetJwksOutput.g.cs` (ns `D2.Edge.KeyCustodian.Client`)                                | `emitCsharpDtos("getJwks", "D2.Edge.KeyCustodian.Client", ..., [], outputFields, [nested("Jwk", ...)])`                                                                                                          | Byte-identical to `GET_JWKS_OUTPUT_FIXTURE`                                              | `tests/byte-parity.test.ts`            | `byteParity_GetJwksOutput_CommittedFixtureIdentical`                                                                            |
...
| ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `D2.Edge.KeyCustodian.Client.GetJwksInput` (parameterless)              | `D2.Edge.KeyCustodian.Client.GetJwksInput` (committed `client/GetJwksInput.g.cs`)                  | `GetJwksTransportDtoTests.GetJwksInput_IsParameterless`                                                        |
| `D2.Edge.KeyCustodian.Client.GetJwksOutput` (`IReadOnlyList<Jwk> Keys`) | `D2.Edge.KeyCustodian.Client.GetJwksOutput` (committed `client/GetJwksOutput.g.cs`)                | `GetJwksTransportDtoTests.GetJwksOutput_HasKeysProperty_OfCorrectType`                                         |
| `D2.Edge.KeyCustodian.Domain.ValueObjects.Jwk` (6 public members)        | `D2.Edge.KeyCustodian.Client.Jwk` (6-field positional record)                                       | `GetJwksTransportDtoTests.Jwk_MatchesDomainVoPublicShape`                                                      |
...
transport `D2.Edge.KeyCustodian.Client.Jwk` is a 6-field positional record. Equivalence is
validated by public-member-set comparison (6 public members, same names and types), NOT by
constructor arity. The transport DTO lives in `D2.Edge.KeyCustodian.Client` — a separate project
...
The real substrate is the dormant cross-workspace `private/services/web` BFF (the `$lib` alias resolves only inside SvelteKit; the real wiring is the host-gated Browser REST client wiring) — so the emitted REST client is driven against a FAITHFUL `apiCall`/`apiCallAnon` double (same signature, returns the real `@d2/result` `D2Result`, records path/method/body/idempotencyKey).
