...
describe("emitGrpcService_TypeAliases_Disambiguate", () => {
  it("emits short using-aliases for proto message and DTO types (distinct names, no prefix required)", () => {
    ...
    expect(svc.content).toContain(
      "using SignInput = global::DcsvIo.D2.Private.Edge.Tests.TypeSpecDto.Generated.SignInput;",
    );
    expect(svc.content).toContain(
      "using SignOutput = global::DcsvIo.D2.Private.Edge.Tests.TypeSpecDto.Generated.SignOutput;",
    );
...
describe("emitGrpcService_TransportMapper_ExtensionBlockForm", () => {
  ...
  it("mapper emits D2Result.Grpc + DcsvIo.D2.Result usings for the envelope block", () => {
    const [, mapper] = emitSign();
    expect(mapper.content).toContain("using DcsvIo.D2.Result;");
    expect(mapper.content).toContain("using DcsvIo.D2.Result.Grpc;");
  });
...
describe("emitGrpcService_EnvelopePopulation_NoThrow", () => {
  ...
  it("service emits using DcsvIo.D2.Result.Grpc for the ToProtoResponse extension", () => {
    const [svc] = emitSign();
    // Prefer global:: so serviceImplNs containing ".Grpc." cannot shadow Result.Grpc / Grpc.Core.
    expect(svc.content).toMatch(/using (global::)?DcsvIo\.D2\.Result\.Grpc;/);
  });
});
...
describe("emitGrpcService_EnumRequestField_ParseBridgeAndAlias", () => {
  ...
  it("emits the enum alias (cross-namespace DTO) + ToWire/ParseWire helpers + D2Result<Input> request mapper", () => {
    ...
    expect(svc.content).toContain("using DcsvIo.D2.Result;");
