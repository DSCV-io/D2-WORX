...
const NS = "DcsvIo.D2.Private.Edge.Tests.TypeSpecRoute.Generated";
...
describe("emitIdempotencyStoreSeam — file shape", () => {
  ...
  it("emits using DcsvIo.D2.Result", () => {
    const file = emitIdempotencyStoreSeam(NS, SOURCE);
    expect(file.content).toContain("using DcsvIo.D2.Result;");
  });
...
describe("buildIdempotencyGate — header keySource", () => {
  ...
  it("extraUsings includes DcsvIo.D2.Utilities.Extensions (for Falsey)", () => {
    const weave = buildIdempotencyGate(makeHeaderGateInput());
    expect(weave.extraUsings).toContain("DcsvIo.D2.Utilities.Extensions");
  });
...
describe("emitRoutePolicy — header idempotency gate woven in", () => {
  ...
  it("emits DcsvIo.D2.Utilities.Extensions using (for Falsey)", () => {
    const file = emitRoutePolicy(
      makeBaseRouteInput({
        idempotency: { keySource: "header", ttlSeconds: 86400, fields: [] },
      }),
    );
    expect(file.content).toContain("using DcsvIo.D2.Utilities.Extensions;");
  });
 ...
 *   - using DcsvIo.D2.Utilities.Extensions (sorted)
