...
describe("emitCsharpDtos_RedactedField_PropertyTarget", () => {
  it("emits [property: RedactData] on redacted param and adds using directives", () => {
    ...
    expect(inputFile!.content).toContain("using DcsvIo.D2.Utilities.Enums;");
...
describe("emitCsharpDtos_NestedRedactedField_PropertyTarget", () => {
  ...
  it("output whose ONLY redaction is nested → both redact usings present exactly once (CS0246 regression pin)", () => {
    ...
    const attrUsings = (
      outputFile!.content.match(/using DcsvIo\.D2\.Utilities\.Attributes;/g) ??
      []
    ).length;
    const enumUsings = (
      outputFile!.content.match(/using DcsvIo\.D2\.Utilities\.Enums;/g) ?? []
    ).length;
