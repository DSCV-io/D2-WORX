...
- **Single-source fixture path**: the .NET emitter writes to the SAME path the TS test reads from. Path is resolved on the .NET side via `TestPaths.RepoRoot()` (walks up to find `D2.slnx`); on the TS side via `fixtureUrl(catalog, scenario)` (URL relative to `import.meta.url`). The directory is `Directory.CreateDirectory`'d on the .NET side so emit-on-fresh-checkout works.
