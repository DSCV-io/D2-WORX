...
export default defineConfig({
  resolve: {
    alias: {
      // `@dcsv-io/d2-encryption-abstractions` — deterministic path to the
      ...
      // suites REQUIRE a fresh `pnpm --filter @dcsv-io/d2-encryption-abstractions
      // build` first; a stale dist/ would be served silently and could
      // mask a regression.
      "@dcsv-io/d2-encryption-abstractions": fileURLToPath(
        new URL("../encryption-abstractions/dist/index.js", import.meta.url),
      ),
      // `@dcsv-io/d2-encryption` runtime crypto twin — same deterministic-dist
      // rationale as the abstractions alias. The crypto-KAT parity suites
      // REQUIRE a fresh `pnpm --filter @dcsv-io/d2-encryption build` first.
      "@dcsv-io/d2-encryption": fileURLToPath(
        new URL("../encryption/dist/index.js", import.meta.url),
      ),
      // `@dcsv-io/d2-geo-abstractions` is wired into the contract-tests workspace
      ...
      // the parity suite (`pnpm --filter @dcsv-io/d2-geo-abstractions build`); a
      // stale dist/ would be served silently and could mask a regression.
      "@dcsv-io/d2-geo-abstractions": fileURLToPath(
        new URL("../geo/abstractions/dist/index.js", import.meta.url),
      ),
      // Records-meta sub-export of @dcsv-io/d2-geo-default — same rationale as
      // the @dcsv-io/d2-geo-abstractions alias above (package.json sub-export
      // resolution requires the pnpm symlink layer to be current; this
      // alias gives Vitest a deterministic path).
      "@dcsv-io/d2-geo-default/_records-meta.g": fileURLToPath(
        new URL(
          "../geo/default/dist/generated/_records-meta.g.js",
          import.meta.url,
        ),
      ),
      // `@dcsv-io/d2-validation` is wired into the contract-tests workspace
      // package.json + tsconfig references, but the pnpm-managed
      // node_modules symlink can lag the package.json edit until the next
      // `pnpm install` rotation. This alias gives Vitest a deterministic
      ...
      // suite REQUIRES a fresh `pnpm --filter @dcsv-io/d2-validation build` first; a
      // stale dist/ would be served silently and could mask a regression.
      "@dcsv-io/d2-validation": fileURLToPath(
      ...
      "@dcsv-io/d2-caching-abstractions": fileURLToPath(
        new URL("../caching/abstractions/dist/index.js", import.meta.url),
      ),
      "@dcsv-io/d2-caching-local-default": fileURLToPath(
        new URL("../caching/local-default/dist/index.js", import.meta.url),
      ),
      "@dcsv-io/d2-caching-distributed-redis": fileURLToPath(
        new URL("../caching/distributed-redis/dist/index.js", import.meta.url),
      ),
      "@dcsv-io/d2-caching-tiered": fileURLToPath(
