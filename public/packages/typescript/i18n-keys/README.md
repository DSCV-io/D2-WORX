...
`D2.Shared.I18n.Keys` on the .NET side — a thin keys layer whose only dependency
...
`@d2/i18n-abstractions` (the `TKMessage` type + `tk()` factory the generated
...
The TK constants are needed by both `@d2/result` (for factory default messages)
and `@d2/i18n` (the Translator and Paraglide runtime). If the constants lived
in `@d2/i18n`, packages that depend on `@d2/result` could not also transitively
...
zero-dep `@d2/i18n-abstractions` leaf — breaks the cycle structurally: any
package may import `@d2/i18n-keys` regardless of where it sits in the dependency
graph.

Consumers of the `@d2/i18n/keys` re-export (`import { TK } from "@d2/i18n/keys"`)
continue to work unchanged — `@d2/i18n` re-exports from this package for
backward compatibility.

## Parity with .NET

Mirrors `D2.Shared.I18n.Keys` (`TK.g.cs`). Both are generated from the same
