...
/// require a <c>spiffe://â€¦</c> URI), so ONE grammar check admits both â€” the map is keyed
...
/// appear in any workload's allowed set â€” granting one is a fail-loud startup error. This
...
/// consumers yet; an empty policy makes every lookup return the empty set â‡’ deny-all â‡’
...
public sealed class KeyringDomainAuthorityOptions
{
    ...
    /// <summary>
    /// Gets the per-workload allowed keyring domains. Key = a bare lowercase caller id â€”
    ...
    /// (default-deny). A non-payload domain must NOT appear under ANY key â€” the config
    ...
    ///     â€” a keyring is a full encrypt+decrypt capability, and only payload domains are
    ///     keyring-grantable.
    ///   </item>
    /// </list>
    /// </summary>
        ...
        foreach (var (workloadKey, domains) in AllowedKeyringDomainsByWorkload)
        {
            ...
            if (!SpiffeWorkloadIdentity.Create(workloadKey).Success)
            {
                return string.Create(
                    ...
                    + $"'{workloadKey}' (must be a bare lowercase [a-z0-9-] identifier â€” a "
            ...
            foreach (var domain in domains)
            {
                ...
                // keyring â€” they are one-way, per-service sealed. Refuse a grant naming one
                ...
                // too â€” but the operator deserves the exact reason (a sealed domain needs no
                // keyring grant) instead of a generic "non-catalog domain" error.
                var normalizedDomain = domain.Trim().ToLowerInvariant();

                if (ProductEncryptionDomainModes.ModeFor(normalizedDomain) == ProductEncryptionDomainMode.Sealed)
                {
                    return string.Create(
                        CultureInfo.InvariantCulture,
                        $"KEYCUSTODIAN_KEYRING_AUTHORITY workload '{workloadKey}' is granted the "
                        + $"sealed-mode domain '{domain}' â€” sealed-mode domains have no keyring "
                ...
                // crown-jewel key whose keyring is never distributable â€” reject it at boot.
                ...
                if (createResult.Data!.KeyType != KeyType.AesPayload)
                {
                    return string.Create(
                        ...
                        + $"domains are keyring-grantable â€” a keyring is a full "
