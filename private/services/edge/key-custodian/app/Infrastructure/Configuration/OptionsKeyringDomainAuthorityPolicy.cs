// -----------------------------------------------------------------------
// <copyright file="OptionsKeyringDomainAuthorityPolicy.cs" company="DCSV">
...
/// <summary>
/// Resolves a workload's allowed keyring domains from
/// <see cref="KeyringDomainAuthorityOptions"/>. A workload absent from the policy (or a
/// null / empty caller id) resolves to the EMPTY set — default-deny.
/// </summary>
/// <remarks>
...
/// not per <see cref="AllowedKeyringDomainsFor"/> call.
/// </remarks>
public sealed class OptionsKeyringDomainAuthorityPolicy : IKeyringDomainAuthorityPolicy
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="OptionsKeyringDomainAuthorityPolicy"/>
    /// class, normalizing every configured grant once.
    /// </summary>
    /// <param name="options">The bound keyring-domain authority options.</param>
    public OptionsKeyringDomainAuthorityPolicy(IOptions<KeyringDomainAuthorityOptions> options)
    {
    ...
    /// <inheritdoc/>
    public IReadOnlySet<string> AllowedKeyringDomainsFor(string? workloadId)
    {
    ...
    private static IReadOnlyDictionary<string, IReadOnlySet<string>> BuildNormalizedMap(
        KeyringDomainAuthorityOptions options)
    {
        ...
        foreach (var (workloadId, domains) in options.AllowedKeyringDomainsByWorkload)
        {
            if (domains.Falsey())
                continue;
