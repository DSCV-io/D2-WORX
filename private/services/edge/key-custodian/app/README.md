...
- **`IKeyCustodianDbContext`** — the only seam Infra implements for App: `DbSet<KeyRecord> Keys`, `DbSet<KeyAuditRecord> Audit`, `DbSet<LeafIssuanceAuditRecord> LeafIssuanceAudit`, `SaveChangesAsync`, `ClearChangeTracker` (clears the EF tracker after a failed save so uniqueness/EXCLUDE losers can re-read the winner cleanly).
...
- `Microsoft.EntityFrameworkCore` (10.0.7) — `IKeyCustodianDbContext` exposes `DbSet<…>` + `SaveChangesAsync` + `ClearChangeTracker`; App owns the DbContext interface; Infra provides the concrete implementation.
