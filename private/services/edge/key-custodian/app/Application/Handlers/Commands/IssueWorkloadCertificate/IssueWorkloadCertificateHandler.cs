...
/// planes (in-process faÃ§ade + gRPC) flow through â€” authority gate, scope check,
...
/// deny BEFORE the CSR is even parsed â€” a garbage or forged CSR from an
...
/// ignored by construction â€” only its verified public key reaches the leaf.
/// </para>
/// <para>
...
/// isolated capability (the ONLY holder of the intermediate-CA unwrap â€” this
      ...
      H
{
    ...
    /// (fail-closed) before the authority rule or any crypto runs â€” layered under
    ...
    {
        // 1) Authority precedes ALL work â€” read the ESTABLISHED Origin +
        //    ImmediateCaller (set by the boundary that produced this context; Â§9.41)
        ...
        // 2) CSR verification â€” AFTER the gate, BEFORE any store access: size cap,
        //    PKCS#10 parse, proof-of-possession, P-256 curve OID. Any failure is the
        //    uniform 400 INVALID_CSR (validation, not an authority rejection â€” no
        ...
        // 4) Sign via the isolated leaf-signing capability â€” the ONLY holder of the
        //    intermediate-CA unwrap; the raw CA private key never enters this
        //    handler. No active intermediate â†’ the retryable 503.
    ...
    private D2Result<O?> DenyWithTelemetry(D2Result authResult, string? immediateCaller)
    {
        ...
        // Issuance is a TARGETLESS capability (no key-domain target) â€” the log's
