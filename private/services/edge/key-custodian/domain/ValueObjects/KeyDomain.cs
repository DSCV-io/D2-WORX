...
/// <c>"jwks-signing"</c> â€” not personally identifying. Do NOT apply
/// <c>[RedactData]</c> to this type.
///
/// <b>Catalog.</b> <see cref="All"/> is the closed catalog â€” the union of the
/// SYMMETRIC-mode <see cref="ProductEncryptionDomains.AllDomains"/> entries (minus
/// <c>"plaintext"</c>, a no-encrypt sentinel, AND minus every SEALED-mode domain â€”
...
/// construction and is NOT a member â€” its per-service sealing keys live under the
...
///
/// <b>Domainâ†’key-type binding.</b> Every catalog entry carries its bound
/// <see cref="Enums.KeyType"/> as a first-class domain fact: <c>jwks-signing</c>
/// â†’ <see cref="Enums.KeyType.RsaSigning"/>; <c>cookie</c> / <c>client-secret</c> â†’
/// <see cref="Enums.KeyType.Secret"/>; the CA domains â†’
/// <see cref="Enums.KeyType.X509CaCertificate"/>; every payload-encryption domain â†’
/// <see cref="Enums.KeyType.AesPayload"/>. Consumers derive the type from the
/// binding â€” there is no second domainâ†’type map anywhere. A <c>(domain, type)</c>
...
/// <see cref="All"/> catalog â€” it is resolved by <see cref="TryResolveCatalogEntry"/>
...
/// wire/spec-anchored constants (Â§5.25 exemption â€” the literal value IS the wire
...
/// query â€” the issuance path signs with the intermediate, never the root.
/// </remarks>
public sealed record KeyDomain
{
    // KC-only domain wire values â€” spec-anchored constants (Â§5.25 exemption).
    ...
    /// (<c>seal:&lt;serviceId&gt;</c>). A structured-prefix, pattern-based family â€”
    /// see the type remarks. The literal is wire/spec-anchored (Â§5.25 exemption).
    /// </summary>
    ...
    // and removed only when the LAST scope disposes â€” a plain remove-by-value would let one
    ...
    /// <summary>
    /// Gets the <see cref="Enums.KeyType"/> canonically bound to this domain â€” the ONLY
    ...
    /// <c>[a-z0-9-]</c>, at most 64 characters) â€” the recipient of a sealed frame IS a
    /// workload â€” and the normalized service id is prefixed with <see cref="SEAL_PREFIX"/>.
    /// </summary>
    ...
    /// the grammar and cannot fail â€” it exists so a caller that already holds a validated
    ...
    /// <see cref="KeyType"/>). For the EF Core read side only â€” use
    ...
    /// still resolves to its canonical entry) but never trims â€” a padded stored
    ...
    public static KeyDomain FromTrusted(string value)
    {
        ...
        if (domain is null)
        {
            throw new ArgumentException(
                ...
                + "key-domain catalog â€” the row is corrupt (every valid row is written "
    ...
    /// anticipates). Default-empty â€” production behavior is byte-for-byte identical when unused.
    /// </summary>
    /// <remarks>
    ...
    /// â€” to simulate the guarded-against re-admission regression â€” stays hermetic against a
    /// concurrently-running rejection pin; such tests MUST also be collection-isolated). Values
    /// carry a Â§7.23 fixture marker in the value itself (e.g. <c>payload-fixture-a</c>).
    /// </remarks>
    ...
    /// <see cref="FromTrusted"/> â€” a future pattern-based domain class (one that is
    ...
    private static KeyDomain? TryResolveCatalogEntry(string normalized)
    {
        ...
        // literal â€” it resolves by validating the service-id suffix against the shared
        // workload-identity grammar and binding EcdhSealing (the seam this method's doc
        // anticipates). A malformed suffix returns null (Create â†’ UnknownKeyDomain;
        // FromTrusted â†’ throw), the same fail-closed shape as an unknown catalog literal.
    ...
    private static IReadOnlyList<KeyDomain> BuildCatalog()
    {
        ...
        // construction, so it is excluded â€” the AES-payload slice is empty until a future
        // symmetric-mode domain is declared in the spec.
        var encDomains = ProductEncryptionDomains.AllDomains
            .Where(d => !string.Equals(d, ProductEncryptionDomains.PLAINTEXT, StringComparison.Ordinal))
            .Where(d => ProductEncryptionDomainModes.ModeFor(d) != ProductEncryptionDomainMode.Sealed)
