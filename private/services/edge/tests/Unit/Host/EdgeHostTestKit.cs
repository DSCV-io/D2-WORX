...
internal sealed class EdgeHostTestKit : IDisposable
{
    ...
    /// <summary>
    /// Sentinel-based walk to the Edge.Api source root (monorepo root with
    /// <c>D2.slnx</c> → <c>private/services/edge/api</c>, or the edge service
    /// directory containing both <c>api/</c> and tests).
    /// </summary>
    /// <returns>Absolute path to <c>private/services/edge/api</c>.</returns>
    ...
    public static string ResolveEdgeApiSourceRoot()
    {
        ...
        while (dir is not null)
        {
            var fromRepoRoot = Path.Combine(
                dir.FullName, "private", "services", "edge", "api");
    ...
    /// <summary>
    /// Absolute path to <c>private/services/edge/tests</c> (sibling of Edge.Api).
    /// </summary>
