// -----------------------------------------------------------------------
// <auto-generated>
//   Generated by the @dcsv-io/d2-typespec-emitters TypeSpec emitter.
//   Source spec: public/contracts/typespec/fixtures/sign-shaped.tsp
//   Manual edits will be lost on rebuild.
// </auto-generated>
// -----------------------------------------------------------------------
#nullable enable

namespace DcsvIo.D2.Private.Edge.Tests.TypeSpecGrpc.Generated;

using SignFixtureInput = global::DcsvIo.D2.Private.Edge.Tests.TypeSpecDto.Generated.SignFixtureInput;
using SignFixtureOutput = global::DcsvIo.D2.Private.Edge.Tests.TypeSpecDto.Generated.SignFixtureOutput;
using DcsvIo.D2.Auth.Outbound.Grpc;
using DcsvIo.D2.Resilience.Pipeline;
using DcsvIo.D2.Resilience.Retry;
using DcsvIo.D2.Result.Grpc;
using Grpc.Core;
using Microsoft.Extensions.DependencyInjection;

/// <summary>Options for the SignFixture gRPC client channel. Supplied by the host composition root.</summary>
public sealed record SignFixtureGrpcClientOptions
{
    /// <summary>gRPC channel endpoint address for the SignFixture service.</summary>
    public required Uri Address { get; init; }
}

/// <summary>
/// Generated DI extension for the SignFixture gRPC client layer.
/// Called from the hand-written host composition root.
/// </summary>
public static class SignFixtureGrpcClientsGeneratedServiceCollectionExtensions
{
    extension(IServiceCollection services)
    {
        /// <summary>
        /// Registers the SignFixture gRPC client: channel, per-op resilience pipelines,
        /// and the <see cref="ISignFixtureGrpcClient"/> → <see cref="SignFixtureGrpcClient"/> binding.
        /// Auto-chains <c>.AddD2ForwardedJwt().AddD2WorkloadCertificate()</c> on the gRPC
        /// client builder so every internal call forwards the request-scoped transaction-token
        /// and presents the workload mTLS leaf — the host never chains the per-channel outbound
        /// auth and so can never forget it. The TLS channel (SecureSsl default) carries the
        /// forwarded-JWT credential, which the inbound transport's ambient holder resolves per
        /// call. The host supplies only the one-time config the generator cannot invent:
        /// <c>AddD2ForwardedJwtOutbound()</c> + <c>AddD2WorkloadCertificateOutbound()</c>.
        /// </summary>
        public IServiceCollection AddD2SignFixtureGrpcClients(SignFixtureGrpcClientOptions options)
        {
            // SignFixtureSigner channel — address from host-supplied options.
            services.AddGrpcClient<global::D2.Services.Protos.SignFixtures.V2Alpha.SignFixtureSigner.SignFixtureSignerClient>(o =>
                o.Address = options.Address)
                // Auto-wired outbound auth — host never chains this (fail-safe).
                .AddD2ForwardedJwt()
                .AddD2WorkloadCertificate();

            // SignFixture resilience pipeline — retry on gRPC transport transients only.
            // Replace with ResilientPipeline<…>.PassThrough in tests that do not need retry.
            services.AddResilientPipeline<string, SignFixtureOutput?>(
                SignFixtureClientKeys.PIPELINE,
                b => b.UseRetries(new RetryOptions<SignFixtureOutput?>
                {
                    IsTransient = ex => ex is RpcException r && ProtoExtensions.IsTransientGrpcException(r),
                }));

            services.AddTransient<ISignFixtureGrpcClient, SignFixtureGrpcClient>();
            return services;
        }
    }
}
