...
/// authority deny branches (every origin ├ù caller ├ù policy) with their telemetry, the
...
public sealed class GetKeyringHandlerTests : IDisposable
{
    ...
    public async Task GetKeyring_ActivePlusRetiring_OrdersActiveFirstThenRetiringNewestActivatedFirst()
    {
        ...
        // Two retiring rows with distinct ActivatedAt â€” newest-activated-first ordering.
    ...
    // -----------------------------------------------------------------------
    // 3. No active key â†’ 503 (+ counter + 9513 log). The 503 fires even when
    ...
    public async Task GetKeyring_NoKeysAtAll_Returns503_FiresCounterAndLog()
    {
        ...
        // The 9513 log carries the domain ONLY â€” no kid, no key material.
    ...
    public async Task GetKeyring_NoActiveButOtherRows_Returns503(KeyStatus status)
    {
        ...
        result.ErrorCode.Should().Be(
            KeyCustodianErrorCodes.KEYCUSTODIAN_KEYRING_KEY_UNAVAILABLE,
            "a keyring with no active kid is unusable â€” 503 even with pending/retiring rows");
    }

    // -----------------------------------------------------------------------
    // 3b. State exclusions â€” compromised / retired never served
    // -----------------------------------------------------------------------
        ...
        RequestOrigin origin)
    {
        ...
        using (var listener = BuildAuthorityListener(tags))
        {
            ...
            // Uniform 403 wire code â€” telemetry distinguishes the plane deny.
            result.ErrorCode.Should().Be(
    ...
    //    authority (403) â€” uniform with an unauthorized payload domain, no domain-type
    ...
    // so lifecycle ops work on seal domains) â€” the keyring surface must deny it exactly
        ...
        string nonPayload)
    {
        ...
        // Same 403 code as an unauthorized PAYLOAD domain â€” no domain-type oracle. The
        ...
        result.ErrorCode.Should().NotBe(
            ...
            "the authority arm denies a non-payload domain BEFORE the type fork â€” no oracle");
    }
    ...
    // the belt-and-braces AesPayload type fork â€” a seal private key can NEVER be served
