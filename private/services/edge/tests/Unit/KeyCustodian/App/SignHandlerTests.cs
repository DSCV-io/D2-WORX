...
/// 400 key-type mismatch ΓÇö never the retryable 503. The shared signing core's
/// happy path (signature verification, empty input, no active key, corrupt material)
/// is exercised through the minter capability in <c>JwtSigningCapabilityTests</c> ΓÇö
...
public sealed class SignHandlerTests : IDisposable
{
    ...
    public void KeyDomainSigner_StaysInternal_NoExternalSigningOracle()
    {
        ...
        // reachable from outside the App assembly â€” no external caller can bypass an
        ...
        string domain)
    {
        ...
        // hold a signing key â€” a PERMANENT 400, not the retryable 503 that "no active
    ...
    public async Task Sign_CrossProcessGrantedNonSigningDomain_WithAnomalousRsaKey_Still400()
    {
        ...
        // cannot resurrect the general surface â€” the binding, not the store contents,
    ...
    public async Task Sign_CrossProcessJwksSigning_ReturnsMinterCapabilityRequired()
    {
        ...
        // allow â€” the cluster root is reachable only through the minter capability.
        var result = await Build(
    ...
    public async Task Sign_UnestablishedOrigin_SignsNothing(string domain)
    {
        ...
        // The scoped default â€” no boundary established the origin. Signs nothing, for any
    ...
    {
        // No internal.kc.sign scope on the request context â†’ BaseHandler's per-handler
        ...
        // authorize (files â†’ the fixture payload domain, cross-process) and the binding check
        // would then surface the 400 type mismatch â€” so a Forbidden (not the 400) proves the
    ...
    public async Task Sign_WithRequiredScope_PassesScopeGate_ReachesAuthorityAndBinding()
    {
        ...
        // internal.kc.sign present â†’ the scope gate admits the call; the request then
