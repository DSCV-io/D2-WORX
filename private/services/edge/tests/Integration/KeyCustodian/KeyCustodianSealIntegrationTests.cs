...
/// <see cref="GetOrLazyProvisionSealPublicKeyHandler"/> ΓåÆ build a producer <see cref="RecipientPublicKeyring"/>
/// from the served SPKI ΓåÆ <see cref="PayloadSealer"/> a payload ΓåÆ fetch the recipient's OWN
/// private key via the REAL <see cref="GetOrLazyProvisionOwnSealPrivateKeyHandler"/> ΓåÆ build a
/// <see cref="RecipientPrivateKeyring"/> ΓåÆ <see cref="PayloadOpener"/> the frame ΓåÆ assert the
...
public sealed class KeyCustodianSealIntegrationTests(KeyCustodianPostgresFixture fixture)
{
    ...
    public async Task SealProvisioningRace_ConcurrentFirstRequests_CollapseToOneWinner_NoConflict()
    {
        ...
        // Scopes are held until WhenAll completes â€” do NOT resolve via a discarded
    ...
    public async Task SealEncryptBroad_ProducerFetchesEveryTargetPublic_ButOnlyItsOwnPrivate()
    {
        ...
        // gets ITS OWN key (seal:edge), never any target's â€” the op carries no target.
        await using (var recipient = BuildProvider(clock, rootCrypto, _PRODUCER, sealEncrypt: false))
        {
            ...
            priv.Data!.ActiveKid.Should().Be(
                ownKid, "getOrLazyProvisionOwnSealPrivateKey serves seal:edge â€” the caller's own domain");
        }
    }
    ...
    public async Task SealProvisioning_MaxLengthServiceId_PersistsThe69CharDomain()
    {
        ...
        // (varchar(69)) â€” a 64-char service id must provision, not fail the INSERT.
    ...
    {
        var scope = sealEncrypt ? ProductScopes.Internal.Kc.Seal.Encrypt : ProductScopes.Internal.Kc.Seal.Open;
