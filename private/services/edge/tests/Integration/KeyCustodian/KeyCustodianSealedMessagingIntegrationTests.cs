...
/// REAL bus path (<c>RabbitMqMessageBus</c> ΓåÆ Testcontainer RabbitMQ); the CONSUMER host ΓÇö
/// whose sealer/opener keys come from the REAL KeyCustodian seal ops served over
/// loopback gRPC + Testcontainer PostgreSQL (lazy provisioning included) ΓÇö consumes and OPENS
...
/// is the faithful stand-in for the interceptor-established mTLS peer identity ΓÇö the loopback
    ...
    KeyCustodianPostgresFixture fixture)
{
    ...
    // Warm-up rotation domain/kid for the consumer-ready probe (never a real seal domain ΓÇö a
    // ┬º7.23 fixture marker in the value). A KeyRotatedEvent on this domain that round-trips
    // through the host's own KeyringRefreshSubscriber ΓåÆ RabbitMqRotationEventChannel proves
    ...
    // multi-minute ceiling ΓÇö far above any healthy broker round-trip, yet a
        ...
        SealedPublish_RealBusRealKeys_ConsumerOpens_ProducerCannot_ForgottenCallCrashes()
    {
        ...
        try
        {
            ...
            // this test PROCESS, so concurrent hosts would contend on it ΓÇö a test-environment
            ...
            // 1) CONSUMER host boots FIRST: own id = the sealed domain's consumer service ΓåÆ
            ...
            using (var consumerBoot = BuildConsumerHost(broker, channel, recorder))
            {
                ...
                // not-yet-declared exchange (a 404 ΓåÆ AlreadyClosed ΓåÆ ServiceUnavailable). Wait
                // for the consumer to be genuinely ready first ΓÇö a warm-up rotation event that
            ...
            // 2) PRODUCER host: NOT a sealed consumer ΓåÆ sealers only, structurally NO opener.
                ...
                services => services.AddD2SealedEncryptionViaKeyCustodian(_PRODUCER_SERVICE)))
            {
                ...
                try
                {
                    ...
                    // holds NO opener registration ΓÇö for the consumer service or itself.
            ...
            // channel opens ΓÇö the SealedConsumerStartupCheck is registered by
    ...
    // the targetless own-private op selects seal:audit ΓÇö exactly the peer-selection shape).
        ...
        var host = new HostBuilder()
            .ConfigureWebHost(web =>
            {
                ...
                web.ConfigureServices(services =>
                {
                    ...
                    // The seal ops' activation announce is not under test ΓÇö recording fake.
    ...
    // host's own KeyringRefreshSubscriber ΓåÆ RabbitMqRotationEventChannel ΓÇö deterministic proof
    ...
    private static void RegisterSealedFixtureMessage()
    {
        ...
        MessageWireResolver.RegisterForTesting(
            ...
            new MqMessageDescriptor(
                ...
                Exchange: _EXCHANGE,
                ExchangeType: "fanout",
                Encryption: ProductEncryptionDomains.AUDIT,
    ...
    // The seal keypairs are wrapped under THIS run's random root key ΓÇö stale rows from a
    // previous run (wrapped under a different root) would fail the unwrap, so every seal
    // domain this test lazily provisions is wiped first (audit children before keys ΓÇö the
    // FK is RESTRICT).
