...
`private/services/files/clients/dotnet/D2.Files.Client.csproj` — gRPC client + multi-tier cache (memory → Redis → service) per the [docs/PATTERNS.md](../../../docs/PATTERNS.md) cache pattern. Uses `DefaultOptions` log-suppression for proto-DTOs that can't carry `[RedactData]`.

## References

- [docs/PATTERNS.md](../../../docs/PATTERNS.md) — handler / D2Result / RedactionSpec / cache patterns
- [`public/packages/dotnet/messaging/rabbitmq/README.md`](../../shared/dotnet/messaging/rabbitmq/README.md) — `d2.files.events` exchange contract + RabbitMQ wire format
