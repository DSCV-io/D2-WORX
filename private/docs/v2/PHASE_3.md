...
**Status**: 🔄 In progress — KeyCustodian (K1) shipped; **T1** (TS caching twin / 0028) **SHIPPED**; **A1** (Edge host platform slice / deliverable **0030**) **SHIPPED** (`docs/dev/deliverables/0030-edge-host.md`); **D2** (advisory-lock domain keys / deliverable **0031**) **SHIPPED** — shared Postgres mechanism only; KC owns `AdvisoryLocks.D2Keycustodian.*`. **Post-0030 / post-D2 product spine (revised):** **D2 (0031) → A2 Auth Core (mint pre-reqs) → A3 Minting → Auth Extras + E1 → E2**. Supersedes the earlier “mint first, then sessions” wording: a proper mint **depends on** Auth domain prims/storage (users, credentials, sessions, org context, …), so those land before embossing.
...
- **All cross-cutting middleware** — rate limiting + fingerprinting ([PHASE_3_RATE_LIMITING.md](PHASE_3_RATE_LIMITING.md) + [PHASE_3_FINGERPRINTING.md](PHASE_3_FINGERPRINTING.md); full design map [PHASE_3_AUTH_CORE.md §0](PHASE_3_AUTH_CORE.md)), request enrichment, JWT validation, idempotency (`Idempotency-Key` header + Redis `SET NX`), CSRF, CORS.
...
Phase 3 is pre-integration — no downstream service wiring yet. Auth events will have nowhere to land until DcsvIo.D2.Private.Audit ships in Phase 4; that is acceptable for the Phase 3 build window.
...
| [PHASE_3_AUTH_CORE.md](PHASE_3_AUTH_CORE.md) | **Auth Core domain design keep** + **§0 full-branch design-review map** — L9–L163; start here for Fable on all auth design on `n/auth-core` |
| [PHASE_3_RATE_LIMITING.md](PHASE_3_RATE_LIMITING.md) | Rate limiting design — token-bucket, AND of ceilings, confidence-keyed dims, dirty-IP, kill switches (read after fingerprinting annex) |
| [PHASE_3_FINGERPRINTING.md](PHASE_3_FINGERPRINTING.md) | Fingerprinting / device confidence — deviceKey, too-common popularity, session binding, WhoIs, risk/step-up |
...
| [PHASE_3_AUTH_CORE_DESIGN_AUDIT.md](PHASE_3_AUTH_CORE_DESIGN_AUDIT.md) | Hostile design audit — C/H/M + Qs remediated. Re-audit / Fable optional before PLAN. |
...
| **Rate limit + fingerprint** | Token-bucket AND of ceilings; confidence (`deviceKey`/IP/userId); dirty WhoIs; too-common → IP primary; kill switches; Restricted fail-closed. [PHASE_3_FINGERPRINTING.md](PHASE_3_FINGERPRINTING.md) + [PHASE_3_RATE_LIMITING.md](PHASE_3_RATE_LIMITING.md). |
...
| **3-tier sessions** | Cookie cache ~5 min → Redis → PostgreSQL `d2-auth.session` dual-write. **Policy SoT:** [PHASE_3_AUTH_CORE.md](PHASE_3_AUTH_CORE.md) §7.3/§7.9–§7.10 — elevate **rotates session id**; hard yeet vs soft re-mint; idle/absolute timeouts (illustrative defaults 72h idle / 30d absolute; Redis TTL ≥ absolute+buffer e.g. 45d). Revocation: PG→Redis→`d2.security.session-revoked` → drop L1. No sticky sessions. |
...
|---|---|---|---|---|
| **K1** | **KeyCustodian** | Key state machine (`pending → active → retiring → retired → compromised`), RSA gen/storage (root-key-wrapped), `encryption_key` + audit schema, rotation cadences, compromise runbook. Peer module within Edge (no standalone service). Builds on `DcsvIo.D2.Encryption`, EF/PG, `IClock`. | M–L | ✅ Shipped (`n/keycustodian`) |
| **C0** | **Unified operation-contract IDL** | TypeSpec front-end + the `@dcsv-io/d2-typespec-emitters` fleet (C#/TS DTOs · proto · OpenAPI · route+policy · in-process leaf · parity) + the `@d2*` decorator vocabulary + the proven dual REST+gRPC binding convention. One source per operation → every representation across the external-REST/SSE, internal-gRPC, and in-process planes. Platform-wide; surfaces first at Edge. See [ADR-0021](../adrs/0021-unified-operation-contract-idl.md). | L | ✅ Shipped (`n/typespec-emitters`, deliverable 0019) |
| **T1** | **TS tiered cache + Redis invalidation-backplane twin** | **Full** TypeScript twin of the .NET caching stack (ADR-0008 surface: Basic + Atomic + Broadcast + Set + tiered + backplane) — layout mirror under `public/packages/typescript/caching/{abstractions,local-default,distributed-redis,tiered}/` (`@dcsv-io/d2-caching-*`). Shared invalidation channel `d2:cache:invalidations` with .NET; universal "everyone acts". Full cross-runtime parity bar. Why now: multi-instance BFF (mesh member) WILL cache between replicas (user trigger 2026-07-03); prior V2 "cache packages out of scope" framing is **superseded**. Prior art: v1 TS Redis/memory cache + .NET `DcsvIo.D2.Caching.*`. Zero KeyCustodian coupling. Deliverable **0028**, branch `n/ts-caching`. | S–M | ✅ SHIPPED 2026-07-10 (`n/ts-caching`; snapshot `docs/dev/deliverables/0028-ts-caching.md`) — REVIEW pending |

> **Foundational ordering (per [ADR-0021](../adrs/0021-unified-operation-contract-idl.md)).** The contract IDL (**C0**) is a foundational deliverable that lands **before** every endpoint-bearing deliverable — Auth Core / Minting / Auth Extras and the E-track all *define endpoints*, so the IDL must precede them to avoid hand-writing endpoints that would later be migrated. C0 shipped first; KeyCustodian transport + Edge host Map surfaces (well-known HTTP, six KC gRPC services, Audit Ping bridge) now exist on disk under A1. Remaining product Auth REST / mint endpoints still land after C0 (A2+). The TypeSpec engine choice was validated by a supervised spike (the dual REST+gRPC single-source binding proven on real running code).

### Auth track (self-rolled .NET Auth module — no BetterAuth)

> **Order law (locked):** **Auth Core → Minting → Auth Extras.**  
> A proper mint is **downstream of** Auth domain prims/storage (session id, liveness, principal, credential material, org operating-context, client registry, …). Do **not** ship embossing first with fixture sessions/users. Letters **A2/A3** below are redefined to this order (supersedes “A2 = token mint, A3 = sessions” from pre-revision spines, including the D2/0031 KEEP wording).

| # | Deliverable | Scope | Size | Status |
|---|---|---|---|---|
| **A1** | **Edge host shell** | Edge ASP.NET host (`api/app/domain/infra` + tests): `AddD2EdgeHost` / `UseD2EdgePipeline` / `MapD2EdgeEndpoints` (health + well-known + six KC gRPC Maps with `Scopes.Internal.Kc.*` + Audit bridges), three-bind Kestrel (8080/8443/9443), CSR outbound issuer, Compose `d2-edge`/`d2-audit` multiproc stubs. The host the auth module will live in — no Auth DB of its own. No upstream deps — parallel with K1. Deliverable **0030** on `n/edge-host`. | M | ✅ **SHIPPED 0030** (platform slice: composition + KC transport + Audit multiproc stub + Compose dual-target; FR_FULL closed — see `docs/dev/deliverables/0030-edge-host.md`). Residual product Auth → **A2 Auth Core+** (not “mint-only”) |
| **A2** | **Auth Core** (mint pre-reqs + auth domain BE) | Everything a **correct** mint and real auth product must already have — **not** JWT embossing. Module-within-host + `d2-auth` EF. **Design SoT:** [PHASE_3_AUTH_CORE.md](PHASE_3_AUTH_CORE.md). Domain/storage/APIs for: user lifecycle SM, multi credential methods (+ IdP/SCIM **BE**), challenges, sessions (3-tier + no-org), org + tree + membership (hot/history), invitations (mechanisms), security policy tables+resolve, package-limits port+projection, `oauth_client`, sign-in attempts, dual-audit publish path, retention jobs. Compose like KeyCustodian. **Out of A2 starring exit:** embosser / `POST /oauth/token` (A3), anon Pattern A product (Extras+E1), full account self-mgmt UX (A4), admin **UI** polish. Multi-step deliverable; design locked before PLAN. O23/O24 design annexes ready for Fable. | L | ☐ Pending — **next** (optional Fable on design set) |
| **A3** | **Minting** | Boundary issuance **on top of Auth Core facts**. Auth composition sole home of `AddD2JwtSigningCapability` / `IJwtSigningCapability` (embosser isolation). Claim builder (locked 16-claim payload; thin slots only where Core has no fact yet). `POST /oauth/token` mints the **one** internal transaction-token (`aud=d2.internal`; RFC 8693 retained for boundary mint + exceptions per [ADR-0022](../adrs/0022-service-auth-mint-once-forward.md)); forward-unchanged thereafter. OIDC discovery `token_endpoint` fields; mint↔validate on live path; multiproc dual-factor proof (AUTH-R3/R4/R5) with a **Core-backed** session principal — not fixture-only cosplay. Backend workload identity remains mTLS ([ADR-0023](../adrs/0023-mtls-workload-identity.md)). | M–L | ☐ Pending (after A2) |
| **Auth Extras** | **Credential product polish + anon + enrichment** | Product HTTP/UX that **consumes** Core + Mint (Core already owns password suite, challenges, throttle delay, `sign_in_attempt`/`sign_in_event` storage — see [PHASE_3_AUTH_CORE.md](PHASE_3_AUTH_CORE.md)). Extras: sign-up/sign-in/sign-out/get-session polish, reset/change flows, risk enrichment + `auth.whois-resolution`, fingerprint bind at elevation, **anon Pattern A** (pairs with **E1** for `d2_whois_id`). May ship fat with E1 or split. | L | ☐ Pending (after A3; E1 co-requisite for anon) |
...
| **A5** | **Orgs product surface** (beyond Core) | Full org CRUD **UI/product polish**, invite admin UX, org-contacts, cleanup jobs — **on top of** A2 Auth Core org/membership/invite **mechanisms** (Core already holds tree, membership, invite domain/storage). **No org emulation** (dead in v2; impersonation only). | L | ☐ Pending |
| **A6** | **Impersonation + admin surface** | Admin impersonation (RFC 8693 act-chain, consent + force, scope stripping, time-limited), impersonation claims, staff suspend/unsuspend product UX (lifecycle SM is Core). Depends on Minting (A3) + org/admin product. | M | ☐ Pending |
...
| **E5** | **Keyring distribution + scheduled-jobs receiver** | `DcsvIo.D2.Private.Edge.KeyCustodian.Client` (consumer-side: `IKeyringClient` / gRPC, rotation event channel, `KeyringBackedPayloadCrypto`), Edge `internal/keys/{domain}` gRPC, `IHostedService` cron receiver (key-rotation checks). | M | ☐ Pending |
...
```
K1 (KeyCustodian) ─┐
                   ├─► A2 Auth Core (domain/storage/prims)
A1 (Edge host) ────┘         │
                             ▼
                      A3 Minting (emboss on Core facts)
                             │
              ┌──────────────┼──────────────────────┐
              ▼              ▼                      ▼
     Auth Extras      E1 (WhoIs/FP) ──needs──► anon mint in Extras
     (credential           │
      product polish)      └─► E2 (rate-limit: needs claims + E1)
              │
              ├─► A4 (account self-mgmt)
              ├─► A5 (orgs product beyond Core prims) ─► A6 (impersonation; needs Minting)
              └─► E4 (SSE: needs sessions + JWT)
E5 (keyring) needs K1 keys (independent of mint order once KC lives)
(E3 YARP: independent; lands once backends exist)
```

**Critical-path spine (post-D2 product order) = D2 (0031) → A2 Auth Core → A3 Minting → Auth Extras + E1 → E2**

- K1 and A1 shipped in parallel (K1 + 0030).
- **D2** (0031) hygiene detour: shared Postgres mechanism only; domain lock keys on KC Infra.
- **A2 Auth Core** = mint pre-reqs + full auth **domain BE** per [PHASE_3_AUTH_CORE.md](PHASE_3_AUTH_CORE.md) (lifecycle, tree, invites, policy, IdP/SCIM APIs, package projection, …). **No** embossing as starring work.
- **A3 Minting** = claim builder + embosser isolation + `POST /oauth/token` + mint↔validate + multiproc dual-factor on **Core-backed** principals.
- **Auth Extras + E1** = credential product polish + anon Pattern A paired with WhoIs (anon needs `d2_whois_id`).
- **E2** = rate-limit + cross-cutting middleware (needs real claim shape + E1 enrichment).
- Foundational DAG: `(K1 ∥ A1) → Auth Core → Minting → Auth Extras` (not mint-before-sessions).

### Minimal "Edge stands up" milestone

**K1 + A1 + Auth Core + Minting + E1 + Auth Extras** → keys, host, domain facts, own-token issuance/validation, enrichment, anon + authed users. First natural user-review checkpoint.

### MVP-cut candidates (can defer without blocking the milestone)

| Deliverable | Why it defers cleanly |
|---|---|
| A6 Impersonation | High-value but self-contained; depends on Minting + A5 product |
...
| A5 org-contacts / invite **UI** polish | Auth Core already owns invite/tree/membership **mechanisms**; A5 is product surface (no emulation) |
...
1. **Edge-auth-module ADRs** — session state machine, OAuth client registry, and impersonation model each need an ADR, written *as* Auth Core / Minting / later product ships are planned and built, not upfront. The broader auth-system ADRs (token model, mTLS workload identity) are already accepted: ADR-0005, ADR-0007, ADR-0012, ADR-0022, ADR-0023. The shared auth libs are built and shipped.
2. **`docs/JWT-CLAIMS.md` prose catalog + claim-spec additions** — the 16-claim payload is locked (see [PHASE_3_AUTH.md §3.1](PHASE_3_AUTH.md)) and `contracts/jwt-claims/jwt-claims.spec.json` exists. Still needed: prose catalog; **tree claims** `d2_parent_org_id` / `d2_root_org_id` ([PHASE_3_AUTH_CORE.md](PHASE_3_AUTH_CORE.md) L72 — names locked, emit at mint); 3 anon claims (`d2_kind`, `d2_whois_id`, `d2_fingerprint_score`) with **Auth Extras + E1**.
...
Durable post-D2 product spine: **D2 (0031) → A2 Auth Core → A3 Minting → Auth Extras + E1 → E2**. Foundational DAG `(K1 ∥ A1) → Auth Core → Minting → Auth Extras`. Supersedes “A2 mint then A3 sessions.” A1–E2 remain in-host / browser-facing — none blocked on mTLS or the first-leaf bootstrap.
...
3. **A2 Auth Core** — Auth module + `d2-auth` per [PHASE_3_AUTH_CORE.md](PHASE_3_AUTH_CORE.md) (design-first; multi-step). **No** product mint / embosser starring exit (A3).
4. **A3 Minting** — embosser isolation + claim builder + `POST /oauth/token` + discovery `token_endpoint` + mint↔validate + multiproc dual-factor on Core-backed principals.
5. **Auth Extras + E1** — credential product polish + anon Pattern A **paired with** WhoIs + fingerprint (anon needs E1's `d2_whois_id`; adds 3 anon claims to the spec).
6. **E2** — net-new middleware: 18-bucket rate-limit + HTTP idempotency + CSRF enforcement + security headers + i18n (needs mint claim shape + the generated markers from the deferred-work checklist §F).
7. **mTLS cross-process residual (first-leaf bootstrap)** — Edge host already Maps `IssueWorkloadCertificate` over gRPC and wires server-side mTLS (A1 checklist; G master CLOSED). Remaining open work is first-leaf bootstrap design only (deployment-orchestrator concern; open prerequisite #3).
8+. **E4** (SSE), **A4/A5/A6** (account / orgs product / impersonation), then **E3** (YARP routing) + **E5** (keyring) — as their first consumers arrive.
...
| # | Item | Status |
| - | ---- | ------ |
| B1 | `D2_INTERNAL_AUDIENCE` constant in `DcsvIo.D2.Auth.Abstractions` | ✅ done in 0023 |
...
| # | Item | Status | Blocked on |
| - | ---- | ------ | ---------- |
| B7 | Edge `/oauth/token` boundary minter | 📐 specified-deferred | PHASE_3 **A3 Minting** (after **A2 Auth Core**) |
| B8 | Anon-JWT minting (Pattern A) | 📐 specified-deferred | PHASE_3 **Auth Extras + E1** (sets `d2_whois_id`; needs E1) |
...
| B13 | Over-the-wire mint↔validate parity test | 📐 specified-deferred | Running minter + validator (PHASE_3 **A3 Minting**; Core facts from **A2**) |
...
| B18 | `@dcsv-io/d2-auth-bff-client` package missing — `private/services/web` typechecks blocked | 📐 specified-deferred | PHASE_3 BFF rebuild (Phase 7) |
...
| The generated TS SSR gRPC client fns (validated against the real `@dcsv-io/d2-grpc-client` seam + real ts-proto types) run against a hand-written BFF composition root (`getChannel`, context-propagation interceptor, boundary-token cache) | BFF gRPC composition root — wire the generated TS server client fns against the real channel + interceptors | BFF rebuild (Phase 7) | C10 + B11 |
...
| ~~KeyCustodian well-known routes~~ — **CLOSED (A1)** | Edge host `MapD2EdgeEndpoints` calls `MapGetJwksRoute()` + `MapGetOidcConfigurationRoute()` | A1 platform slice; **A3 Minting** still owns `token_endpoint` discovery extension | A3 Minting (token fields only) |
| `GetOidcConfigurationOutput` minimal discovery document (`issuer` + `jwks_uri` + `id_token_signing_alg_values_supported` + the `response_types_supported` / `subject_types_supported` placeholders) | Auth module — extend the discovery document with `token_endpoint` + the OAuth grant-type / response-mode fields once the token endpoint exists | token endpoint built (PHASE_3 **A3 Minting**) | A3 Minting |
...
| ~~Signing-capability isolation + Edge host~~ — **PARTIAL (A1)** | Edge host registers `AddD2CaLeafSigningCapability` + `AddD2CaRootSigningCapability`; **JWT minter remains structurally absent** on the general host (`IJwtSigningCapability` null). Host DI isolation tests pin CA present + JWT absent. Auth-mint composition root (**A3 Minting**, after **A2 Auth Core** module exists) is the sole future home for `AddD2JwtSigningCapability` | Auth-mint composition (PHASE_3 **A3 Minting**) for JWT minter only | 0026 / Edge host |
