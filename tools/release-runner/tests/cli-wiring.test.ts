...
describe("--legacy-commit-type routing (unit)", () => {
  const pkg: PackageDescriptor = {
    name: "@d2/utilities",
    ecosystem: "npm",
    dir: "server/shared/typescript/utilities",
    manifestPath: "server/shared/typescript/utilities/package.json",
    changelogPath: "server/shared/typescript/utilities/CHANGELOG.md",
    currentVersion: "0.1.0",
    dependencies: [],
  };
  ...
  it("WITH --legacy-commit-type: runRelease is the bump source (commit type drives bump)", () => {
    ...
    const commitRecord = {
      ...
      files: ["server/shared/typescript/utilities/src/index.ts"],
    };
