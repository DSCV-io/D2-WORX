 ...
 * (`.git` or `server/D2.slnx`) is found. Depth-invariant — no hardcoded
...
function findRepoRoot(startDir) {
  ...
  while (true) {
    ...
    if (parent === current) {
      throw new Error(
        `repo-root sentinel: no .git or server/D2.slnx found walking up from ${startDir}`,
      );
    }
...
test("findRepoRoot is depth-invariant (sentinel, not fixed ../../..)", () => {
  ...
  assert.ok(
    ...
    "sentinel walk must land on a directory that owns server/D2.slnx",
  );
  assert.ok(
