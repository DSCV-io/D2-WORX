...
internal static class JsonConverterEmitter
{
        ...
        IReadOnlyList<string> codes)
    {
        ...
        sb.AppendLine("    /// <c>TryParse</c> path so both routes share a single source of truth.");
        sb.AppendLine("    /// </summary>");
        ...
        sb.AppendLine(
            $"    public static bool IsKnown(string value) => {validSetFieldName}.Contains(value);");
        sb.AppendLine();
        ...
        sb.AppendLine(
            $"                $\"Expected a string for {displayLabel}; got {{reader.TokenType}}.\");");
        sb.AppendLine("        }");
        sb.AppendLine();
        ...
        sb.AppendLine("            throw new JsonException(");
        sb.AppendLine(
            $"                \"Expected a non-empty {displayLabel}; got null / empty / whitespace.\");");
        sb.AppendLine("        }");
        sb.AppendLine();
        ...
        sb.AppendLine("            throw new JsonException(");
        sb.AppendLine(
            $"                $\"Unknown {displayLabel} '{{value}}' — not present in the closed catalog set.\");");
        sb.AppendLine("        }");
        sb.AppendLine();
