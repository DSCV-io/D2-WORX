...
internal static class GeopoliticalEntityDataEmitter
{
    ...
    public static ImmutableArray<EmitResult> EmitAll(GeoSpecContext context)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            sb.AppendLine(
                $"    /// <summary>{DefaultEmitterHelpers.EscapeXmlDoc(entry.Name)} ({entry.ShortCode}).</summary>");
            sb.AppendLine(
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        // First pass: construct every record (MemberCountries empty until wire-nav).");
        sb.AppendLine("        var byCode = new Dictionary<GeopoliticalEntityCode, GeopoliticalEntity>();");
        seen.Clear();
        foreach (var entry in entries)
        {
            if (entry.ShortCode.Falsey() ||
        ...
        sb.AppendLine("        {");
        sb.AppendLine("            var resolved = new List<Country>(entity.MemberCountryIso31661Alpha2Codes.Count);");
