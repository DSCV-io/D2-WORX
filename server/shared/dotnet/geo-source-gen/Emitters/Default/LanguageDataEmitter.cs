...
internal static class LanguageDataEmitter
{
    ...
    public static ImmutableArray<EmitResult> EmitAll(GeoSpecContext context)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            sb.AppendLine(
                $"    /// <summary>{DefaultEmitterHelpers.EscapeXmlDoc(entry.Name)} ({entry.Iso6391Code}).</summary>");
            sb.AppendLine(
                $"    public static Language {member} => LanguageLookup.ByCode[LanguageCode.{member}];");
            sb.AppendLine();
        }
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        // First pass: construct every Language record (reverse navs empty until wire-nav).");
        ...
        foreach (var entry in entries)
        {
            ...
            sb.AppendLine(
                $"        byCode[LanguageCode.{member}] = " + LanguageRecordLiteral(entry, member) + ";");
        }
        ...
        sb.AppendLine("            byIso[kvp.Value.Iso6391Code.ToString().ToLowerInvariant()] = kvp.Value;");
        sb.AppendLine();
        sb.AppendLine("        ByIso6391 = byIso.ToFrozenDictionary(System.StringComparer.OrdinalIgnoreCase);");
        sb.AppendLine();
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        var countriesByLanguage = new Dictionary<LanguageCode, List<Country>>();");
        ...
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        var localesByLanguage = new Dictionary<LanguageCode, List<Locale>>();");
        ...
        sb.AppendLine("        {");
        sb.AppendLine("            if (countriesByLanguage.TryGetValue(language.Iso6391Code, out var countries))");
        sb.AppendLine("            {");
        ...
        sb.AppendLine();
        sb.AppendLine("                language.SpokenInCountryIso31661Alpha2Codes = countryCodeSet.ToFrozenSet();");
        sb.AppendLine("            }");
        sb.AppendLine();
        sb.AppendLine("            if (localesByLanguage.TryGetValue(language.Iso6391Code, out var locales))");
        sb.AppendLine("            {");
        ...
        sb.AppendLine();
        sb.AppendLine("                language.LocaleIetfBcp47Tags = localeCodeSet.ToFrozenSet();");
        sb.AppendLine("            }");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine("}");
    ...
    private static string LanguageRecordLiteral(LanguageSpec entry, string member)
    {
        ...
        sb.Append($"Endonym = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.Endonym ?? entry.Name)}\", ");
        sb.Append($"WritingDirection = {DefaultEmitterHelpers.MapWritingDirection(entry.WritingDirection)}, ");
