...
/// <list type="bullet">
///   <item><description>Static <c>Countries</c> accessor (<c>Countries.US</c> → <c>Country</c>).</description></item>
///   <item><description>Static <c>CountryLookup</c> class with the FrozenDictionary indexes.</description></item>
///   <item><description><c>WireNav()</c> wire-nav method invoked by the coordinator.</description></item>
/// </list>
/// </summary>
/// <remarks>
/// <para>
...
internal static class CountryDataEmitter
{
    ...
    public static ImmutableArray<EmitResult> EmitAll(GeoSpecContext context)
    {
        ...
        {
            sb.AppendLine(
                $"    /// <summary>{DefaultEmitterHelpers.EscapeXmlDoc(entry.DisplayName)} ({entry.Iso31661Alpha2Code}).</summary>");
            sb.AppendLine(
                $"    public static Country {entry.Iso31661Alpha2Code} => CountryLookup.ByCode[CountryCode.{entry.Iso31661Alpha2Code}];");
            sb.AppendLine();
        }
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        // First pass: construct every Country record with default/empty nav values.");
        sb.AppendLine("        var byCode = new Dictionary<CountryCode, Country>();");
        sb.AppendLine();

        foreach (var entry in entries)
            EmitCountryRecordConstruction(sb, entry, validCurrencyMembers, validLanguageMembers, validGpeMembers);
        ...
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        ByIso31661Alpha2 = byAlpha2.ToFrozenDictionary(System.StringComparer.OrdinalIgnoreCase);");
        sb.AppendLine("        ByIso31661Alpha3 = byAlpha3.ToFrozenDictionary(System.StringComparer.OrdinalIgnoreCase);");
        sb.AppendLine();
        ...
        HashSet<string> validGpeMembers)
    {
        ...
        sb.AppendLine(
            $"            Iso31661Alpha3Code = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.Iso31661Alpha3Code)}\",");
        sb.AppendLine(
            $"            Iso31661NumericCode = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.Iso31661NumericCode)}\",");
        sb.AppendLine(
            $"            DisplayName = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.DisplayName)}\",");
        sb.AppendLine(
            $"            OfficialName = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.OfficialName)}\",");
        sb.AppendLine(
            $"            EndonymDisplayName = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.EndonymDisplayName ?? entry.DisplayName)}\",");
        sb.AppendLine(
            $"            EndonymOfficialName = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.EndonymDisplayName ?? entry.OfficialName)}\",");
        sb.AppendLine(
            $"            PhoneNumberPrefix = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.PhoneNumberPrefix ?? string.Empty)}\",");
        sb.AppendLine(
            $"            PhoneNumberNationalFormat = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.PhoneNumberNationalFormat ?? string.Empty)}\",");
        sb.AppendLine(
            $"            PhoneNumberMinDigits = {DefaultEmitterHelpers.IntLiteralOrNull(entry.PhoneNumberMinDigits)},");
        sb.AppendLine(
            $"            PhoneNumberMaxDigits = {entry.PhoneNumberMaxDigits ?? 0},");
        sb.AppendLine(
            $"            FirstDayOfWeek = {DefaultEmitterHelpers.MapDayOfWeek(entry.FirstDayOfWeek)},");
        sb.AppendLine(
            $"            WeekendStart = {DefaultEmitterHelpers.MapDayOfWeek(entry.WeekendStart)},");
        sb.AppendLine(
            $"            WeekendEnd = {DefaultEmitterHelpers.MapDayOfWeek(entry.WeekendEnd)},");
        sb.AppendLine(
            $"            MeasurementSystem = {DefaultEmitterHelpers.MapMeasurementSystem(entry.MeasurementSystem)},");
        ...
        if (primaryLangMember is not null && validLanguageMembers.Contains(primaryLangMember))
            sb.AppendLine($"            PrimaryLanguageIso6391Code = LanguageCode.{primaryLangMember},");
        else
        ...
        if (entry.PrimaryCurrencyIso4217AlphaCode.Truthy()
            ...
            sb.AppendLine($"            PrimaryCurrencyIso4217AlphaCode = CurrencyCode.{entry.PrimaryCurrencyIso4217AlphaCode},");
        else
            sb.AppendLine("            PrimaryCurrencyIso4217AlphaCode = null,");

        if (entry.PrimaryLocaleIetfBcp47Tag.Truthy())
            sb.AppendLine($"            PrimaryLocaleIetfBcp47Tag = LocaleCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(entry.PrimaryLocaleIetfBcp47Tag!)}\"),");
        else
        ...
        if (entry.SovereignCountryIso31661Alpha2Code.Truthy()
            ...
            sb.AppendLine($"            SovereignCountryIso31661Alpha2Code = CountryCode.{entry.SovereignCountryIso31661Alpha2Code},");
        else
        ...
        EmitTypedGpeSet(sb, "GeopoliticalEntityShortCodes", entry.GeopoliticalEntityShortCodes, validGpeMembers);
        EmitTypedCurrencySet(sb, "CurrencyIso4217AlphaCodes", entry.Currencies, validCurrencyMembers);
        ...
        foreach (var cc in entry.Currencies)
        {
            ...
            sb.AppendLine("                {");
            sb.AppendLine($"                    Iso4217AlphaCode = CurrencyCode.{cc.Iso4217AlphaCode},");
            sb.AppendLine(
                $"                    Level = {DefaultEmitterHelpers.MapCurrencyAcceptanceLevel(cc.Level)},");
            sb.AppendLine("                },");
        }
        ...
        IReadOnlyList<string> tags)
    {
        ...
        foreach (var t in present)
            sb.AppendLine($"                LocaleCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(t)}\"),");
        ...
        HashSet<string> validGpeMembers)
    {
        ...
        var primaryLangMember = ToLanguageMember(entry.PrimaryLanguageIso6391Code);
        if (primaryLangMember is not null && validLanguageMembers.Contains(primaryLangMember))
            sb.AppendLine($"            rec.PrimaryLanguage = LanguageLookup.ByCode[LanguageCode.{primaryLangMember}];");

        // PrimaryCurrency nav.
        if (entry.PrimaryCurrencyIso4217AlphaCode.Truthy()
            && DefaultEmitterHelpers.IsValidIdentifier(entry.PrimaryCurrencyIso4217AlphaCode!))
            sb.AppendLine($"            rec.PrimaryCurrency = CurrencyLookup.ByCode[CurrencyCode.{entry.PrimaryCurrencyIso4217AlphaCode}];");
        ...
        {
            sb.AppendLine(
                $"            rec.PrimaryLocale = LocaleLookup.ByCode[LocaleCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(entry.PrimaryLocaleIetfBcp47Tag!)}\")];");
        }

        // SovereignCountry nav.
        if (entry.SovereignCountryIso31661Alpha2Code.Truthy()
            && DefaultEmitterHelpers.IsValidIdentifier(entry.SovereignCountryIso31661Alpha2Code!))
            sb.AppendLine($"            rec.SovereignCountry = ByCode[CountryCode.{entry.SovereignCountryIso31661Alpha2Code}];");
        ...
        if (localeTags.Count > 0)
        {
            ...
            {
                sb.AppendLine(
                    $"                LocaleLookup.ByCode[LocaleCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(tag)}\")],");
            }
        ...
        if (gpes.Count > 0)
        {
            ...
            foreach (var gpe in gpes)
                sb.AppendLine($"                GeopoliticalEntityLookup.ByCode[GeopoliticalEntityCode.{gpe}],");
        ...
        sb.AppendLine($"            if (SubdivisionLookup.ByCountry.TryGetValue(CountryCode.{c}, out var subs))");
        sb.AppendLine("            {");
        ...
        sb.AppendLine("            {");
        sb.AppendLine("                if (CurrencyLookup.ByCode.TryGetValue(cc.Iso4217AlphaCode, out var cur))");
