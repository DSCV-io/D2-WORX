...
internal static class TimezoneDataEmitter
{
        ...
        HashSet<string> validCountryMembers)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine(
            "    /// <summary>Timezone records indexed by typed <see cref=\"TimezoneCode\"/>.</summary>");
        sb.AppendLine(
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        // First pass: construct every Timezone record (nav refs null/empty until wire-nav).");
        sb.AppendLine("        var byCode = new Dictionary<TimezoneCode, Timezone>();");
        foreach (var entry in entries)
        {
            sb.AppendLine(
                $"        byCode[TimezoneCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(entry.IanaIdentifier)}\")] = "
        ...
        sb.AppendLine("            {");
        sb.AppendLine("                var list = new List<Country>(tz.CoApplicableCountryIso31661Alpha2Codes.Count);");
        sb.AppendLine("                foreach (var cc in tz.CoApplicableCountryIso31661Alpha2Codes)");
        sb.AppendLine("                {");
        ...
        HashSet<string> validCountryMembers)
    {
        ...
        sb.Append($"IanaName = TimezoneCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(entry.IanaIdentifier)}\"), ");
        sb.Append($"DisplayName = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.DisplayName)}\", ");
        sb.Append("LocalizedDisplayNames = new System.Collections.Generic.Dictionary<string, string>(), ");
        sb.Append($"CurrentStdOffsetMinutes = {entry.CurrentStdOffsetMinutes}, ");
        sb.Append($"CurrentDstOffsetMinutes = {DefaultEmitterHelpers.IntLiteralOrNull(entry.CurrentDstOffsetMinutes)}, ");
        sb.Append($"CurrentStdAbbrev = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.CurrentStdAbbrev)}\", ");
        sb.Append($"CurrentDstAbbrev = {DefaultEmitterHelpers.StringLiteralOrNull(entry.CurrentDstAbbrev)}, ");

        if (entry.CountryIso31661Alpha2Code.Truthy()
            && validCountryMembers.Contains(entry.CountryIso31661Alpha2Code!))
        {
            sb.Append($"PrimaryCountryIso31661Alpha2Code = CountryCode.{entry.CountryIso31661Alpha2Code}, ");
        }
        else
        {
    ...
    private static void EmitTrieNode(StringBuilder sb, TrieNode node, int indent)
    {
        ...
        if (node.Children.Count == 0 && node.LeafIana is not null)
        {
            ...
            sb.AppendLine(
                ...
                + $"TimezoneCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(node.LeafIana)}\");");
        }
        else
        {
            sb.AppendLine($"{pad}/// <summary>Timezone subtree under <c>{node.Segment}</c>.</summary>");
