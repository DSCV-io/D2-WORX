...
internal static class SubdivisionDataEmitter
{
        ...
        HashSet<string> validSubdivisionCodes)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine(
            "    /// <summary>Subdivision records indexed by typed <see cref=\"SubdivisionCode\"/>.</summary>");
        sb.AppendLine(
            "    public static readonly FrozenDictionary<SubdivisionCode, Subdivision> ByCode;");
        sb.AppendLine();
        sb.AppendLine(
            "    /// <summary>Subdivision records grouped by parent <see cref=\"CountryCode\"/>.</summary>");
        sb.AppendLine(
            "    public static readonly FrozenDictionary<CountryCode, IReadOnlyList<Subdivision>> ByCountry;");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>All Subdivision records in spec order (alpha-sorted by code).</summary>");
        sb.AppendLine(
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        // First pass: construct every Subdivision record (nav refs null until wire-nav).");
        sb.AppendLine("        var byCode = new Dictionary<SubdivisionCode, Subdivision>();");
        foreach (var entry in entries)
        {
            sb.AppendLine(
                $"        byCode[SubdivisionCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(entry.Iso31662Code)}\")] = "
        ...
        sb.AppendLine("        var byCountryMutable = new Dictionary<CountryCode, List<Subdivision>>();");
        ...
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        var byCountry = new Dictionary<CountryCode, IReadOnlyList<Subdivision>>();");
        ...
        sb.AppendLine("        foreach (var sub in All)");
        sb.AppendLine("        {");
        sb.AppendLine("            if (CountryLookup.ByCode.TryGetValue(sub.CountryIso31661Alpha2Code, out var country))");
        ...
        HashSet<string> validSubdivisionCodes)
    {
        ...
        sb.Append($"Iso31662Code = SubdivisionCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(entry.Iso31662Code)}\"), ");
        sb.Append($"ShortCode = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.ShortCode)}\", ");
        sb.Append($"DisplayName = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.DisplayName)}\", ");
        sb.Append($"OfficialName = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.OfficialName)}\", ");
        sb.Append($"EndonymDisplayName = \"{DefaultEmitterHelpers.EscapeStringLiteral(endonymDisplay)}\", ");
        sb.Append($"EndonymOfficialName = \"{DefaultEmitterHelpers.EscapeStringLiteral(endonymOfficial)}\", ");
        ...
        {
            sb.Append($"ParentSubdivisionIso31662Code = SubdivisionCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(entry.ParentIso31662Code!)}\"), ");
        }
        else
        {
        ...
        sb.Append($"Type = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.Type ?? string.Empty)}\"");
        sb.Append(" }");
        return sb.ToString();
    }
    ...
    private static EmitResult EmitNested(IReadOnlyList<SubdivisionSpec> entries)
    {
        ...
        var grouped = new SortedDictionary<string, List<SubdivisionSpec>>(System.StringComparer.Ordinal);
        foreach (var entry in entries)
        {
        ...
        foreach (var kvp in grouped)
        {
            ...
            foreach (var entry in kvp.Value)
            {
                ...
                sb.AppendLine(
                    $"        /// <summary>{DefaultEmitterHelpers.EscapeXmlDoc(entry.DisplayName)} ({entry.Iso31662Code}).</summary>");
                sb.AppendLine(
                    $"        public static readonly SubdivisionCode {memberName} = "
                    + $"SubdivisionCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(entry.Iso31662Code)}\");");
            }
