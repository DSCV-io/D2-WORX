...
internal static class GeoDataInitializerEmitter
{
    ...
    public static ImmutableArray<EmitResult> EmitAll(GeoSpecContext context)
    {
        ...
        sb.AppendLine("        // Order here doesn't matter — each ctor only touches its own catalog");
        sb.AppendLine("        // and its own ByCode dict (no cross-catalog references in the first pass).");
        sb.AppendLine("        RuntimeHelpers.RunClassConstructor(typeof(CountryLookup).TypeHandle);");
        sb.AppendLine("        RuntimeHelpers.RunClassConstructor(typeof(SubdivisionLookup).TypeHandle);");
        sb.AppendLine("        RuntimeHelpers.RunClassConstructor(typeof(CurrencyLookup).TypeHandle);");
        sb.AppendLine("        RuntimeHelpers.RunClassConstructor(typeof(LanguageLookup).TypeHandle);");
        sb.AppendLine("        RuntimeHelpers.RunClassConstructor(typeof(LocaleLookup).TypeHandle);");
        sb.AppendLine("        RuntimeHelpers.RunClassConstructor(typeof(TimezoneLookup).TypeHandle);");
        sb.AppendLine("        RuntimeHelpers.RunClassConstructor(typeof(GeopoliticalEntityLookup).TypeHandle);");
        sb.AppendLine();
        sb.AppendLine("        // ---- Wire-nav step: wire nav refs in dependency order. ----");
        sb.AppendLine("        // Subdivision.Country needs Country first-pass; Country.Subdivisions");
        sb.AppendLine("        // needs Subdivision first-pass (the ByCountry index) — so Subdivision");
        ...
        sb.AppendLine("        // first-pass records present in their ByCode dicts — they are because");
        sb.AppendLine("        // first-pass ran for all above). Locale/Currency/Language/Timezone/Gpe");
