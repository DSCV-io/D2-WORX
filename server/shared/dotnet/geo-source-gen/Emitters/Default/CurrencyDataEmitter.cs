...
internal static class CurrencyDataEmitter
{
    ...
    public static ImmutableArray<EmitResult> EmitAll(GeoSpecContext context)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            sb.AppendLine(
                $"    /// <summary>{DefaultEmitterHelpers.EscapeXmlDoc(entry.DisplayName)} ({entry.Iso4217AlphaCode}).</summary>");
            sb.AppendLine(
                $"    public static Currency {entry.Iso4217AlphaCode} => CurrencyLookup.ByCode[CurrencyCode.{entry.Iso4217AlphaCode}];");
            sb.AppendLine();
        }
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        // First pass: construct every Currency record (reverse navs empty until wire-nav).");
        ...
        foreach (var entry in entries)
        {
            ...
            sb.AppendLine(
                $"        byCode[CurrencyCode.{entry.Iso4217AlphaCode}] = " + CurrencyRecordLiteral(entry) + ";");
        }
        ...
        sb.AppendLine();
        sb.AppendLine("        ByIso4217Alpha = byAlpha.ToFrozenDictionary(System.StringComparer.OrdinalIgnoreCase);");
        sb.AppendLine();
        ...
        sb.AppendLine("            {");
        sb.AppendLine("                if (!grouped.TryGetValue(cc.Iso4217AlphaCode, out var list))");
        sb.AppendLine("                {");
        ...
        sb.AppendLine();
        sb.AppendLine("            currency.AcceptedInCountryIso31661Alpha2Codes = codeSet.ToFrozenSet();");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine("}");
    ...
    private static string CurrencyRecordLiteral(CurrencySpec entry)
    {
        ...
        sb.Append($"Iso4217NumericCode = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.Iso4217NumericCode ?? string.Empty)}\", ");
        sb.Append($"DisplayName = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.DisplayName)}\", ");
        sb.Append($"OfficialName = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.DisplayName)}\", ");
        sb.Append($"DecimalPlaces = {entry.DecimalPlaces}, ");
        sb.Append($"Symbol = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.Symbol ?? string.Empty)}\", ");
