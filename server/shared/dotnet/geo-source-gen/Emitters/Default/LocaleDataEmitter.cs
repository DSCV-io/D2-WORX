...
internal static class LocaleDataEmitter
{
        ...
        HashSet<string> validCountryMembers)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine(
            "    /// <summary>Locale records indexed by typed <see cref=\"LocaleCode\"/>.</summary>");
        sb.AppendLine(
            "    public static readonly FrozenDictionary<LocaleCode, Locale> ByCode;");
        sb.AppendLine();
        sb.AppendLine("    /// <summary>All Locale records in spec order (alpha-sorted by tag).</summary>");
        sb.AppendLine(
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        // First pass: construct every Locale record (nav refs null until wire-nav).");
        sb.AppendLine("        var byCode = new Dictionary<LocaleCode, Locale>();");
        foreach (var entry in entries)
        {
            sb.AppendLine(
                $"        byCode[LocaleCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(entry.IetfBcp47Tag)}\")] = "
        ...
        HashSet<string> validCountryMembers)
    {
        ...
        sb.Append($"IetfBcp47Tag = LocaleCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(entry.IetfBcp47Tag)}\"), ");
        ...
        sb.Append($"FirstDayOfWeek = {DefaultEmitterHelpers.MapDayOfWeek(entry.FirstDayOfWeek)}, ");
        sb.Append($"DecimalSeparator = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.DecimalSeparator)}\", ");
        sb.Append($"ThousandsSeparator = \"{DefaultEmitterHelpers.EscapeStringLiteral(entry.ThousandsSeparator)}\", ");
        sb.Append($"DateFormatPattern = {DefaultEmitterHelpers.MapDateFormatPattern(entry.DateFormatPattern)}");
        sb.Append(" }");
        return sb.ToString();
    }
    ...
    private static void EmitTrieNode(StringBuilder sb, TrieNode node, int indent)
    {
        ...
        if (node.Children.Count == 0 && node.LeafTag is not null)
        {
            ...
            sb.AppendLine(
                ...
                + $"LocaleCode.FromString(\"{DefaultEmitterHelpers.EscapeStringLiteral(node.LeafTag)}\");");
        }
        else
        {
            sb.AppendLine($"{pad}/// <summary>Locale subtree under <c>{node.Segment}</c>.</summary>");
