...
The vocabulary slice (enums, `ActorEntry`, `JwtClaimTypes`, `RequestHeaders`, `IJwksProvider`, `ISessionLivenessTracker`, codegen-emitted `Scopes` / `Audiences`) lives in the sibling [`D2.Shared.Auth.Abstractions`](../auth-abstractions/) project. Domain code references that, never this runtime lib.
...
| Package | Why |
|---|---|
| `D2.Shared.Auth.Abstractions` | `IJwksProvider`, `ISessionLivenessTracker`, `Audiences`, `JwtClaimTypes`, `RequestHeaders`, `Scopes`. |
