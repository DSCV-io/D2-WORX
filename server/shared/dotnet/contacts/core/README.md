...
- [`ValueObjects/Demographics.cs`](ValueObjects/Demographics.cs) — optional `DateOfBirth` (`DateOnly?`, not in the future, not more than 150 years in the past) + `BiologicalSex` (`BiologicalSex?`). The all-null record is rejected. The date-of-birth bounds resolve "today" from an injectable `TimeProvider` (defaults to `TimeProvider.System`) so boundary behavior is deterministic under test.
  - `Demographics.Create(dateOfBirth?, biologicalSex?, timeProvider?)` → `D2Result<Demographics>`.
