...
/// <see cref="ForwardedJwt.RevealForForwarding"/> — the "forward" leg the
/// outbound credential will perform later. The reveal is test-only in this step
/// (no production reveal caller exists yet); the point is to prove that even
...
public sealed class ForwardedJwtLogCaptureTests
{
    ...
    public void ForwardedJwt_LoggedEveryWay_BytesNeverSurface_PlaceholderDoes()
    {
        ...
        // Capture into the real holder, then play the future consumer: read it
