...
public sealed class SealedConsumerStartupCheckTests
{
    ...
    {
        SeedSealedSubscriberDescriptor<SealedFixtureMessage>(EncryptionDomains.AUDIT);
        var registry = Registry<SealedFixtureMessage>("audit-consumer-queue");
        ...
        ex.WithMessage("*sealed encryption domain*audit*");
    ...
    public async Task StartAsync_SealedSubscriberWithOpener_Passes()
    {
        ...
        services.AddKeyedSingleton<IPayloadOpener>("audit", new StubOpener());
