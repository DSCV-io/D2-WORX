...
| `Generated/.../D2ResultEnvelopeFieldNames.g.cs` | JSON property-name catalog for the D2Result Shape B wire envelope (the BFF gateway response shape every frontend reads) — 8 constants: `SUCCESS`, `DATA`, `MESSAGES`, `INPUT_ERRORS`, `ERROR_CODE`, `TRACE_ID`, `STATUS_CODE`, `CATEGORY`. Codegen-emitted by [`D2.Shared.Result.Envelope.SourceGen`](../envelope-source-gen/README.md) from [`contracts/d2result-envelope/d2result-envelope.spec.json`](../../../../../contracts/d2result-envelope/d2result-envelope.spec.json) (single-target dispatch on `D2.Shared.Result`). Referenced by `[JsonPropertyName(D2ResultEnvelopeFieldNames.*)]` attributes on every wire-emitted `D2Result` property. Same spec drives the TS-side `@d2/result` `D2ResultEnvelopeFieldNames` catalog via `tools/ts-codegen/src/d2result-envelope-emit.ts`; the BFF gateway parser reads via these constants instead of hand-rolled string literals. Cross-language wire drift on the 8 envelope field names is structurally impossible. The class also exposes `IReadOnlyList<string> AllFields` (spec-order enumeration). |
