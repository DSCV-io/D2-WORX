...
| [`grpc-trailers-source-gen/`](grpc-trailers-source-gen/README.md) | Roslyn generator emitting the gRPC trailer-key catalog into `auth/grpc/` from `contracts/grpc-trailers/grpc-trailers.spec.json` — the gRPC wire envelope for `D2Result` (error code, messages, trace id). |
