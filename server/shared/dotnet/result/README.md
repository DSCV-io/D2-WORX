...
| `Generated/.../ErrorCodes.g.cs` | Standardized error code constants — `NOT_FOUND`, `FORBIDDEN`, `VALIDATION_FAILED`, `PARTIAL_SUCCESS`, `IDEMPOTENCY_IN_FLIGHT`, `COULD_NOT_BE_SERIALIZED`, `COULD_NOT_BE_DESERIALIZED`, etc. (15 total). Codegen-emitted by [`D2.Shared.Result.ErrorCodes.SourceGen`](../error-codes-source-gen/README.md) from [`contracts/error-codes/error-codes.spec.json`](../../../../contracts/error-codes/error-codes.spec.json). Same spec drives the TS-side `@d2/result` `ErrorCodes` catalog via `tools/ts-codegen/src/error-codes-emit.ts` — cross-language drift is structurally impossible. The class also exposes `IReadOnlyList<string> AllCodes` (spec-order enumeration) and `int GetHttpStatus(string)` (declared HTTP status per code; `500` for unknown). |
| `Generated/.../InputErrorWireShape.g.cs` | JSON property-name catalog for the `InputError` wire shape — `FIELD = "field"` + `ERRORS = "errors"`. Codegen-emitted by [`D2.Shared.WireShapes.SourceGen`](../wire-shapes-source-gen/README.md) from [`contracts/input-error/input-error.spec.json`](../../../../contracts/input-error/input-error.spec.json) (multi-target dispatch by assembly name). Referenced by `InputError`'s `[JsonPropertyName]` attributes; same spec drives the TS-side `@d2/result` `InputErrorWireShape` catalog via `tools/ts-codegen/src/wire-shape-emit.ts` — cross-language wire drift on the property names is structurally impossible. |
| `Generated/.../D2ResultEnvelopeFieldNames.g.cs` | JSON property-name catalog for the D2Result Shape B wire envelope (the BFF gateway response shape every frontend reads) — 7 constants: `SUCCESS`, `DATA`, `MESSAGES`, `INPUT_ERRORS`, `ERROR_CODE`, `TRACE_ID`, `STATUS_CODE`. Codegen-emitted by [`D2.Shared.Result.Envelope.SourceGen`](../d2result-envelope-source-gen/README.md) from [`contracts/d2result-envelope/d2result-envelope.spec.json`](../../../../contracts/d2result-envelope/d2result-envelope.spec.json) (single-target dispatch on `D2.Shared.Result`). Referenced by `[JsonPropertyName(D2ResultEnvelopeFieldNames.*)]` attributes on every wire-emitted `D2Result` property. Same spec drives the TS-side `@d2/result` `D2ResultEnvelopeFieldNames` catalog via `tools/ts-codegen/src/d2result-envelope-emit.ts`; the BFF gateway parser reads via these constants instead of hand-rolled string literals. Cross-language wire drift on the 7 envelope field names is structurally impossible. The class also exposes `IReadOnlyList<string> AllFields` (spec-order enumeration). |
