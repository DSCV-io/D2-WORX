...
| [`source-gen-shared/`](source-gen-shared/) <!-- §11.36 carve-out: shared .cs files only; no README. --> | **Built** | Shared source-gen scaffolding (NOT a project — no csproj). Holds the netstandard2.0 polyfills (`IsExternalInit` + Falsey/Truthy `StringExt`) plus the cross-source-gen records (`EmitDiagnostic`, `LoadResult<TSpec>`, `SpecFile`). Each of the 10 `*-source-gen/` csprojs wires the files in via `<Compile Include="..\source-gen-shared\**\*.cs">` so the polyfills + scaffolding can never drift between source-gens (the missing-polyfill bug class is structurally impossible). | [PATTERNS.md](../../../docs/PATTERNS.md) Spec-driven codegen section |
...
| [`location/`](location/README.md) | Placeholder | Location value objects — `AdminLocation` (country / state / city / postal), `Coordinates`, `StreetAddress`. Content-addressable hash IDs (built-in dedup + cacheability). | [PATTERNS.md](../../../docs/PATTERNS.md) (content-addressable entities) |
...
| **Auth / scope** | [`jwt-claims-source-gen/`](jwt-claims-source-gen/README.md) | Emits `JwtClaimTypes.g.cs` into `auth-abstractions/` from `contracts/jwt-claims/jwt-claims.spec.json`. Same spec drives the TS-side `@d2/auth-abstractions` catalog. | [PATTERNS.md](../../../docs/PATTERNS.md) |
...
| **Context / headers** | [`in-process-keys-source-gen/`](in-process-keys-source-gen/README.md) | Emits `D2HttpContextItems.g.cs` (public class) into `auth-abstractions/` AND `D2GrpcUserStateKeys.g.cs` (internal class) into `auth-grpc/` from `contracts/in-process-keys/keys.spec.json`. Cross-binding parity structurally guaranteed. | [PATTERNS.md](../../../docs/PATTERNS.md) |
| **D2Result / error codes** | [`error-codes-source-gen/`](error-codes-source-gen/README.md) | Emits `ErrorCodes.g.cs` (constants + `AllCodes` + `GetHttpStatus`) into `result/` from `contracts/error-codes/error-codes.spec.json`. Same spec drives the TS-side `@d2/result` catalog. | [PATTERNS.md](../../../docs/PATTERNS.md) |
| **D2Result / error codes** | [`d2result-envelope-source-gen/`](d2result-envelope-source-gen/README.md) | Emits `D2ResultEnvelopeFieldNames.g.cs` (7 JSON property-name constants + `AllFields`) into `result/` from `contracts/d2result-envelope/d2result-envelope.spec.json`. Same spec drives the TS-side gateway parser. | [PATTERNS.md](../../../docs/PATTERNS.md) |
...
| **Messaging** | [`dlq-failure-metadata-source-gen/`](dlq-failure-metadata-source-gen/README.md) | Emits `DlqFailureMetadataFields.g.cs` (JSON property-name catalog) into `messaging-abstractions/` AND `DlqFailureCauses.g.cs` (closed-enum cause-string catalog) into `messaging-rabbitmq/` from `contracts/dlq-failure-metadata/dlq-failure-metadata.spec.json`. Same spec drives the TS-side `@d2/messaging-abstractions` package. | [messaging-rabbitmq](messaging-rabbitmq/README.md) |
...
| **Observability** | [`otel-messaging-tags-source-gen/`](otel-messaging-tags-source-gen/README.md) | Emits `MessagingActivityTags.g.cs` into `messaging-rabbitmq/` from `contracts/otel-messaging-tags/otel-messaging-tags.spec.json`. Spec-drives the OTel sem-conv attribute-name catalog used by both publisher + consumer spans. Same spec drives the TS-side `@d2/telemetry` catalog. | [messaging-rabbitmq](messaging-rabbitmq/README.md) |
| **Observability** | [`grpc-trailers-source-gen/`](grpc-trailers-source-gen/README.md) | Emits `D2GrpcTrailers.g.cs` (constants + `AllTrailers` set) into `auth-grpc/` from `contracts/grpc-trailers/grpc-trailers.spec.json`. Single source of truth for the gRPC trailer-key catalog (`ERROR_CODE` / `MESSAGES` / `TRACE_ID`). | [messaging-rabbitmq](messaging-rabbitmq/README.md) |
...
[`source-gen-shared/`](source-gen-shared/) — not a csproj; the `.cs` files are
wired into each `*-source-gen/` csproj via `<Compile Include="..\source-gen-shared\**\*.cs">`
...
graph LR
    ...
    subgraph JWTCLAIMS["JWT-claims + in-process slot keys (codegen-emitted)"]
        direction TB
        JwtClaimsSG[jwt-claims-source-gen]:::analyzer
        InProcessKeysSG[in-process-keys-source-gen]:::analyzer
