...
internal sealed class SubscriberChannel : IAsyncDisposable
{
    ...
    /// read of <c>_inflightCallbacks</c> — internal observable for the
    /// H6 in-flight-drain test.</summary>
    ...
    /// <summary>Internal access for the F1 race-coverage test. Production
    /// code MUST go through the failure path which calls this lazily.</summary>
    [MustDisposeResource(false)]
