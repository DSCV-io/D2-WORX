...
public sealed class PayloadCryptoKeyring : IDisposable
{
    ...
    /// <summary>Maximum kid length (capped by the one-byte length prefix in the frame format).</summary>
    ...
    /// </summary>
    /// <param name="activeKid">The kid used for new encryptions. Must be present in <paramref name="keys"/>.</param>
    /// <param name="keys">All kids the keyring can decrypt. Each value must be exactly <see cref="KEY_SIZE_BYTES"/>.</param>
    ...
    /// </param>
    /// <exception cref="ArgumentException">A constructor argument violates a stated invariant.</exception>
    public PayloadCryptoKeyring(string activeKid, IReadOnlyDictionary<string, byte[]> keys, ReadOnlyMemory<byte> aadContext)
    {
        ArgumentNullException.ThrowIfNull(activeKid);
        ArgumentNullException.ThrowIfNull(keys);

        if (activeKid.Length is < MIN_KID_LENGTH or > MAX_KID_LENGTH)
            throw new ArgumentException($"activeKid length must be in [{MIN_KID_LENGTH}, {MAX_KID_LENGTH}].", nameof(activeKid));

        if (aadContext.IsEmpty)
            throw new ArgumentException("aadContext must be non-empty so AEAD binding is meaningful.", nameof(aadContext));

        if (!keys.ContainsKey(activeKid))
            throw new ArgumentException($"activeKid '{activeKid}' is not present in keys.", nameof(activeKid));
        ...
        foreach (var (kid, key) in keys)
        {
            ...
            if (kid.Length is < MIN_KID_LENGTH or > MAX_KID_LENGTH)
                throw new ArgumentException($"kid '{kid}' length must be in [{MIN_KID_LENGTH}, {MAX_KID_LENGTH}].", nameof(keys));

            if (key.Length != KEY_SIZE_BYTES)
                throw new ArgumentException($"key for kid '{kid}' must be exactly {KEY_SIZE_BYTES} bytes (got {key.Length}).", nameof(keys));
    ...
    /// </summary>
    /// <param name="kid">The kid to look up. Null returns false (defensive — callers may pass externally-sourced strings).</param>
