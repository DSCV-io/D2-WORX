...
/// <item>that the active kid is present in the keyring (would throw <see cref="KidNotInKeyringException"/> otherwise),</item>
/// <item>that the AAD context is consistent encrypt-side and decrypt-side (would throw <see cref="System.Security.Cryptography.AuthenticationTagMismatchException"/> otherwise).</item>
/// </list>
/// </remarks>
        ...
        foreach (var key in r_registry.ServiceKeys)
        {
            ...
            if (!roundTripped.AsSpan().SequenceEqual(sr_sentinel))
            {
                throw new InvalidOperationException(
                    $"Encryption self-test failed for service key '{key}': round-trip plaintext did not match the sentinel.");
            }
