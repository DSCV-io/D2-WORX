...
public sealed class EncryptionRegistry
{
    ...
    /// <param name="registrations">All registrations made via <see cref="EncryptionServiceCollectionExtensions.AddD2EncryptionFor"/>.</param>
    public EncryptionRegistry(IEnumerable<EncryptionRegistration> registrations)
    {
        ArgumentNullException.ThrowIfNull(registrations);
        ServiceKeys = registrations.Select(r => r.ServiceKey).Distinct(StringComparer.Ordinal).ToArray();
    }
