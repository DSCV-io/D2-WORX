...
{
    /// <summary>GCM nonce length in bytes. Used by <see cref="PayloadCrypto"/> when allocating the nonce span.</summary>
    internal const int NONCE_SIZE = 12;

    /// <summary>GCM authentication-tag length in bytes. Used by <see cref="PayloadCrypto"/> when sizing the ciphertext+tag buffer.</summary>
    ...
    /// </summary>
    private const int _MIN_FRAME_SIZE = _VERSION_SIZE + _KID_LENGTH_PREFIX_SIZE + NONCE_SIZE + TAG_SIZE;

    /// <summary>
    /// Encodes a frame around an already-encrypted ciphertext+tag span.
    /// </summary>
    /// <param name="kid">The kid to embed in the frame header.</param>
    /// <param name="nonce">The nonce used for encryption (must be exactly <see cref="NONCE_SIZE"/> bytes).</param>
    /// <param name="ciphertextWithTag">The encrypted bytes followed by the auth tag.</param>
    /// <returns>The complete frame.</returns>
    internal static byte[] Encode(string kid, ReadOnlySpan<byte> nonce, ReadOnlySpan<byte> ciphertextWithTag)
    {
        var kidByteCount = Encoding.UTF8.GetByteCount(kid);
        if (kidByteCount is < PayloadCryptoKeyring.MIN_KID_LENGTH or > PayloadCryptoKeyring.MAX_KID_LENGTH)
            throw new ArgumentException($"kid UTF-8 byte length must be in [{PayloadCryptoKeyring.MIN_KID_LENGTH}, {PayloadCryptoKeyring.MAX_KID_LENGTH}].", nameof(kid));

        if (nonce.Length != NONCE_SIZE)
            throw new ArgumentException($"nonce must be exactly {NONCE_SIZE} bytes (got {nonce.Length}).", nameof(nonce));

        if (ciphertextWithTag.Length < TAG_SIZE)
            throw new ArgumentException($"ciphertextWithTag must be at least {TAG_SIZE} bytes (the tag).", nameof(ciphertextWithTag));

        var totalSize = _VERSION_SIZE + _KID_LENGTH_PREFIX_SIZE + kidByteCount + NONCE_SIZE + ciphertextWithTag.Length;
    ...
    internal static FrameView Decode(ReadOnlySpan<byte> framed)
    {
        if (framed.Length < _MIN_FRAME_SIZE)
            throw new FrameMalformedException($"Frame too short: {framed.Length} bytes (min {_MIN_FRAME_SIZE}).");
        ...
        if (kidLength < PayloadCryptoKeyring.MIN_KID_LENGTH)
            throw new FrameMalformedException($"Frame kid_length {kidLength} is below minimum {PayloadCryptoKeyring.MIN_KID_LENGTH}.");

        var headerSize = _VERSION_SIZE + _KID_LENGTH_PREFIX_SIZE + kidLength + NONCE_SIZE;
        if (framed.Length < headerSize + TAG_SIZE)
            throw new FrameMalformedException($"Frame too short for declared kid_length={kidLength}: have {framed.Length}, need ≥ {headerSize + TAG_SIZE}.");
