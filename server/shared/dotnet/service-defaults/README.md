...
- Caller-side auth — `D2.Shared.Auth.Outbound` (the per-request forwarded-transaction-token credential, the workload-certificate mTLS leaf, and RFC 8693 token-exchange for the boundary mint + exceptions). Cross-process workload identity is mTLS ([ADR-0023](../../../../docs/adrs/0023-mtls-workload-identity.md)).
