...
describe("byteParity_SignProto_CommittedFixtureIdentical", () => {
  ...
  it("deliberate-drift detection: mutated fixture does NOT match re-emitted output", () => {
    // §1.20 non-vacuous guard: corrupt the fixture by one byte.
    const drifted = SIGN_PROTO_FIXTURE.replace("KeyCustodianSigner", "KeyCustodianSignerDRIFTED");
    const result = emitProto(
      "sign",
      "KeyCustodianSigner",
      "Sign",
...
// ---------------------------------------------------------------------------
// byteParity_KeyCustodianSignerService
// ---------------------------------------------------------------------------

describe("byteParity_KeyCustodianSignerService_CommittedFixtureIdentical", () => {
  it("re-emitted service .g.cs is byte-identical to the committed fixture", () => {
  ...
  it("deliberate-drift detection: mutated fixture does NOT match re-emitted output", () => {
    const drifted = SIGN_SERVICE_FIXTURE.replace("KeyCustodianSignerService", "KeyCustodianSignerServiceDRIFTED");
    const [svc] = emitGrpcService(
      "sign",
      "KeyCustodianSigner",
      "Sign",
