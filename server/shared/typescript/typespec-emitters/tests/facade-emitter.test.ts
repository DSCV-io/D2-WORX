...
describe("facadeEmitter_ByteGate_Interface", () => {
  ...
  it("deliberate-drift detection: mutated fixture does NOT match regenerated output", () => {
    const drifted = INTERFACE_FIXTURE.replace(
      "IKeyCustodianApi",
      "IKeyCustodianApiDRIFTED",
    );
    const [iface] = emitFacade(
      "KeyCustodian",
      [makeOp("getJwks", "GetJwksInput", "GetJwksOutput")],
...
describe("facadeEmitter_ByteGate_Impl", () => {
  ...
  it("deliberate-drift detection: mutated fixture does NOT match regenerated output", () => {
    const drifted = IMPL_FIXTURE.replace(
      "KeyCustodianApi",
      "KeyCustodianApiDRIFTED",
    );
    const [, impl] = emitFacade(
      "KeyCustodian",
      [makeOp("getJwks", "GetJwksInput", "GetJwksOutput")],
...
describe("facadeEmitter_ByteGate_DiExtension", () => {
  ...
  it("deliberate-drift detection: mutated fixture does NOT match regenerated output", () => {
    const drifted = DI_FIXTURE.replace(
      "AddD2KeyCustodianClients",
      "AddD2KeyCustodianClientsDRIFTED",
    );
    const [, , di] = emitFacade(
      "KeyCustodian",
      [makeOp("getJwks", "GetJwksInput", "GetJwksOutput")],
