...
D2 encryption-domain identifiers + encryption-frame binary layout constants (symmetric version-1 AND sealed version-2). Mirrors .NET `D2.Shared.Encryption.EncryptionDomains`, the `D2.Shared.Encryption.EncryptionFrameLayout` byte-offset layout, and the `D2.Shared.Encryption.SealedFrameLayout` sealed layout — all spec-driven.
...
| `EncryptionFrame`             | `encryption-frame.g.ts`        | `D2.Shared.Encryption.EncryptionFrameLayout`        |
...
| `SealedFrame`                 | `encryption-frame-sealed.g.ts` | `D2.Shared.Encryption.SealedFrameLayout`            |
...
- `EncryptionFrame`: TS-side reader for the on-wire symmetric encryption frame produced by `D2.Shared.Encryption.EncryptedBodyComposer`. The frame layout is binary (`[version:1][kid_len:1][kid:UTF-8][nonce:12][ct+tag]`); this catalog exposes the field-byte-offsets and lengths a parser needs.
