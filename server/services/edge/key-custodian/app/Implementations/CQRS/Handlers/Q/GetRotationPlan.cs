// -----------------------------------------------------------------------
// <copyright file="GetRotationPlan.cs" company="DCSV">
...
/// </remarks>
public sealed class GetRotationPlan(
    HandlerContext<GetRotationPlan> ctx,
    IKeyCustodianDbContext db,
    IRotationPolicyProvider policyProvider,
    IClock clock)
    : BaseHandler<GetRotationPlan, GetRotationPlanInput, RotationPlan>(ctx), IGetRotationPlan
{
    /// <inheritdoc/>
    protected override async ValueTask<D2Result<RotationPlan?>> ExecuteAsync(
        GetRotationPlanInput input, CancellationToken ct)
    {
        ...
        foreach (var domain in KeyDomain.All)
        {
            ...
            if (policyResult.BubbleOnFailure<RotationPolicy, RotationPlan>(out var bubbled, out var policy))
                return bubbled;
        ...
        return D2Result<RotationPlan?>.Ok(
            new RotationPlan(bootstrap, dueToActivate, dueToRotate, dueToGenerateSuccessor, dueToRetire));
    }
