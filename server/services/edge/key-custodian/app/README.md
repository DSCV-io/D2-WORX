...
- **`KeyRecordQueryExtensions`** — composable server-side filters on value columns (`Pending` / `Active` / `Retiring` / `Live` / `ForDomain` / `Signing` / `Payload`).
...
| `IssueWorkloadCertificateHandler`  | `Commands` | `BaseRepoHandler` | `IssueWorkloadCertificateInput` → `IssueWorkloadCertificateOutput` | **Interim fail-closed DENY-ALL**: the first gate is `WorkloadCertificateAuthority.AuthorizeIssuance`, whose committed skeleton denies EVERY origin until the real caller↔subject binding rule lands with the cross-process issuance transport wiring. Below the gate: validate the workload identity, load + decrypt the active `mtls-ca-intermediate` key, issue a short-lived leaf via the domain rule, write a `LeafIssuanceAuditRecord`, and return the leaf + chain to the caller. Private key bytes are zeroed in `finally`. |
...
- **Keyring distribution is the one deliberate raw-key output.** `GetKeyringOutput.Entries[].KeyBytes` carries the root-UNWRAPPED AES-256 key on the wire — the single path by which raw key material leaves KeyCustodian (a full encrypt+decrypt capability handed to an authorized payload-domain consumer). It is `[RedactData(SecretInformation)]`-masked so it never renders in logs, but custody of the raw bytes transfers to the caller (which copies them into a defensive-copying + zeroing `PayloadCryptoKeyring`); KeyCustodian does NOT zero them — the caller owns their lifetime. The companion `AadContext` is authenticated-not-secret (an AEAD AAD is public context by construction) and is deliberately NOT redacted, so operators can debug a cross-service AAD disagreement.
