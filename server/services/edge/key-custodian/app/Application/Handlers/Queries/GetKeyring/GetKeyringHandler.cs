...
/// caller (which assembles a <c>PayloadCryptoKeyring</c>) — KeyCustodian never zeroes them,
    ...
    : BaseHandler<GetKeyringHandler, I, O>(ctx), H
{
    ...
    /// <c>IRequestContext.Scopes</c> (fail-closed) before any authority rule or crypto runs —
        ...
        I input, CancellationToken ct)
    {
        ...
        //    permanent 400 — never the retryable 503. DEFENSE-IN-DEPTH ONLY: unreachable in
        ...
        //    denied at step 2 with a uniform 403). Do NOT reorder before authority — that
        ...
        // 6) No ACTIVE key → 503 (retryable). The 503 fires whenever there is no Active row,
        //    even if Retiring rows exist — a keyring with no active kid cannot be assembled
        ...
        // 7) Assemble entries — active first, then retiring newest-activated-first
        //    (deterministic; multiple Retiring rows are possible when rotations outpace the
        //    grace window). Custody of the unwrapped bytes transfers to the caller — KC does
        ...
        //    (Query — no DB write, no audit row).
        ...
        D2Result authResult, string? immediateCaller, RequestOrigin origin, KeyDomain domain)
    {
        ...
        // uniform 403 KEYRING_DOMAIN_NOT_AUTHORIZED splits by deny arm for TELEMETRY ONLY —
        ...
        var reason = authResult.ErrorCode switch
        {
            ...
            // Forbidden — an authorized plane with no caller identity.
        ...
        // No SR_CrossProcessSigningRejections here — that counter is the signing
