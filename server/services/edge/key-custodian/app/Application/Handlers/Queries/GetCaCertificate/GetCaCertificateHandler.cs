...
/// workload pins) + the active issuing intermediate (chain convenience) — as
...
/// material — no per-workload policy map).
/// </summary>
/// <remarks>
/// Transport-agnostic — reads ONLY the scoped request context
...
/// seeded or is between rotations). No decrypt — the certificate columns are
/// plaintext public material; no DB write — Query.
/// </remarks>
    ...
    : BaseHandler<GetCaCertificateHandler, I, O>(ctx), H
{
    ...
    /// <c>IRequestContext.Scopes</c> (fail-closed) before the authority rule runs —
    ...
    protected override HandlerOptions DefaultOptions => new()
    {
        ScopeRequirement = new ScopeRequirement(
            ...
            new HashSet<string>(StringComparer.Ordinal) { Scopes.Internal.Kc.Cacert }),
    };
    ...
    {
        // 1) Authority gate — read the ESTABLISHED Origin + ImmediateCaller (set by
        //    the boundary that produced this context; §9.41) and call the pure rule.
        //    Authority precedes the store reads so an unauthorized caller learns
        //    nothing about CA state (403 before any 503 — no CA-state oracle).
        ...
        //    malformed → the retryable 503 (a partial chain is not "the chain").
        ...
        // 3) Return the chain (Query — no DB write, no audit row). Public trust
        //    material — presented on the wire in every TLS handshake.
    ...
    /// a non-CA shape, or missing certificate material — all "tier not ready").
    /// </summary>
    ...
    private async Task<byte[]?> LoadActiveCaCertificateAsync(string domain, CancellationToken ct)
    {
        ...
        // material — a malformed shape is treated as the tier being absent.
    ...
    private D2Result<O?> DenyWithTelemetry(D2Result authResult, string? immediateCaller)
    {
        ...
        // for TELEMETRY ONLY — the wire code stays uniform.
        ...
        // The CA-chain fetch is a TARGETLESS capability (no key-domain target) — the
