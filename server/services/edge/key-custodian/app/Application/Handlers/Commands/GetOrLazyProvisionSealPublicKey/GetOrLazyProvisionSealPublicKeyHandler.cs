...
/// <c>IRequestContext.Origin</c> + <c>IRequestContext.ImmediateCaller</c> — broad within the
...
/// <c>PublicKeyMaterial</c> (plaintext at rest — no root-decrypt).
/// </summary>
/// <remarks>
...
/// oracle — the same posture as the issuance surface). No Active key yet is the retryable 503.
/// </remarks>
    ...
    : BaseRepoHandler<GetOrLazyProvisionSealPublicKeyHandler, I, O>(ctx, classifier), H
{
    ...
    /// or crypto runs — layered under the transport-level scope check.</remarks>
    ...
    {
        // 1) Authority gate FIRST — read the ESTABLISHED Origin + ImmediateCaller and call the
        ...
        // 2) Validate the target service id (§9.4) → 400 before any DB/crypto. The gate has
        ...
        // 3) Build the seal:<serviceId> domain from the ALREADY-VALIDATED identity — no second
        //    grammar pass (the service id is validated exactly once, at step 2) — and
        ...
        // 4) Serve the SPKI public keys straight from PublicKeyMaterial (plaintext at rest —
    ...
    private D2Result<O?> DenyWithTelemetry(D2Result authResult, string? immediateCaller)
    {
        ...
        var reason = authResult.ErrorCode switch
        {
            ...
            // Forbidden — a served plane with no caller identity.
            _ => KeyCustodianMetrics.AuthorityRejections.Reason.IDENTITY_ABSENT,
        };

        // Seal-encrypt denies precede serviceId resolution, so the target is unresolved →
