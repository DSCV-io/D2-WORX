...
/// <see cref="WorkloadCapabilityAuthority.AuthorizeSealDecrypt"/> — the seal-decrypt hard gate:
...
/// key — impersonation is structurally unrepresentable. The first request provisions the
/// service's keypair lazily (a <c>Command</c>).
/// </summary>
/// <remarks>
/// Transport-agnostic — reads ONLY the scoped request context (<c>Context.Request</c>), never
/// <c>ServerCallContext</c> / <c>Grpc.Core</c> / <c>HttpContext</c>. The structural hard-gate
/// guarantee = (decrypt arm is cross-process-only) ∧ (the interceptor's atomic
/// Origin⟺ImmediateCaller coupling) ∧ (the op carries no serviceId / target). A forged
/// in-process caller is denied AT THE PLANE ARM — it never reaches key selection. No Active
    ...
    : BaseRepoHandler<GetOrLazyProvisionOwnSealPrivateKeyHandler, I, O>(ctx, classifier), H
{
    ...
    // base) — the shared provisioning helper needs the classifier to recognize the
    ...
    /// service, and root-unwrap is slow crypto — both can exceed the platform default
    ...
    /// authority rule or crypto runs — layered under the transport-level scope check.</remarks>
    protected override HandlerOptions DefaultOptions => new()
    {
        ...
        ScopeRequirement = new ScopeRequirement(
            ...
            new HashSet<string>(StringComparer.Ordinal) { Scopes.Internal.Kc.Seal.Open }),
    };
    ...
    {
        // 1) Authority gate — the seal-decrypt HARD GATE holds structurally: cross-process-only, so
        //    ImmediateCaller is the unforgeable mTLS peer id. Unestablished / non-cross-process
        //    plane / identity-absent → deny (targetless — the op carries no key-domain target).
        ...
        //    authorized — belt-and-braces re-validation pins the invariant against a
    ...
    private D2Result<O?> DenyWithTelemetry(D2Result authResult, string? immediateCaller)
    {
        ...
        // 403 SEAL_NOT_AUTHORIZED splits by deny arm for TELEMETRY ONLY — the wire code stays
        ...
        // Seal-decrypt is a TARGETLESS capability (no key-domain target — the key is selected
        // from the caller identity alone) → the log's target field carries Target.NONE.
