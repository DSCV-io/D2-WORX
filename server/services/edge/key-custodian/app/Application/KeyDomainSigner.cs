...
internal static class KeyDomainSigner
{
    ...
    /// payload; <c>SigningKeyUnavailable</c> (503) when no active signing key exists for
    /// the domain; <c>PreconditionViolated</c> (500) when the stored row is not an active
    /// RSA signing key (corruption) or the crypto fails.
    /// </returns>
    ...
    {
        // Validate input at the top — reject empty input before any key load or crypto.
