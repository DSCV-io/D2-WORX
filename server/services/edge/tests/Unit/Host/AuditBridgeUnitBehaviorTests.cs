...
/// <summary>
/// Cheap unit doubles for Edge HTTP→gRPC Audit bridge map behavior (typed NIE
/// vs transport-fault Audit-down vs mid-handler cancel). Not multiproc proof —
/// dual-Kestrel OUT. §1.32: doubles assert CallCount + input on the real
...
public sealed class AuditBridgeUnitBehaviorTests
{
    ...
    public async Task AuditBridge_Canceled_PropagatesCancellation()
    {
        ...
        // so Map/client cancel path runs — never pre-cancel GetAsync.
        using var jwt = new TestJwtBuilder();
            ...
            .ConfigureWebHost(webHost =>
            {
                webHost
                    ...
                    .Configure(app =>
                    {
                        ...
                        {
                            // Production bridge Map (generated) — not handler alone.
    ...
    /// <summary>
    /// §1.32 double — typed business NIE (ServiceUnavailable envelope).
    ...
    /// <summary>
    /// §1.32 double — transport-fault path via
    ...
    private sealed class TransportFaultAuditGrpcClientStub : IAuditGrpcClient
    {
            ...
            CancellationToken ct = default)
        {
            ...
            // Mirror AuditGrpcClient transport-fault remap (Unavailable → SU).
    ...
    /// <summary>
    /// §1.32 double — blocks after entry so the test can cancel mid-handler.
    /// Replace-trigger: live <c>AddD2AuditGrpcClients</c> on Edge host.
    /// </summary>
    private sealed class BlockingCancelAuditGrpcClientStub : IAuditGrpcClient
    {
            ...
            CancellationToken ct = default)
        {
            ...
            // Hold until RequestAborted / client cancel — proves Map hit client.
