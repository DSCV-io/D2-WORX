...
/// Map binding uses production <see cref="Scopes.Internal.Kc.Keyring"/> constants —
...
public sealed class KcGrpcHostAuthAdversarialTests
{
    ...
    public async Task KcGrpc_WrongAudience_Denied()
    {
        ...
        var token = jwt.MintToken(
            ...
            {
                ["scope"] = Scopes.Internal.Kc.Keyring,
            });
    ...
    {
        // Matrix name KcGrpc_MissingScope_Returns403 is stale Living State alias —
    ...
    public async Task KcGrpc_WrongScope_Returns401ScopeInsufficient()
    {
        ...
        // Map binding still uses Scopes.Internal.Kc.Keyring (production).
        var token = jwt.MintToken(
            _ISSUER,
            _AUDIENCE,
            extraClaims: new Dictionary<string, object>
            {
                ["scope"] = Scopes.Internal.Kc.Sign,
            });
        var headers = new Metadata { { "authorization", "Bearer " + token } };
            ...
            .ConfigureWebHost(webHost =>
            {
                webHost
                    ...
                    .Configure(app =>
                    {
                        ...
                        {
                            // Production Map pattern — Scopes.Internal.Kc.* constant.
