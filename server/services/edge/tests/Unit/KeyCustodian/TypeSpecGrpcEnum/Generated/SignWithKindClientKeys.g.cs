// -----------------------------------------------------------------------
// <auto-generated>
//   Generated by the @d2/typespec-emitters TypeSpec emitter.
//   Source spec: contracts/typespec/fixtures/enum-shaped.tsp
//   Manual edits will be lost on rebuild.
// </auto-generated>
// -----------------------------------------------------------------------
#nullable enable

namespace D2.Edge.Tests.TypeSpecGrpcEnum.Clients;

/// <summary>
/// Service-key constants for the <c>SignWithKind</c> gRPC client resilience pipeline.
/// </summary>
public static class SignWithKindClientKeys
{
    /// <summary>DI service key for the keyed <c>ResilientPipeline&lt;TKey, TValue&gt;</c> registration.</summary>
    public const string PIPELINE = "SignWithKindGrpcClientPipeline";

    /// <summary>
    /// Per-call pipeline key passed to <c>ExecuteAsync</c>. Inert for retry-only
    /// pipelines; a singleflight-enabled pipeline must supply a per-request key
    /// instead to avoid wrongly deduplicating distinct calls.
    /// </summary>
    internal const string PIPELINE_KEY = "SignWithKindGrpcClientCall";
}
