...
/// EXACTLY the CSR's key with a SAN that is ALWAYS the authenticated peer — a
...
public sealed class IssueWorkloadCertificateTests
{
    ...
    // -----------------------------------------------------------------------
    // Authority deny matrix — every arm through the REAL handler, with telemetry
    // -----------------------------------------------------------------------
    ...
    {
        // No internal.kc.issue scope on the request context → BaseHandler's
    ...
    // -----------------------------------------------------------------------
    // Ordering pins — no oracle for unauthenticated probers
    // -----------------------------------------------------------------------
    ...
    public async Task Issue_GarbageCsrFromUnauthorizedContext_403_Not400_NoCsrParseOracle()
    {
        ...
        result.ErrorCode.Should().NotBe(
            ...
            "authority precedes CSR validation — an unauthorized prober learns "
    ...
    public async Task Issue_NoCaAndWrongPlane_403_Not503_NoCaStateOracle()
    {
        ...
        result.StatusCode.Should().Be(
            HttpStatusCode.Forbidden,
            "authority precedes the CA load — an unauthorized caller learns nothing "
            + "about CA state");
    }
    ...
    public async Task Issue_InvalidCsrWithUnseededCa_400_Not503_CsrValidationPrecedesCaLoad()
    {
        ...
        result.StatusCode.Should().Be(
            ...
            "CSR validation runs before any store access — the 400 fires even with "
            + "no CA seeded");
        result.ErrorCode.Should().Be(KeyCustodianErrorCodes.KEYCUSTODIAN_INVALID_CSR);
    }

    // -----------------------------------------------------------------------
    // The uniform 400 INVALID_CSR matrix — through the real handler
    // -----------------------------------------------------------------------
    ...
    public async Task Issue_InvalidCsr_Uniform400_NoLeaf_NoAudit(string kind)
    {
        ...
        result.ErrorCode.Should().Be(
            KeyCustodianErrorCodes.KEYCUSTODIAN_INVALID_CSR,
            "every CSR failure class folds into the one coarse 400 — no "
            + "which-check-failed leak");
        AssertNoLeafNoAudit(db, result);
    }

    // -----------------------------------------------------------------------
    // The 503 no-active-CA arm — authorized caller, valid CSR, no intermediate
    // -----------------------------------------------------------------------
    ...
    public async Task Issue_AuthorizedValidCsr_MintsLeaf_KeyPairsSanFromPeer_AuditWritten()
    {
        ...
        using (listener)
        {
            ...
            // The chain builds: root anchor → issuing intermediate → leaf.
    ...
    public async Task Issue_ForgedCsrSanAndSubject_LeafSanIsStillThePeer_NoForgery()
    {
        ...
        // DIFFERENT identity (subject CN=files + SAN spiffe://…/files) still yields
        // a leaf whose SAN is the AUTHENTICATED peer ("edge") — nothing from the
        ...
        result.Success.Should().BeTrue(
            "a PoP-valid P-256 CSR is acceptable regardless of its subject — the "
    ...
    {
        // The wire-facing output wraps the all-public VO — no private-key member is
