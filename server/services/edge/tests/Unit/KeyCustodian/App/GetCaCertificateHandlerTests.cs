...
public sealed class GetCaCertificateHandlerTests
{
    ...
    {
        // A partial chain is not "the chain" — root present, intermediate missing.
        await using var db = KeyCustodianTestDbContext.CreateEmpty();
    ...
    {
        // The inverse partial chain — intermediate present, root missing.
        await using var db = KeyCustodianTestDbContext.CreateEmpty();
    ...
    {
        // Rows exist but neither tier is ACTIVE — still the retryable 503.
        await using var db = KeyCustodianTestDbContext.CreateEmpty();
    ...
    {
        // A malformed ACTIVE row in the root domain — an active row whose KeyType is
    ...
    public async Task GetCaCertificate_ActiveIntermediateNullCertificateMaterial_503()
    {
        ...
        // NULL certificate material (corruption) — the malformed-active-row guard
    ...
    // -----------------------------------------------------------------------
    // Deny matrix — with the ca-cert capability telemetry
    // -----------------------------------------------------------------------
    ...
    {
        // No internal.kc.cacert scope → the BaseHandler scope gate fires first; the
    ...
    {
        // Authority precedes the store reads — an unauthorized caller learns
