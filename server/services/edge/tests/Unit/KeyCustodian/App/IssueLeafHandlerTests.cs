...
/// wire DTO (leaf DER, issuer DER, Instant → DateTimeOffset), and an inner denial
/// bubbles UNCHANGED (code + status preserved — the shell adds no gate and no
...
public sealed class IssueLeafHandlerTests
{
    ...
    public async Task IssueLeaf_HappyPath_MapsInnerOutputFieldByField()
    {
        ...
        // Field-by-field mapping: validity window Instant → DateTimeOffset. notBefore
    ...
    public async Task IssueLeaf_InnerAuthorityDeny_BubblesUnchanged()
    {
        ...
        // The INNER handler's context is EdgeInbound → the plane deny; the shell
