...
/// provisioning — production-shaped PG exceptions + the real
...
/// when-clause misses, or break 23P01 → UniqueViolation mapping — these cases
...
public sealed class SealKeyProvisioningConvergeTests
{
    ...
    // Production classifier — the same type AddD2Postgres registers. Do NOT
    ...
    [Theory]
    [InlineData("23P01")] // exclusion_violation — one-Active EXCLUDE (the race)
    [InlineData("23505")] // unique_violation — one-Pending unique (related race)
        ...
        string sqlState)
    {
        ...
        // The flake was UNIQUE_VIOLATION leaking via BaseRepoHandler — pin that gone.
    ...
    /// converge poll exhausts → 503 (or maps non-unique kinds via BaseRepoHandler).
    /// </summary>
