// -----------------------------------------------------------------------
// <copyright file="JwksAssemblerTests.cs" company="DCSV">
...
/// <summary>
/// Tests for the pure <see cref="JwksAssembler"/>: n/e base64url correctness
/// against <see cref="RSA.ExportParameters"/> and the fixed RS256/sig/RSA header.
/// </summary>
public sealed class JwksAssemblerTests
{
    [Fact]
    public void ToJwk_NAndE_MatchExportedRsaParametersBase64Url()
    {
        ...
        var jwk = JwksAssembler.ToJwk("kid-jwk", spki);
    ...
    {
        using var rsa = RSA.Create(2048);
        var jwk = JwksAssembler.ToJwk("kid", rsa.ExportSubjectPublicKeyInfo());
    ...
    public void ToJwk_Jwk_ExposesNoRsaPrivateParameters()
    {
        ...
        using var rsa = RSA.Create(2048);
        var jwk = JwksAssembler.ToJwk("kid-priv-check", rsa.ExportSubjectPublicKeyInfo());
        ...
        var jwkProperties = typeof(D2.Edge.KeyCustodian.App.Models.Jwk)
