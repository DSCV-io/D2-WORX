...
/// within a class) — the fixture seam is a static registry, so the two-bypass test that
...
public sealed class SealedDomainRemovalTests
{
    private static readonly string[] sr_sealedValues =
        [EncryptionDomains.AUDIT, EncryptionDomains.NOTIFICATIONS, EncryptionDomains.COURIER];
    ...
    // ---- Sealed-value removal pins (real values — sequential within this class) ---------------
    ...
    public void SealedValue_FromTrusted_ThrowsCorruptRowFailLoud()
    {
        foreach (var sealedValue in sr_sealedValues)
        {
            ...
            act.Should().Throw<ArgumentException>(
                "a de-cataloged sealed domain in a stored row is corrupt → fail loud");
        }
    }
    ...
    public void Validator_SealedGrant_RefusedWithPreciseMessage()
    {
        ...
        options.AllowedKeyringDomainsByWorkload["audit-consumer"] = [EncryptionDomains.AUDIT];
    ...
    public async Task GetKeyring_ReAdmittedSealedDomain_FiresSealedArm_PastKeyTypeFork()
    {
        ...
        // step-3 KeyType fork passes (it binds AesPayload) — proving the sealed arm is GENUINELY
        // ADDITIVE past that fork.
        using var fixtureSeam = FixturePayloadDomains.Register(EncryptionDomains.AUDIT);
        KeyDomain.Create(EncryptionDomains.AUDIT).Data!.KeyType.Should().Be(
        ...
        var result = await handler.HandleAsync(new GetKeyringInput(EncryptionDomains.AUDIT));

        // The NEW sealed-mode arm fires with the sharp-400 — the DB is never queried.
        result.Success.Should().BeFalse();
