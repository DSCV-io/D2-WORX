...
/// authority deny branches (every origin × identity) with their telemetry, the
...
/// straight from <c>PublicKeyMaterial</c> and is NOT redacted; the full seal→open crypto
...
public sealed class GetOrLazyProvisionSealPublicKeyHandlerTests
{
    ...
    // -----------------------------------------------------------------------
    // 1. Authority deny — unestablished origin (fail-closed first arm)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // 2. Authority deny — unserved plane (System / EdgeInbound). InProcessModule +
    ...
    // -----------------------------------------------------------------------
    // 3. Authority deny — served plane, no caller identity (fail-closed peer)
    // -----------------------------------------------------------------------
    ...
    public async Task GetOrLazyProvisionSealPublicKey_WrongScope_ReturnsForbidden()
    {
        ...
        var result = await BuildPublic(
                ...
                RequestOrigin.CrossProcessHop,
                _FILES,
                scopes: new HashSet<string>(StringComparer.Ordinal) { Scopes.Internal.Kc.Seal.Open })
            .HandleAsync(new GetOrLazyProvisionSealPublicKeyInput(_AUDIT));
    ...
    // -----------------------------------------------------------------------
    // 5. serviceId validation — 400 on garbage (authorized caller)
    // -----------------------------------------------------------------------
    ...
    public async Task GetOrLazyProvisionSealPublicKey_UppercaseServiceId_NormalizesAndProvisions()
    {
        ...
        // VALID input that normalizes — NOT a 400. It provisions the seal:audit domain.
        await using var db = KeyCustodianTestDbContext.CreateEmpty();
    ...
    // -----------------------------------------------------------------------
    // 6. Ordering — authority precedes validation (no serviceId oracle)
    // -----------------------------------------------------------------------
    ...
    public async Task GetOrLazyProvisionSealPublicKey_UnauthorizedCaller_GarbageServiceId_Returns403_NoValidationOracle()
    {
        ...
        // uniform 403 — the authority arm denies BEFORE the serviceId is validated, so no
        ...
        result.ErrorCode.Should().NotBe(
            KeyCustodianErrorCodes.KEYCUSTODIAN_INVALID_WORKLOAD_IDENTITY,
            "authority denies before validation — no serviceId oracle");
        result.StatusCode.Should().Be(HttpStatusCode.Forbidden);
        db.Keys.Should().BeEmpty();
    }

    // -----------------------------------------------------------------------
    // 7. Lazy provisioning — first request provisions one Active EcdhSealing key
    // -----------------------------------------------------------------------
    ...
    {
        // A live Pending successor (no Active) blocks provisioning — the domain is a
    ...
    // -----------------------------------------------------------------------
    // 8. Serving shape — Active + Retiring (active leads)
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // 9. Redaction posture — the served public SPKI is NOT redacted
    // -----------------------------------------------------------------------

    [Fact]
    public void SealPublicEntry_PublicSpki_IsNotRedacted()
    {
        // Public key material is wire-public and must be visible in logs / telemetry —
        ...
        IReadOnlySet<string>? scopes = null)
    {
        var grantedScopes = scopes
            ?? new HashSet<string>(StringComparer.Ordinal) { Scopes.Internal.Kc.Seal.Encrypt };
