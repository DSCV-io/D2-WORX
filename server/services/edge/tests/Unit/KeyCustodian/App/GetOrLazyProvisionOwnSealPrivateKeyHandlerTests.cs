...
/// <summary>
/// The <see cref="GetOrLazyProvisionOwnSealPrivateKeyHandler"/> matrix driven through the REAL handler — the
...
/// structural — a forged in-process caller never reaches provisioning, and a foreign caller on
...
public sealed class GetOrLazyProvisionOwnSealPrivateKeyHandlerTests
{
    ...
    // -----------------------------------------------------------------------
    // 2. The seal-decrypt hard gate — EVERY non-cross-process plane is denied
    // -----------------------------------------------------------------------
    ...
    public async Task GetOrLazyProvisionOwnSealPrivateKey_ForgedInProcessCaller_DeniedAtPlaneArm_NeverProvisions()
    {
        ...
        // ("audit") is denied AT THE PLANE ARM — it never reaches key selection, so no
    ...
    // -----------------------------------------------------------------------
    // 3. Self-only is structural — the key is selected from the caller, not a target.
    ...
    public async Task GetOrLazyProvisionOwnSealPrivateKey_TwinCallers_EachGetsOwnKey_NeverAnothers()
    {
        ...
        // The domain is selected from the caller — audit → seal:audit, files → seal:files.
    ...
    // -----------------------------------------------------------------------
    // 6. Redaction — private PKCS#8 is SecretInformation and never appears in logs
    // -----------------------------------------------------------------------
    ...
    public void SealPrivateEntry_PrivatePkcs8_IsRedactedAsSecretInformation()
    {
        ...
        // The private sealing key MUST carry a redaction marker — it is secret material.
