...
internal static class SealingTestFixtures
{
    ...
    public static RecipientPrivateKeyring SingleKidPrivateKeyring(string serviceId = FIXTURE_SERVICE_ID)
        => new(
            serviceId,
            new Dictionary<string, byte[]>(StringComparer.Ordinal) { [KID_ONE] = sr_keyOne.Pkcs8 });

    /// <summary>A rotated private keyring holding kid1 (retiring) + kid2.</summary>
    /// <param name="serviceId">The recipient service id (defaults to the fixture id).</param>
    /// <returns>A two-kid private keyring.</returns>
    public static RecipientPrivateKeyring RotatedPrivateKeyring(string serviceId = FIXTURE_SERVICE_ID)
        => new(
            serviceId,
    ...
    public static RecipientPublicKeyring SingleKidPublicKeyring(string serviceId = FIXTURE_SERVICE_ID)
        => new(
            serviceId,
            KID_ONE,
            new Dictionary<string, byte[]>(StringComparer.Ordinal) { [KID_ONE] = sr_keyOne.Spki });
    ...
    public static RecipientPublicKeyring RotatedPublicKeyring(string serviceId = FIXTURE_SERVICE_ID)
        => new(
            serviceId,
            KID_TWO,
            new Dictionary<string, byte[]>(StringComparer.Ordinal)
