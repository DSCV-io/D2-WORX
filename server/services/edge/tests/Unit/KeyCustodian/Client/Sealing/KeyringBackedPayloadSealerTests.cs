...
public sealed class KeyringBackedPayloadSealerTests
{
    ...
    {
        var client = new FakeSealingClient(publicResponder: i => D2Result<RecipientPublicKeyring>.Ok(
            i == 1
        ...
        var openedByBoth = new PayloadOpener(SealingTestFixtures.RotatedPrivateKeyring()).Open(frame);
        Encoding.UTF8.GetString(openedByBoth).Should().Be("post");

        var kid1Only = () => new PayloadOpener(SealingTestFixtures.SingleKidPrivateKeyring()).Open(frame);
        kid1Only.Should().Throw<Exception>("the frame was sealed under the rotated (kid2) key");
    }
