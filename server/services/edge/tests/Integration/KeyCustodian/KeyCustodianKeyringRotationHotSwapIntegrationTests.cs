...
/// to refetch over the same gRPC path and atomically swap to the new active kid — while a
    ...
    KeyCustodianPostgresFixture fixture) : IDisposable
{
    ...
    public async Task Rotation_RealHandlerToRealBroker_SwapsWrapperToNewActiveKid_AndDecryptsOldFrame()
    {
        ...
        try
        {
            ...
            // Rotate through the REAL handler — its post-commit announce publishes the real
    ...
    // Publishes warm-up rotation events for the domain until one lands on the channel —
