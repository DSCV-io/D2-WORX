...
/// <c>PayloadCrypto</c> produced under the SAME keyring — a cross-runtime,
/// over-the-wire end-to-end pin.
/// </summary>
/// <remarks>
/// <para>
/// <b>Lane reuse — the <see cref="NodeLeafClientMutualTlsHarnessTests"/> shape.</b>
...
/// this class is self-contained — each harness carries its own probe/spike/cert plumbing.
/// </para>
/// <para>
/// <b>Real KeyCustodian surface — the distribution-test composition.</b> The mTLS host wires
...
/// real overlap partition — the same graph the
...
/// caller identity is the VALIDATED mutual-TLS peer (<c>GetD2PeerWorkloadIdentity()</c> — the
...
/// <c>internal.kc.keyring</c> scope — the token half a production caller would forward — is
...
/// cannot be presented over a loopback socket (Windows-Schannel) — gated by the one-time
...
/// </para>
/// <para>
/// <b>Domain choice — the fixture-seam domain.</b> The sealed flip removed
    ...
    : IDisposable
{
    ...
    // Live-handshake feasibility spike (the FIRST-task gate) — computed ONCE per test run
    ...
    public async Task LiveGetKeyring_OverMutualTls_FetchedKeyringDecryptsDotNetEncryptedFrame()
    {
        ...
        // 2. Fetch the real keyring in-process → encrypt a known plaintext to a v1 frame. This
    ...
    // ----------------------------------------------------------------------
    // Real KeyCustodian composition + seeding (instance — bound to the PG fixture)
    // ----------------------------------------------------------------------
        ...
        IServiceCollection services, TestClock clock, IPayloadCrypto rootCrypto)
    {
        ...
        // The lifecycle-mutation handlers (GenerateKey/Activate/… used to seed the domain)
        // require the dedicated §9.44 root-signing capability, which the general
        // AddD2KeyCustodianApp() deliberately does NOT register (structural isolation) —
    ...
    // The cached live-handshake feasibility outcome — a single immutable reference published
    ...
    /// <see cref="RequestOrigin.Unestablished"/> and the authority rule denies (§9.42).
    /// </summary>
