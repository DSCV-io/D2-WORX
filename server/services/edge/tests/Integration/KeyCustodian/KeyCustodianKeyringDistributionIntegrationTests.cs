...
/// handlers → fetch through the REAL <see cref="GetKeyringHandler"/> → assemble a
/// <see cref="PayloadCryptoKeyring"/> from the wire output (using the returned
/// <c>aadContext</c> verbatim) → encrypt/decrypt round-trip; the rotation-overlap decrypt
    ...
    KeyCustodianPostgresFixture fixture)
{
    ...
    public async Task KeyringDistribution_EndToEnd_RoundTripOverlapExclusionAndUnavailable()
    {
        ...
        // 1. No active key yet → 503 + SR_EmptyKeyringServed + 9513 log.
        ...
        // 4. Generate successor + rotate → overlap (Active + Retiring).
        ...
        // 6. Compromise the retiring (old) key → never served again.
        await using (var sys = BuildSystemProvider(clock, rootCrypto))
        {
