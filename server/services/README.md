...
| [`audit/`](audit/README.md)                 | **Multiproc stub host** (standalone) — Edge gRPC bridge + NIE `PingAudit` path live; product append-only store / RMQ consumer / `audit_db` INSERT role is OUT OF SCOPE (tracked on the product roadmap). See [`audit/README.md`](audit/README.md). |
