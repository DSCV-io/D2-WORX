...
public static class LoadPublicCaAnchors
{
    ...
    /// Env form: <c>AUDIT_MTLS__TRUST_ANCHOR_PATH</c>.
    /// </summary>
    public const string TRUST_ANCHOR_PATH_KEY = "AUDIT_MTLS:TrustAnchorPath";
    ...
    /// The provider does not re-load per handshake.
    /// </remarks>
        ...
        IConfiguration configuration)
    {
        ...
        // Resolve from IConfiguration only (host maps AUDIT_MTLS__* env → config).
        ...
        if (path.Falsey())
        {
            throw new InvalidOperationException(
                $"{TRUST_ANCHOR_PATH_KEY} (or AUDIT_MTLS__TRUST_ANCHOR_PATH) is required "
                + "when MutualTls is enabled — path to the PUBLIC CA root certificate only.");
        }

        // Host-owned cache: load once at provider construction (AddD2AuditHost).
