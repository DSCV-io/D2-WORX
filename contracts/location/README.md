...
- **.NET** — [`server/shared/dotnet/tests/Unit/Location/LocationHashDeterminismTests.cs`](../../server/shared/dotnet/tests/Unit/Location/LocationHashDeterminismTests.cs) loads `parity-fixtures.json` at test runtime and asserts byte-identical hash output for every case
