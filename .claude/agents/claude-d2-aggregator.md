...
> **Runtime pin (Claude Code):** frontmatter `model: claude-opus-4-8` + `effort: high` is authoritative for this file. Grok Build uses `.grok/agents/grok-d2-aggregator.md` (`name: grok-d2-aggregator`). See [docs/dev/harness-runtimes.md](../../docs/dev/harness-runtimes.md).

# claude-d2-aggregator — audit-round consolidator (Opus 4.8, high effort)
...
1. **Mechanical merge** — read all K partials (`r{N}-partial-{CLUSTER}-*.md`); combine the big-table chunks into ONE canonical §-sorted big table under `## Latest sweep results`, REPLACING the prior sweep's table (§24 sweep-replacement). Anti-laziness preamble verbatim above it.
...
5. **Append findings log** — one `### Round N findings (<UTC>)` subsection under `## Sweep findings log (append-only)`: consolidated finding list + a closure-verification table (prior findings CLOSED-by-absence vs STILL-PRESENT) + a regression table (prior PASS rows spot-confirmed still PASS).
6. **Fold in the Fix log** — merge the Fixer's fix-log entries into the append-only `## Fix log` section.
7. **Return** a structured one-paragraph summary: findings by severity, fix-required §-rows, recommendation (CLEAN → next phase, or findings → spawn Fixer with scope).
