...
>
> rules.md = canonical predicate catalog; process.md = canonical workflow + sub-agent architecture + audit-loop protocol. CLAUDE.md condenses both for fast access — update CLAUDE.md + rules.md + process.md in lockstep when either changes (per [rules.md §11.32](docs/dev/rules/11-documentation-parity-best-practices.md#11-documentation-parity--best-practices)).
...
>
> **The main thread ORCHESTRATES — it does NOT plan, implement, audit, or fix. EVERY round of plan / impl / audit / fix is a FRESH sub-agent spawned via `Agent`** (a second audit round = brand-new K=12 Auditor batch + Aggregator; a follow-up fix = brand-new Fixer). The fresh-context property is the entire point — WHY (empirical justification) → [process.md §3](docs/dev/process.md#3-sub-agent-architecture).
>
> **Model per [rules.md §24.0i](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit)** (canonical pinned definitions in `.claude/agents/*.md`): Orchestrator / Planner / Plan-Auditor / Plan-amender → **Fable 5** (`max` only for the Planner); Aggregator / Auditor-deep / Implementer / Fixer → **Opus 4.8**; Auditor / Fixer-mechanical / Investigator → **Sonnet 4.6**. Auditors are two-tier (d2-auditor Sonnet for mechanical clusters; d2-auditor-deep Opus for C2/C3/E2 + ruling-critical); Final-reviewer = the Auditor definitions at deliverable scope. Sweeping Implementer / Fixer Fable carve-out (atomic >40-file / >3-concern / cross-runtime / cascading pipeline — cite the criterion); any other pinned-tier override needs per-occurrence user approval → §24.0i.
>
> **K=12 parallel-cluster dispatch is the default per audit round.** K=1 requires explicit per-round user permission per [rules.md §24.0h](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit) — never self-invoked.
>
...
- **D²-WORX is enterprise-grade, production-ready, robust.** Every change survives bad input, infrastructure failure, concurrency, hostile users, future engineers. "Works on my machine" is not the bar.
...
- **Sub-agent model policy** ([rules.md §24.0i](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit); canonical pinned definitions in `.claude/agents/*.md`): Orchestrator / Planner / Plan-Auditor / Plan-amender → Fable; Aggregator / Auditor-deep / Implementer / Fixer → Opus; Auditor / Fixer-mechanical / Investigator → Sonnet. Auditors two-tier (Sonnet mechanical clusters; Opus d2-auditor-deep for C2/C3/E2 + ruling-critical); Final-reviewer = the Auditor definitions at deliverable scope. Sweeping Implementer / Fixer Fable carve-out per the four §24.0i criteria (atomic >40-file / >3-concern / cross-runtime / cascading pipeline) — cite the criterion + self-attest; any other pinned-tier override (a different model / effort / relaxed tool fence) needs per-occurrence user approval ([§13.14](docs/dev/rules/13-permission-action-discipline.md#13-permission--action-discipline)). Table + trust-but-verify → [process.md §3](docs/dev/process.md#sub-agent-model-policy-per-role) + [§4](docs/dev/process.md#orchestrator-verification-of-sub-agent-outputs).
...
- **Agent-facing docs (CLAUDE.md / rules / process.md / reference docs) are agent-first — human navigability NEVER at the cost of context bloat; densest faithful form wins (no Mermaid / box-drawing art).** [rules.md §11.45]
...
| File | Contents | Committed? | Claude read? | Claude edit? |
|---|---|---|---|---|
...
**Adding a new secret**: (1) add `NEW_THING_API_KEY=replace_with_real_value` to `.env.secrets.example`; (2) wire it into the right service in `infra/compose/compose.yml`; (3) tell the operator to copy it into `.env.secrets`, set the real value, and restart the service. Claude cannot edit `.env.secrets` (deny rule). Same pattern for encryption keys: update `tools/scripts/gen-dev-keys.sh`; operator runs it; output lands in `secrets/`.

The deny rules live in `.claude/settings.json` (committed). The exact-match `**/.env.secrets` deliberately does NOT match `.env.secrets.example` — the template stays fully editable.
