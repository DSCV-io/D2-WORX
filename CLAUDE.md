...
- **Consumable shared packages are versioned per-package** (`D2.Shared.*`, KeyCustodian client, `@d2/*`) — semver + CHANGELOG driven by the commit footer the `tools/release-runner` reads; wire/contract breaks auto-gated by `tools/contract-gate`; library-API breaks author-declared via footer; registry publishing never automatic. [rules.md §26.19]
