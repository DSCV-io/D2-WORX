...
>
> **Roles (model per [rules.md §24.0i](docs/dev/rules.md#24-audit-evidence-discipline-meta--how-to-audit))**: Planner / Plan-amender / Aggregator / Orchestrator → **Fable 5**. Implementer / Auditor / Plan-Auditor / Fixer / Final-reviewer / Investigator → **Opus 4.8**. Sweeping Implementer / Fixer Fable carve-out (atomic >40-file, >3-concern, cross-runtime, cascading pipeline) — cite criterion in dispatch brief. Auditor-shape Fable escalation requires explicit per-occurrence user approval.
>
...
- **Sub-agent model policy (per [rules.md §24.0i](docs/dev/rules.md#24-audit-evidence-discipline-meta--how-to-audit))**: Auditor / Plan-Auditor / Final-reviewer / Implementer / Fixer / Investigator → Opus; Planner / Plan-amender / Aggregator → Fable; Orchestrator (main thread) → Fable. Sweeping Implementer / Fixer Fable carve-out per the four criteria in §24.0i (atomic >40-file / >3-concern / cross-runtime / cascading pipeline) — cite the criterion in the dispatch brief + sub-agent self-attestation. Auditor-shape role escalation to Fable requires explicit per-occurrence user approval per [§13.14](docs/dev/rules.md#13-permission--action-discipline). Canonical table + WHY per role + self-documentation requirement: [process.md §3 Sub-agent model policy per role](docs/dev/process.md#sub-agent-model-policy-per-role). Trust-but-verify discipline (orchestrator verification of sub-agent outputs): [process.md §4 Orchestrator verification of sub-agent outputs](docs/dev/process.md#orchestrator-verification-of-sub-agent-outputs).
