...
>
> **Before planning, implementing, or auditing ANYTHING — read [docs/dev/rules.md](docs/dev/rules.md) + [docs/dev/process.md](docs/dev/process.md) first.** If your work conflicts with them, you're making a mistake **UNLESS the user explicitly acknowledges in writing the SPECIFIC rules / steps being bypassed** (per [rules.md §13.14](docs/dev/rules/13-permission-action-discipline.md#13-permission--action-discipline) — Process-bypass requires explicit written naming).
>
> rules.md is the canonical predicate catalog. process.md is the canonical workflow + sub-agent architecture + audit-loop protocol. CLAUDE.md condenses both for fast-access mental-model purposes per [rules.md §11.32](docs/dev/rules/11-documentation-parity-best-practices.md#11-documentation-parity--best-practices) — update CLAUDE.md + rules.md + process.md in lockstep when either canonical doc changes.
...
>
> **Roles (model per [rules.md §24.0i](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit))**: Planner / Plan-amender / Aggregator / Orchestrator → **Fable 5**. Implementer / Auditor / Plan-Auditor / Fixer / Final-reviewer / Investigator → **Opus 4.8**. Sweeping Implementer / Fixer Fable carve-out (atomic >40-file, >3-concern, cross-runtime, cascading pipeline) — cite criterion in dispatch brief. Auditor-shape Fable escalation requires explicit per-occurrence user approval.
>
> **K=12 parallel-cluster dispatch is the default per audit round.** K=1 (single Auditor doubles as Aggregator) requires **explicit per-round user permission** per [rules.md §24.0h](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit) — never self-invoked.
>
...
>
> _Canonical form (full permission-discipline predicate catalog): [rules.md §13 Permission / Action Discipline](docs/dev/rules/13-permission-action-discipline.md#13-permission--action-discipline) + [process.md](docs/dev/process.md). Update both in lockstep when either changes (per §11.32 KEEP-doc duplication discipline)._
...
>
> ## ⚠️ ROUND SEQUENCE (per [rules.md §24.0a-f](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit))
>
...
>
> _Canonical form (full §24 predicate catalog with Evidence + Why + How blocks per predicate): [rules.md §24 Audit Evidence Discipline](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit). Update both in lockstep when either changes (per §11.32 KEEP-doc duplication discipline)._
...
| --------------------------------------------------------- | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [docs/dev/process.md](docs/dev/process.md)                | Before starting ANY deliverable                                   | Workflow / sub-agent architecture / audit-loop mechanics / Plan-Audit changes (paired with [rules.md §24](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit) if predicate changes)                                                    |
| [docs/dev/rules.md](docs/dev/rules.md) (index) + per-category files under [docs/dev/rules/](docs/dev/rules/) | Read end-to-end at PLAN; walked each audit round (each K=12 cluster reads only its category files under `docs/dev/rules/`) | New predicate or category change — edit the category file under `docs/dev/rules/` (and codegen-discipline row also touches [§26](docs/dev/rules/26-codegen-discipline-spec-proto-schema-derived-types.md#26-codegen-discipline-spec--proto--schema-derived-types))                                                                                            |
...
| [docs/README.md](docs/README.md)                          | Understanding how the doc set is organized (tiers + lifecycle)    | A new tier-level category or holding-pen lifecycle rule changes (paired with [rules.md §11.43](docs/dev/rules/11-documentation-parity-best-practices.md#11-documentation-parity--best-practices))                                                                                          |
...
Handler context is injected via the primary-constructor `HandlerContext<TSelf>` parameter. Per-handler PII redaction via the `[RedactData]` attribute on data types (KEEP) + `DefaultOptions` overrides for proto-generated DTOs that can't carry the attribute. (Full predicate: [rules.md §5.29](docs/dev/rules/05-csharp-code-conventions.md#5-c-code-conventions).)
...
- **NEVER commit without explicit user permission** for THIS commit (not "go ahead" from earlier). [rules.md §13.1](docs/dev/rules/13-permission-action-discipline.md#13-permission--action-discipline)
...
- **Sub-agent model policy (per [rules.md §24.0i](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit))**: Auditor / Plan-Auditor / Final-reviewer / Implementer / Fixer / Investigator → Opus; Planner / Plan-amender / Aggregator → Fable; Orchestrator (main thread) → Fable. Sweeping Implementer / Fixer Fable carve-out per the four criteria in §24.0i (atomic >40-file / >3-concern / cross-runtime / cascading pipeline) — cite the criterion in the dispatch brief + sub-agent self-attestation. Auditor-shape role escalation to Fable requires explicit per-occurrence user approval per [§13.14](docs/dev/rules/13-permission-action-discipline.md#13-permission--action-discipline). Canonical table + WHY per role + self-documentation requirement: [process.md §3 Sub-agent model policy per role](docs/dev/process.md#sub-agent-model-policy-per-role). Trust-but-verify discipline (orchestrator verification of sub-agent outputs): [process.md §4 Orchestrator verification of sub-agent outputs](docs/dev/process.md#orchestrator-verification-of-sub-agent-outputs).
...
- Other convention predicates (string.Empty, no this., brace rules, C# 14 extension members, sealed default, American English, line length, no phase verbiage, tests next to feature) → [rules.md §5/§7](docs/dev/rules/05-csharp-code-conventions.md#5-c-code-conventions) (also covered by MEMORY.md feedback entries).
...
> **Predicates** (zero-tolerance for warnings, write tests, regression-pin every fix, never commit without permission, never defer without permission, etc.) live in [docs/dev/rules.md §13 Permission / Action Discipline](docs/dev/rules/13-permission-action-discipline.md#13-permission--action-discipline) and elsewhere in rules.md. They're walked each audit round.
