...
>
> **Multi-seat K=7 dispatch is the default per audit round:** Default full audit partition is **K=7 concern bundles (A–G)**. **K=12 atomic dispatch is retired.** Targeted **Y ⊆ K=7** (first code-audit default when justified; pure-meta Plan-Audit default E+G) and dirty-only re-dispatch apply on per-step rounds. **FINAL-REVIEW open is mode-selected:** **FR_FULL** (default product open = full K=7 at deliverable scope + own FR journal); **FR_LITE** (deliverable-scope multi-seat Y + Aggregator when all eligibility gates pass); **FR_COLLAPSED** (narrow pure-meta 1-step + README lock; step CLEAN Y-audit = gate; no separate FR journal). FR_LITE / FR_COLLAPSED are multi-seat Y — not K=1. After findings re-dispatch **dirty seats only** (+ sister-blast) — dirty-only is not K=1. K=1 requires explicit per-round user permission per [rules.md §24.0h](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit) — never self-invoked. Wave policy → [process.md Audit wave policy](docs/dev/process.md#audit-wave-policy).
>
...
>
> **EVERY audit round MUST embed a complete markdown evidence table in the journal (`docs/wip/<deliverable>/<NN>-<step>/journal.md`).** Prose-only journals = audit INCOMPLETE = step NOT done. **Compact evidence is REQUIRED** (coverage mandatory; verbosity is not evidence): PASS = short `file:line` (+ optional ≤8-word tag / `E#`); essay PASS illegal; N/A = closed reason-code; FINDING = full four fields; Evidence ledger once per partial. Canonical anti-patterns + N/A codes → [process.md Evidence requirements](docs/dev/process.md#evidence-requirements-mechanical-no-exceptions). Shared-context is SoT for scope/pre-flight/gates/mode — slim briefs. Canonical big table always full catalog (non-Y seats N/A-coded under Y).
>
...
> 1. **Big table** (`## Latest sweep results`) — REPLACED each sweep, one row per rules.md subsection, status `✅ PASS` / `⚪ N/A` / `❌ FINDING-{H/M/L}` / `🟡 *` + compact evidence cell (not essays).
...
> 4. **Next sweep** with brand-new **dirty (+ sister-blast) seat Auditors** (or full K / active Y when justified or mode-required) + Aggregator **full-catalog** merge — clean seats re-cited only via **explicit dirty-only merge**, never silent/Fixer inheritance. PASS in Round N+1 = closed (absence); STILL FINDING = re-fix. **CLEAN** = zero FINDING on the full-catalog table.
...
- **PLAN step breakdown** — **fat-step default** (prefer fewer fatter steps; split only when mechanically beneficial). Fat-step = **PLAN granularity only** — does not waive journals, multi-seat Y/K, tests, or §24 for small code changes inside a step (MANDATORY 1 still binds). Canonical: [process.md PLAN](docs/dev/process.md#plan).
- **EXECUTE** — Per step, in prerequisite order, the orchestrator spawns fresh sub-agents per phase: (1) **Planner** — appends a Plan section to `docs/wip/<deliverable>/<NN>-<step>/journal.md` with pre-emptive gate checks (test-coverage plan, convention / PII / layer check). Plan-Audit three-way: Skip (narrow carve-outs) \| pure-meta **Y** (E+G default) \| product full **K=7**. (2) **Implementer** — writes code + tests; returns files-touched + build/inspectcode status. (3) **Audit loop** — fresh multi-seat Auditor batch + Aggregator per round (**K=7** max; **Y ⊆ K=7** first code-audit default when justified; dirty-only after findings) → fresh Fixer per finding → next round = brand-new sub-agents; terminate on a zero-FINDING sweep (10-iter ceiling); compact 3-layer partials + full-catalog big table ([process.md §4](docs/dev/process.md#4-audit-loop-mechanics) / [rules.md §24](docs/dev/rules/24-audit-evidence-discipline-meta-how-to-audit.md#24-audit-evidence-discipline-meta--how-to-audit)). (4) Per-step distillation → root README's kinds-of-misses log + candidate rules.md predicates.
- **FINAL-REVIEW** — Same audit loop at deliverable scope under mode: **FR_FULL** (default product = full K=7 + own FR journal) / **FR_LITE** (Y + Aggregator when gates pass) / **FR_COLLAPSED** (pure-meta 1-step; step CLEAN Y-audit = gate). Catches integration / cross-step / consistency bugs.
...
| [docs/v2/PHASE_3_AUTH_CORE.md](docs/v2/PHASE_3_AUTH_CORE.md) | Auth Core domain / sessions / orgs / invites / lifecycle; **§0** = full design-set review map for Fable | Auth Core decisions (L-ids); keep annex pointers |
| [docs/v2/PHASE_3_RATE_LIMITING.md](docs/v2/PHASE_3_RATE_LIMITING.md) | Any rate-limit middleware work | Token-bucket RL, AND ceilings, dirty-IP tables, kill-switch |
| [docs/v2/PHASE_3_FINGERPRINTING.md](docs/v2/PHASE_3_FINGERPRINTING.md) | Fingerprint / deviceKey / risk inputs | Device confidence, too-common popularity, session binding, WhoIs modulation |
| [docs/v2/PHASE_3_AUTH.md](docs/v2/PHASE_3_AUTH.md) | JWT mint shape, Pattern A, KeyCustodian / key rotation | JWT claims, anon Pattern A, KC lifecycle, compromise runbook |
