...
| **A2** | **Token issuance + JWKS + service identity** | `POST /oauth/token` (client_credentials + RFC 8693 token-exchange), JWKS publishing + OIDC discovery, OAuth client registry (`oauth_client`), `JsonWebTokenHandler` RS256 (15-min user / 5-min service), the 16-claim payload. | M–L | ☐ Pending |
