...
| F4 | **Keyring distribution endpoint + its consumer wiring.** The consumer-side keyring types (`IKeyringClient`, `GrpcKeyringClient`, `RabbitMqRotationEventChannel`) will ship in 0026 Step 7 — designed in [PHASE_3_AUTH.md](PHASE_3_AUTH.md) §6.4, not yet built (`server/shared/dotnet/auth/keyring/` does not exist). No endpoint exists yet either (the server-side `getKeyring` op is 0026 Step 5). | 📐 specified-deferred | KeyCustodian keyring gRPC endpoint (0026 Step 5; design-era name `internal/keys/{domain}`, built op `KeyCustodianKeyring/GetKeyring`) + messaging |
...
| `D2.Shared.Auth.Keyring` `IKeyringClient` / `GrpcKeyringClient` / `RabbitMqRotationEventChannel` (designed in [PHASE_3_AUTH.md](PHASE_3_AUTH.md) §6.4, not yet built — the lib ships in 0026 Step 7; no endpoint yet) | KeyCustodian keyring gRPC endpoint (0026 Step 5, `KeyCustodianKeyring/GetKeyring`) + `GrpcKeyringClient` wired to it + messaging for the rotation channel | keyring distribution endpoint built (0026 Step 5 / PHASE_3 E5) | F4 |
