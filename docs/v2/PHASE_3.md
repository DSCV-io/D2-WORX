...
| F4 | **Keyring distribution endpoint + its consumer wiring.** The server-side endpoint is now BUILT (0026, the `KeyCustodianKeyring/GetKeyring` gRPC service + the in-process `IKeyCustodianApi.GetKeyringAsync` leaf; authority-gated, `aadContext` on the wire, `keyBytes` redacted). The consumer-side keyring types (`IKeyringClient`, `GrpcKeyringClient`, `RabbitMqRotationEventChannel`) still ship in 0026 Step 7 — designed in [PHASE_3_AUTH.md](PHASE_3_AUTH.md) §6.4, not yet built (`server/shared/dotnet/auth/keyring/` does not exist). | 📐 endpoint built, consumer-deferred | consumer keyring lib + `GrpcKeyringClient` wired to the built `KeyCustodianKeyring/GetKeyring` endpoint (0026 Step 7) + messaging |
...
| `D2.Shared.Auth.Keyring` `IKeyringClient` / `GrpcKeyringClient` / `RabbitMqRotationEventChannel` (designed in [PHASE_3_AUTH.md](PHASE_3_AUTH.md) §6.4, not yet built — the consumer lib ships in 0026 Step 7). The server endpoint is now BUILT (0026, `KeyCustodianKeyring/GetKeyring` gRPC service + the in-process leaf). | `GrpcKeyringClient` wired to the built `KeyCustodianKeyring/GetKeyring` endpoint + messaging for the rotation channel | consumer keyring lib built (0026 Step 7 / PHASE_3 E5) — the endpoint dependency is satisfied | F4 |
| Keyring authority policy `KeyringDomainAuthorityOptions` — built + boot-validated, initial grant map EMPTY (deny-all, fail-closed); no workload can yet fetch any keyring | The in-process Edge consumer's module grant (0026 Step 7) + the Phase-3 per-consumer cross-process grants (audit/notifications/courier services) — set `KEYCUSTODIAN_KEYRING_AUTHORITY__ALLOWEDKEYRINGDOMAINSBYWORKLOAD__<caller>__<n>=<domain>` per workload (boot validator refuses any non-payload grant) | each keyring consumer comes online (0026 Step 7 in-process; Phase-3 host wiring per service) | 0026 Step 5 (keyring grants, Decision 1 B1) |
...
| ~~`PropagatedContext` (operational `x-d2-context`) is read/written on AMQP only; sync .NET hops build context from JWT claims; the call-path interceptor is not wired~~ — **CLOSED (0026)**: `x-d2-context` read/write + the call-path interceptor pair are built and `TestServer`-proven on sync gRPC ([ADR-0025](../adrs/0025-request-context-establishment.md)) | Edge host composition root — register the interceptors on a live pipeline | running Edge host (PHASE_3 A1) | B9 |
| KeyCustodian well-known routes (`GetJwksRouteRegistration` / `GetOidcConfigurationRouteRegistration` — generated `MapGet` extensions on `IEndpointRouteBuilder`, currently committed + TestServer-proven in the KC test assembly) | Edge host composition root — call `endpoints.MapGetJwksRoute()` + `endpoints.MapGetOidcConfigurationRoute()`; the route delegate already resolves `IKeyCustodianApi` from the in-process façade | running Edge host (PHASE_3 A1) | A2 |
...
| `WorkloadCapabilityAuthority.AuthorizeSealEncrypt` pure rule — no production consumer | 0026 Step 9 seal ops — wire `AuthorizeSealEncrypt` into the seal-public-key handler (`getSealPublicKey`) + add real deny tests | 0026 Step 9 (seal ops) built | 0026 DF-02 |
| `WorkloadCapabilityAuthority.AuthorizeSealDecrypt` pure rule — no production consumer | 0026 Step 9 seal ops — wire `AuthorizeSealDecrypt` into the own-seal-private-key handler (`getOwnSealPrivateKey`) + add real deny tests | 0026 Step 9 (seal ops) built | 0026 DF-03 |
| `WorkloadCertificateAuthority.AuthorizeIssuance` is a committed fail-closed deny-all skeleton; `IssueWorkloadCertificateHandler` calls it, so the handler mints a leaf for nobody yet — no caller↔subject binding exists | The certificate-issuance handler — replace the deny-all arm with the real issuance rule IN THE HANDLER: fail-closed on a non-cross-process origin / absent peer identity, and require the requested workload id to equal the authenticated mTLS caller (or route through an explicit, isolated delegated-issuer capability, never the general surface); add the per-handler scope requirement + per-mismatch deny tests. A transport scope ALONE is an impersonation-issuance oracle — the rule must land WITH or BEFORE the cross-process issuance transport | the 0026 cross-process certificate-issuance transport (0026 Step 6 — hard gate) | 0026 |
| Seal-decrypt key selection is designed self-only through the op shape, but the in-process plane's immediate-caller id is caller-supplied | The sealed-decrypt op — select the private key from the unforgeable mTLS peer identity on the cross-process plane, NEVER from the in-process immediate-caller id; add a structural test that an in-process caller cannot select another service's seal key | the 0026 sealed-encryption ops (0026 Step 9 — hard gate) | 0026 |
| The JWT-signing minter capability's DI isolation is proven at module scope only (a module-scope resolvability test); the shared Edge-host container is not built yet | Edge host — a host-level assembly-scan DI-isolation test proving no type outside the auth-mint composition can resolve the JWT-signing capability | running Edge host | 0026 / Edge host |
| mTLS is off by default; a cross-process-mapped host with mTLS disabled would establish no peer identity (a fail-closed deny, but silently dead), and the KeyCustodian signer's transport scope + `d2.internal` audience validators are not wired into a live host | Edge host — hard-require mTLS at boot (fail loud when cross-process services are mapped but mTLS is disabled); wire the transport scope + the `d2.internal` audience validators for the KeyCustodian signer, each with resolvability + negative tests (extends the existing mTLS-wiring row) | running Edge host | 0026 / Edge host |

**Note on the JWKS + OIDC discovery routes** — `/.well-known/jwks.json` and `/.well-known/openid-configuration` ARE generated (route registration + DTOs + in-process façade), proving the earlier "well-known JSON is fringe" claim in ADR-0021 was an untested assumption (amended 2026-06-28). The generated route registrations are committed + TestServer-proven in the KC test assembly today; the only deferred piece is the Edge host calling `Map…Route()` in its composition root (A1) and extending the discovery document with the `token_endpoint` fields when the token endpoint ships (A2) — both rows above.
