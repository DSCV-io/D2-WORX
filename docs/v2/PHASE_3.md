...
| `WorkloadCapabilityAuthority.AuthorizeSealEncrypt` pure rule — no production consumer | 0026 Step 9 seal-encrypt handler — wire `AuthorizeSealEncrypt` into the handler + add real deny tests | 0026 Step 9 (seal-encrypt op) built | 0026 DF-02 |
| `WorkloadCapabilityAuthority.AuthorizeSealDecrypt` pure rule — no production consumer | 0026 Step 10 seal-decrypt handler — wire `AuthorizeSealDecrypt` into the handler + add real deny tests | 0026 Step 10 (seal-decrypt op) built | 0026 DF-03 |
