...
| EXECUTE — 0009 Step 7: Final-review (K=5 + Aggregator per audit-framework.md §3a-c) + Deliverable Completeness Checklist | ☐ Not started | |
...
**Scope**: standalone TS workspace at `tools/geo-data-pipeline/` that fetches raw data from upstream sources (CLDR / IANA tzdb / libphonenumber / SIX ISO 4217 / datasets/* / Wikidata SPARQL); transforms into our spec shape (per Decision 6 entity definitions in this doc); merges with hand-curated overlays; provides diff/approve tooling; emits the committed `contracts/geo/*.spec.json` files.
