...
| C18 | **gRPC transport-mapper: nested-model / array-of-model field path (capability gap).** The generated gRPC client + service transport mappers map **scalar / bytes / enum fields only** (`buildClientProtoToDto` / `buildProtoToDto` assign `source.Property` directly); a nested-model or array-of-model gRPC RESPONSE field assigns a proto sub-message straight onto a DTO of a different type → it does NOT compile (CS0200-class). The proto emitter itself already emits nested message blocks (so the wire shape exists); only the mapper's per-field recursion is missing. Today no op exercises it: the `sign` / `signWithKind` / `placeOrder` fixtures are flat-mappable, and the **`placeOrderV2`** nested/array-of-model fixture deliberately commits only its DTOs + predicate twin (the predicate emitter crawls the output MODEL + AST at gen time and needs NO transport mapper — so the cross-language predicate behavior is fully parity-proven without a compiled client). When a real `@d2GrpcMethod` op first needs a nested-model / array-of-model gRPC response, extend the transport-mapper emitter to recurse per field into a nested proto↔DTO sub-mapper **and** add the matching byte-gate. | 📐 specified-deferred | `server/shared/typescript/typespec-emitters/src/lib/grpc-client-emitter.ts` (`buildClientProtoToDto` ~:996) + `src/lib/grpc-service-emitter.ts` (`buildProtoToDto`) + `server/shared/typescript/typespec-emitters/VALIDATION.md` (gRPC-client rows note) | nothing structural — buildable now (host-independent); unblocked the moment a `@d2GrpcMethod` op first carries a nested-model / array-of-model response field |
