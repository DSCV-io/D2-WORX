...
| 4. Shared library implementation (per V2.md §4 Phase 0)          | ✅ **Complete — Waves 1-3 done (Result, Utilities, Resilience, I18n trio); Wave 2 done (full handler stack + repo-handler trio + auth/context vocabulary + spec-driven codegen); Wave 4 done (`DcsvIo.D2.Encryption`); Wave 5 done (caching stack — Abstractions / Local.Default / Distributed.Redis / Tiered); Wave 6 done (`DcsvIo.D2.Messaging` — `[MqPub]` / `[MqSub]` spec-driven model + RabbitMQ impl + DLQ republish + W3C trace propagation); Wave 7 done (`DcsvIo.D2.Logging` + `DcsvIo.D2.Telemetry` + `DcsvIo.D2.AspNetCore` + `DcsvIo.D2.ServiceDefaults` — squash `b3a05f1c` on `nova`, deliverable 0004 SHIPPED 2026-05-12 per [`docs/dev/deliverables/0004-service-defaults.md`](../dev/deliverables/0004-service-defaults.md)); **2914 tests passing across all built libs** incl. Testcontainers Redis + Testcontainers RabbitMQ + synthetic-host integration coverage. Phase 0 is closed. Two pre-Phase-1 deliverables are next (codegen cleanup + TS bridge — see "Pre-Phase-1 Plan" section below) before Phase 1 (Geo libs) begins.** | [Per-library checklist (Stage 4)](#per-library-checklist-stage-4)                 |
...
- `DcsvIo.D2.Tests` was scaffolded alongside `DcsvIo.D2.Result` (rather than waiting for Wave 3) so the Result lib lands with full test coverage at point-of-merge. Each subsequent shared-lib PR adds its own `Unit/{Lib}/` subdirectory.
...
Design decisions captured during planning that govern Phase 0 implementation. Each note describes the _intent_; implementation lands in the per-library code under `server/shared/dotnet/{lib}/` when `DcsvIo.D2.Handler` (and its consumers) are built. Summarized in `docs/PATTERNS.md` once landed.
...
**Single source of truth** across (1) the constant catalog, (2) the factory catalog, (3) the telemetry tag whitelist, (4) the i18n key references. **Ready for TS-side emission** in deliverable 0006 (where `@dcsv-io/d2-auth-abstractions` reads the same spec).
...
Per the locked Tier 1 list in V2.md §5.8: `@dcsv-io/d2-result`, `@dcsv-io/d2-utilities`, `@dcsv-io/d2-resilience`, `@dcsv-io/d2-protos`, `@dcsv-io/d2-auth-context-abstractions`, `@dcsv-io/d2-request-context-abstractions`, `@dcsv-io/d2-auth-abstractions`, `@dcsv-io/d2-i18n`, `@dcsv-io/d2-logging`, `@dcsv-io/d2-telemetry`, `@dcsv-io/d2-service-defaults`, `@dcsv-io/d2-headers`, `@dcsv-io/d2-grpc-client`. Per-package READMEs follow the .NET shared-lib README convention (Purpose / Public API / Dependencies / V2.md reference). The four `@dcsv-io/d2-headers-{common,http,amqp,grpc}` per-transport catalogs ship alongside as codegen-emitted siblings to the .NET `DcsvIo.D2.Headers.*` libs (one spec, parallel emission).
...
- Ripped out v1-leftover code in `server/web/src/lib/server/` (`auth.server.ts`, `hooks/*`, `middleware.server.ts`, REST gateway clients) that referenced dead `@dcsv-io/d2-*` packages from v1.
