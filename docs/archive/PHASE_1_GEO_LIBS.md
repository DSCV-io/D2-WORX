...
> **ARCHIVED 2026-06-05** — 0009-geo-libs deliverable SHIPPED. Moved to `docs/archive/PHASE_1_GEO_LIBS.md`. Phase 2 (DataGovernance + Contacts) also shipped on `n/data-governance` + `n/contacts`. Phase 3 (Build Edge) is next.
...
**Deliverable workspaces**: Phase 1 splits into TWO deliverables (see "Phase 1 deliverable structure" section below).

- **0008-geo-data-pipeline** (first; data-ingestion de-risk): [`docs/wip/0008-geo-data-pipeline/`](../wip/0008-geo-data-pipeline/) (gitignored) — lightweight PLAN + append-only journal for the iterative-spike data pipeline build.
- **0009-geo-libs** (second; consumes 0008's specs): [`docs/wip/0009-geo-libs/`](../wip/0009-geo-libs/) (gitignored) — execution journals + per-step status for the lib build. The workspace README intentionally duplicates this doc's decision set for redundancy (user-approved).
...
| Stage                                                                                                                                                                              | Status                | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PLAN — 0009-geo-libs (this doc)                                                                                                                                                    | ✅ Complete           | Deep multi-turn design conversation locked all decisions; documented in full below                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| PLAN — 0008-geo-data-pipeline                                                                                                                                                      | ✅ Complete (NEW)     | Lightweight PLAN at `docs/wip/0008-geo-data-pipeline/README.md`; iterative-spike approach                                                                                                                                                                                                                                                                                                                                                                                                                               |
| EXECUTE — Deliverable 0008-geo-data-pipeline (data pipeline, spec authoring)                                                                                                       | ✅ Complete           | On branch `n/geo-data-pipeline` (off clean `nova` @ `c8709b5f`); shipped 8 spikes + final-review audit (R1→R4 K=5 + Aggregator; today's canonical default is K=12; 24/24 priors CLOSED; 5,477 active tests passing). Output: 13 spec files at `contracts/geo/` + 13 JSON Schema files + standalone TS pipeline at `tools/geo-data-pipeline/` + operator CLI (`geo:refresh` / `geo:diff` / `geo:approve` / `geo:bump-version`). See [`docs/dev/deliverables/0008-geo-data-pipeline.md`](../dev/deliverables/0008-geo-data-pipeline.md) for the snapshot. |
| Go/no-go user-review gate (between 0008 and 0009)                                                                                                                                  | ✅ Complete           | 0008 commits queued for user review (`6920c1be` daily-sweep close-out + `30bdad4f` 0008 audit cleanup + this PHASE_1.md sync + deliverable doc snapshot + 13 schema files + 1 `rules.md` §24 augmentation). Explicit user approval gates 0009 EXECUTE start.                                                                                                                                                                                                                                                            |
| EXECUTE — Deliverable 0009-geo-libs (consumes 0008's specs)                                                                                                                        | ✅ Complete           | Step ordering below (per Decision 18) — Step 0 Branch checkout `n/geo-libs` off clean `nova` after 0008 ships                                                                                                                                                                                                                                                                                                                                                                                                           |
| EXECUTE — 0009 Step 1: `DcsvIo.D2.Time` + `@dcsv-io/d2-time`                                                                                                                               | ✅ Complete           | NodaTime wrapper, IClock, EF value converters; Temporal API wrapper TS-side                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| EXECUTE — 0009 Step 2: `DcsvIo.D2.Geo.Abstractions` + `@dcsv-io/d2-geo-abstractions`                                                                                                       | ✅ Complete           | Types + lookup interfaces + DeprecationInfo (zero deps)                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| EXECUTE — 0009 Step 3: Spec consumption + .NET SourceGen + TS emitter                                                                                                              | ✅ Complete           | Consume 0008-produced specs (countries / subdivisions / currencies / languages / locales / timezones spec.json)                                                                                                                                                                                                                                                                                                                                                                                                         |
| EXECUTE — 0009 Step 4: `DcsvIo.D2.Geo.Default` + `@dcsv-io/d2-geo-default`                                                                                                                 | ✅ Complete           | Codegen'd lookups + sub-exports for tree-shake                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| EXECUTE — 0009 Step 5: `DcsvIo.D2.Location` + `@dcsv-io/d2-location`                                                                                                                       | ✅ Complete           | 3 VOs + composition function + `IPostalCodeValidator`                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| EXECUTE — 0009 Step 6: Cross-cutting (DROP `Region` field from IRequestContext spec — entire field removed, no rename; PATTERNS.md + new TIMESTAMPS.md; rules.md predicate drafts) | ✅ Complete           | Doc updates + spec drop                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| EXECUTE — 0009 Step 7: Final-review (K=12 + Aggregator per process.md §3a-c) + Deliverable Completeness Checklist                                                                  | ✅ Complete           |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| SHIP — 0008-geo-data-pipeline                                                                                                                                                      | ✅ Complete           | Deliverable doc snapshot at [`docs/dev/deliverables/0008-geo-data-pipeline.md`](../dev/deliverables/0008-geo-data-pipeline.md) ready. Squash-merge `n/geo-data-pipeline` → `nova` completed. **Process lesson captured**: `rules.md` §24 augmented with tamper-evident Fixer dispatch protocol after the F-3 false-closure pattern (TWO consecutive Fixers self-reported running `pnpm tier-2:build` without the regen actually landing).                                                                               |
| SHIP — 0009-geo-libs                                                                                                                                                               | ✅ Complete           | Squash merged `n/geo-libs` → `nova`; applied **14 approved rules.md predicates** (§1.22-§1.30 + §1.31 strengthened + §1.32 new + §13.5 + §13.6 new + §7.x); snapshot README at `docs/dev/deliverables/0009-geo-libs.md`; V2.md §4 Phase 1 row updated to ✅                                                                                                                                                                                                                                                             |

**Status legend**: ✅ Complete · 🔄 In progress · ☐ Not started · ⏸ Blocked

**Branches**:

- 0008-geo-data-pipeline: `n/geo-data-pipeline` (already checked out from clean `nova` @ `c8709b5f`).
- 0009-geo-libs: `n/geo-libs` (TBD — 0009 Step 0 will check out from clean `nova` after 0008 ships).

---

## Overview

Phase 1 ships **four .NET shared libraries** + **four TypeScript parity packages**, plus cross-cutting doc + spec changes. The work is split across **two deliverables**: `0008-geo-data-pipeline` (data ingestion + spec authoring; ships the 14 JSON spec files at `contracts/geo/*.spec.json`) and `0009-geo-libs` (consumes those specs to build the 4 libs + 4 TS packages, executed in 8 ordered steps). See "Phase 1 deliverable structure" section above for the split rationale. No new services. No DB migrations (the libs are pure embedded data + types). Estimated effort: ~2-3 weeks per V2.md §4 (slightly expanded from the original "1-2 wks" estimate because the locked PLAN folds DcsvIo.D2.Time forward from "deferred" + ships a temporal model with full v1-vs-v2 reasoning).

**Forward-looking sections included** (per the §13.6 itemization-discipline + §1.31 v1-functional-preservation rules): this doc additionally carries (a) a **Phase 2 (DcsvIo.D2.Contacts) v1 carry-forward inventory** so the Phase 2 PLAN doc inherits an already-audited scope baseline, (b) a **Phase 3 (Edge WhoIs module) v1 carry-forward inventory** for the same reason — _INCLUDING a net-new geo-security capability row_ (per-country block-list + per-country `RateLimitTier` override + WhoIs-driven geographic risk scoring) operationalizing user's strategic framing about blocking high-abuse-risk countries, (c) an explicit **"Intentional drops from v1 (with rationale)"** section separating user-approved-and-rationale-documented v1 drops from §1.31-audit-caught silent drops (which this PLAN restores), and (d) a **"Languages deferred from Phase 1 (with revisit triggers)"** section documenting the user's strategic stance on which languages stay out of Phase 1 scope (Russian, mainland Chinese zh-CN, Brazilian Portuguese pt-BR, Hindi/Tamil/Bengali, Arabic, Hebrew, Turkish, SEA langs, Nordic langs) and the revisit triggers per language. See later sections in this doc for each.
...
```
DcsvIo.D2.Time                      (NEW Phase 1 lib — NodaTime wrapper)
  └── depends on: NodaTime, Npgsql.NodaTime

DcsvIo.D2.Geo.Abstractions          (types + lookup interfaces; ZERO deps — no NodaTime, no Geo.Default)
  └── depends on: Result, Utilities (Phase 0)

DcsvIo.D2.Geo.Default               (embedded denormalized in-memory catalogs)
  └── depends on: Geo.Abstractions

DcsvIo.D2.Location                  (3 value objects + composition function)
...
```
@dcsv-io/d2-time                            (Temporal API wrapper; polyfilled for old browsers)
...
1. **`DcsvIo.D2.Time`** — NodaTime wrapper + `IClock` + `SystemClock` + `TestClock` + Npgsql.NodaTime EF value-converter config + `ZonedInstant` + `LocalAnchoredEvent` records. Pulled forward from "deferred" because Phases 4-5 (Notifications scheduling, dkron local-anchored jobs) and historical-accuracy use cases (invoicing / audit logs) need NodaTime now, not later. ~1.5MB DLL footprint is the price.
2. **`DcsvIo.D2.Geo.Abstractions`** — pure types + lookup interfaces. Zero external deps (deliberately — no NodaTime drag). Domain layers in any service can reference this freely without pulling the catalog data or temporal lib.
3. **`DcsvIo.D2.Geo.Default`** — embedded denormalized in-memory catalogs (countries, subdivisions, currencies, languages, locales, timezones), codegen'd from JSON specs. Heavy data (~110KB raw / ~30KB gzipped per the catalog-size analysis); separated from Abstractions so consumers that only need types/interfaces don't drag the data.
4. **`DcsvIo.D2.Location`** — three value objects (`Coordinates`, `StreetAddress`, `AdminLocation`) + `ComposeLocationHash` free function + `IPostalCodeValidator` interface + `DefaultPostalCodeValidator`. Depends on Geo.Abstractions ONLY (NOT Default) so the Location lib stays pure-domain.

**Why this shape**: each lib has one clear responsibility. Mirrors Phase 0's Abstractions/.Default convention (`DcsvIo.D2.Caching.Abstractions` + `.Local.Default` + `.Distributed.Redis`). Geo.Abstractions stays pure (just data records + lookup interfaces); Location depends only on Abstractions so it stays pure-domain without dragging in ~110KB of catalog data; Default holds the heavy embedded data; Time is wholly separate concern (temporal math).
...
## Phase 1 deliverable structure (two-deliverable split)

Phase 1 ships as **TWO sequential deliverables** with a hard go/no-go user-review gate between them. This split de-risks the data-ingestion concern (the highest-uncertainty piece of Phase 1) before sinking architectural effort into consuming the data.

### Deliverable 0008 — `geo-data-pipeline` (first; de-risks data ingestion)

**Goal**: produce + populate the 14 JSON spec files (7 Layer A upstream-derived + 7 Tier 2 manually-curated) that 0009-geo-libs will consume at codegen time.

**Scope**: standalone TS workspace at `tools/geo-data-pipeline/` that fetches raw data from upstream sources (CLDR / IANA tzdb / libphonenumber / SIX ISO 4217 / datasets/_ / Wikidata SPARQL); transforms into our spec shape (per Decision 6 entity definitions in this doc); merges with hand-curated overlays; provides diff/approve tooling; emits the committed `contracts/geo/_.spec.json` files.

**Approach**: iterative-spike (light planning, heavy trying); 0008 PLAN at `docs/wip/0008-geo-data-pipeline/README.md` is intentionally minimal. Lessons captured in append-only journal at `docs/wip/0008-geo-data-pipeline/journal.md`.

**Definition of Done**:

- All 7 Layer A spec files populated from upstream (countries / subdivisions / currencies / languages / locales / timezones / phone-metadata)
- All 7 Tier 2 spec files initial authorship complete (selectable-locales / supported-languages / supported-currencies / selectable-timezones / subdivision-types / country-currency-overrides / geopolitical-entities)
- Diff/approve tooling proven (`pnpm geo:diff` + `pnpm geo:approve` work on a simulated upstream change)
- License bundle clean (all sources confirmed PolyForm-Strict-compatible; `THIRD-PARTY-NOTICES.md` complete)
- Performance acceptable (full refresh completes in <10 min)
- CatalogVersion v1.0 baseline established
- **Go/no-go user review with explicit approval before 0009 starts**

**Pivot triggers** (if any of these fail in 0008 → 0009 architecture revisits):

- CLDR JSON extraction is too painful → pivot to NPM packages (i18n-iso-countries, cldr-json) instead of raw upstream
- Some sources unreliable → reduce scope (skip full ~700 locales; ship just selectable + minimal supplement)
- License audit reveals issues → swap sources per the research's "AVOID" list
- Refresh tooling too complex → drop diff/approve workflow; full-snapshot replace; rely on `git diff` for review
- Pipeline architecture wrong entirely → pivot to v1-style hand-maintained catalogs (smaller scope, no upstream pull)

### Deliverable 0009 — `geo-libs` (second; consumes 0008's specs)

**Goal**: ship the 4 .NET libs + 4 TS packages per the design locked in this PHASE_1.md doc.

**Scope**: everything currently designed in this doc (all 18+ Decisions, value objects, lookup APIs, Pattern B, etc.) — consumes the spec files produced by 0008.

**Prerequisite**: 0008 ships + user go/no-go review passes. If pivot triggers fired during 0008, 0009 PLAN is revised first (consultation with user before EXECUTE).

**Approach**: heavy planning (this doc IS the plan), per-step review per Phase 1 execution discipline.

### Why split?

Data ingestion carries unique risks (CLDR JSON structure complexity, IANA tzdb parsing, libphonenumber XML, SIX XML, Wikidata SPARQL pagination, cross-source merging, license compliance, performance). If any breaks badly, the architecture pivots. Better to discover that during 0008 — before writing handlers + DI + tests in 0009 that assume the pipeline works.
...
| ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Reference data: Country entity** (250 entries)                                                                                    | `Geo.Infra/Repository/Seeding/CountrySeeding.cs:34-44`                                                                                                             | `DcsvIo.D2.Geo.Abstractions/Country.cs` + `DcsvIo.D2.Geo.Default/CountryLookup.cs` (codegen from `contracts/geo/countries.spec.json`)                                                                                                                                                                                               | ✅ Carried; v2 adds endonyms + phone metadata + Deprecation + CLDR enrichments                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **Reference data: Subdivision entity** (183 entries, US/CA full + sparse)                                                           | `Geo.Infra/Repository/Seeding/SubdivisionSeeding.cs:36-43`                                                                                                         | `DcsvIo.D2.Geo.Abstractions/Subdivision.cs` + `DcsvIo.D2.Geo.Default/SubdivisionLookup.cs` (codegen)                                                                                                                                                                                                                                | ✅ Carried + EXPANDED (full ISO 3166-2 ~3,600 entries; v1's sparse coverage was a known debt; confirmed full ISO 3166-2 per user re-affirmation 2026-05-17 — massive expansion from v1's 183-sparse stopgap)                                                                                                                                                                                                                                                                                                                                                      |
| **Reference data: Currency entity** (5 sample entries)                                                                              | `Geo.Infra/Repository/Seeding/CurrencySeeding.cs:36-43`                                                                                                            | `DcsvIo.D2.Geo.Abstractions/Currency.cs` + `DcsvIo.D2.Geo.Default/CurrencyLookup.cs`                                                                                                                                                                                                                                                | ✅ Carried + EXPANDED (full ISO 4217 active ~180 entries; historical entries deferred per Decision 16)                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Reference data: Language entity** (6 entries)                                                                                     | `Geo.Infra/Repository/Seeding/LanguageSeeding.cs:35-40`                                                                                                            | `DcsvIo.D2.Geo.Abstractions/Language.cs` + `DcsvIo.D2.Geo.Default/LanguageLookup.cs`                                                                                                                                                                                                                                                | ✅ Carried + MASSIVELY EXPANDED (v1 = 6 → v2 = **~180 (full ISO 639-1 catalog; 11 marked `IsSupported=true`)** per Decision 6c data-vs-selectability principle. MORE preserving than v1 (every ISO 639-1 language now resolvable, not just 6); the 11 supported (en, es, fr, de, it, ja v1 carry-forward + nl, ko, zh, pt, pl NEW additive per user re-affirmation 2026-05-17) have translation files and `IsSupported=true`; the remaining ~169 ship catalog-only as `IsSupported=false`. v2 adds Deprecation + LTR/RTL writing direction + `IsSupported` flag.) |
| **Reference data: Locale entity** (138 entries)                                                                                     | `Geo.Infra/Repository/Seeding/LocaleSeeding.cs:38-45`                                                                                                              | `DcsvIo.D2.Geo.Abstractions/Locale.cs` + `DcsvIo.D2.Geo.Default/LocaleLookup.cs`                                                                                                                                                                                                                                                    | ✅ Carried + MASSIVELY EXPANDED (138 v1 → **~700 v2, full CLDR BCP-47 locale catalog** per user re-affirmation 2026-05-17; ships every CLDR-defined locale for future selectable expansion without re-codegen; ~75-100KB gzipped trivial cost; **18 selectable** for UI: en-US, en-CA, en-GB, en-AU, en-NZ, es-ES, es-MX, fr-FR, fr-CA, de-DE, it-IT, ja-JP, nl-NL, nl-BE, ko-KR, zh-TW, pt-PT, pl-PL [10 v1-style + 8 NEW]; v2 adds `IsSelectable` flag + Deprecation + decimal/thousands separators + date format pattern + first-day-of-week)                  |
| **Reference data: Timezone entity** (309 entries)                                                                                   | `Geo.Infra/Repository/Seeding/TimezoneSeeding.cs:36-43`                                                                                                            | `DcsvIo.D2.Geo.Abstractions/Timezone.cs` + `DcsvIo.D2.Geo.Default/TimezoneLookup.cs`                                                                                                                                                                                                                                                | ✅ Carried + EXPANDED (~600 full IANA, ~150-200 selectable; v2 adds localized display names + aliases + Selectable + Deprecation)                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Reference data: GeopoliticalEntity entity** (59 entries; 23-value type enum)                                                      | `Geo.Domain/Entities/GeopoliticalEntity.cs` + `Geo.Domain/Enums/GeopoliticalEntityType.cs` + `Geo.Infra/Repository/Seeding/GeopoliticalEntitySeeding.cs`           | `DcsvIo.D2.Geo.Abstractions/GeopoliticalEntity.cs` + `DcsvIo.D2.Geo.Abstractions/GeopoliticalEntityType.cs` + `DcsvIo.D2.Geo.Default/GeopoliticalEntityLookup.cs` (codegen from `contracts/geo/geopolitical-entities.spec.json`)                                                                                                    | ✅ Carried (entity + 23-value enum + 59 seeded entries + many-to-many Country↔GE navigation — see Decision 6a)                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| **Coordinates VO** (lat/lon record)                                                                                                 | `Geo.Domain/ValueObjects/Coordinates.cs:24-133`                                                                                                                    | `DcsvIo.D2.Location/ValueObjects/Coordinates.cs`                                                                                                                                                                                                                                                                                    | ✅ Carried; v2 adds `AccuracyMeters` + `Geohash` + `PlusCode` + per-VO `HashId`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **StreetAddress VO** (Line1/2/3 record)                                                                                             | `Geo.Domain/ValueObjects/StreetAddress.cs:22-149`                                                                                                                  | `DcsvIo.D2.Location/ValueObjects/StreetAddress.cs`                                                                                                                                                                                                                                                                                  | ✅ Carried; v2 adds per-VO `HashId`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Location aggregate** (atomic Location bundling coordinates + street + city + postal + subdivision + country with single `HashId`) | `Geo.Domain/Entities/Location.cs:31-219`                                                                                                                           | DECOMPOSED into `Coordinates` + `StreetAddress` + `AdminLocation` (new VO) + `ComposeLocationHash` free function in `DcsvIo.D2.Location`                                                                                                                                                                                            | ✅ Carried (composition gives the v1-style atomic identity to consumers who want it — see Decision 4)                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **AdminLocation concept** (city + postal + subdivision + country slot of v1 Location)                                               | Implicit in `Geo.Domain/Entities/Location.cs` fields                                                                                                               | EXPLICIT new VO: `DcsvIo.D2.Location/ValueObjects/AdminLocation.cs`                                                                                                                                                                                                                                                                 | ✅ Carried + ELEVATED (v1 had it implicit; v2 makes it a first-class VO with its own HashId)                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Contact entity (v1 was a Geo entity)                                                     | NOT in Phase 1 — `DcsvIo.D2.Contacts` folded owned-component library in Phase 2 (see ADR-0001)                                                                     | Phase 2 Contacts                                                  |
...
_Applies to BOTH Phase 1 deliverables (0008-geo-data-pipeline AND 0009-geo-libs); NOT general workflow. Reason: foundational work dealing with data structures + data stored and moved over wire. High cost to get wrong; high vigilance required. The go/no-go user-review gate between 0008 and 0009 is itself an instance of the per-step review discipline below — applied at the inter-deliverable boundary._
...
**Why**: each lib has one clear responsibility. Mirrors Phase 0's Abstractions/.Default convention (`DcsvIo.D2.Caching.Abstractions` + `.Local.Default` + `.Distributed.Redis`).
...
- **One monolithic `DcsvIo.D2.Geo` lib** (v1 shape minus the service wrapper). Rejected: violates the established Abstractions/.Default split convention; forces every consumer to take the ~110KB catalog data even if they only use lookup interfaces; ties pure domain code to NodaTime.
...
| Consumer access | gRPC call to Geo service via `Geo.Client` | Direct in-process import of `DcsvIo.D2.Geo.Abstractions` / `.Default` / `.Location` |
...
| Contacts | Geo entity | Folded owned-component library (`DcsvIo.D2.Contacts` — Phase 2; see ADR-0001) |
...
Each entity below ships in `DcsvIo.D2.Geo.Abstractions` as a sealed record. **Records, not classes** — they're immutable wire-shape data. Codegen emits the records + the lookups in `DcsvIo.D2.Geo.Default`.
...
// @dcsv-io/d2-geo-default/geopolitical-entities → GeopoliticalEntityLite (no countries[] field);
// @dcsv-io/d2-geo-default/geopolitical-entities/full → GeopoliticalEntityFull (with countries: CountryLite[]).
```
...
No gRPC. No reference-data cache lookup. No round-trip. The denormalized arrays in `DcsvIo.D2.Geo.Default` are loaded once at lib import and live as static readonly data.

#### v1 vs v2 comparison

| Concern                   | v1                                                                                                                                                    | v2                                                                                                                         |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| Entity definition         | `Geo.Domain/Entities/GeopoliticalEntity.cs` (record with `ShortCode`, `Name`, `Type`, `ICollection<Country>` EF nav)                                  | `DcsvIo.D2.Geo.Abstractions/GeopoliticalEntity.cs` (sealed record + lite/full denormalized arrays + Deprecation)           |
| Enum                      | `Geo.Domain/Enums/GeopoliticalEntityType.cs` (23 values, 4 region-tagged categories)                                                                  | `DcsvIo.D2.Geo.Abstractions/GeopoliticalEntityType.cs` (carried verbatim)                                                  |
...
A small shared module under `DcsvIo.D2.Geo.Abstractions/NameResolution/` (parallel TS at `@dcsv-io/d2-geo-abstractions/src/name-resolution/`):

```
DcsvIo.D2.Geo.Abstractions/NameResolution/
...
Identical logic shipped TS-side under `@dcsv-io/d2-geo-abstractions/src/name-resolution/` (NFD via `String.prototype.normalize('NFD').replace(/\p{M}/gu, '')`).
...
### Fail-closed safety predicates (Amendment 40 — added 2026-05-23)

Per user direction: prefer "unresolved" over incorrect resolution. Worst-case scenario is silent data corruption from a wrong fuzzy match. Downstream consumers (per the WhoIs enrichment flow above) preserve raw upstream strings in audit, so unresolved outcomes are recoverable — wrong outcomes are not.

1. **Pass 1 (exact)** — no min input length. Always run.
2. **Pass 2 (startsWith)** — skip if `q.length < 4`. Short prefixes are too promiscuous (e.g., "IR" prefix-matches IRAN + IRAQ + IRELAND).
3. **Pass 3 (contains)** — skip if `q.length < 5`. Short substrings overmatch (e.g., "Carol" contains-matches both North + South Carolina).
4. **Pass 4 (Levenshtein)** — skip if `q.length < 5`. Within-pass, `maxDistance ≤ min(2, floor(q.length / 5))`. Hard cap from this section's `≤ 2`; scaled lower for short inputs (e.g., "IRAM" at 4 chars would be skipped; "Califrnia" at 9 chars allows distance 1; "United Statez" at 13 chars allows distance 2).
5. **Ambiguity detection (Pass 2/3/4)** — if multiple entities match at the SAME score/distance, return null. NOT "first match wins". Eliminates silent catalog-order-dependent picks.
6. **Cascade short-circuit** — first matching pass returns; later passes don't run. Cascade ORDER encodes confidence (exact > prefix > substring > fuzzy) — don't second-guess.
7. **Catalog uniqueness assertion** — codegen-time check enforces no duplicate normalized names across any matchable name field per entity type. Build fails with `D2GEO007` diagnostic if duplicates found. Eliminates the "first match wins" determinism risk at the source.

**Illustrative adversarial cases (representative — NOT exhaustive; Step 3 parity fixture is the comprehensive set):**

MUST return null: `""`, `"   "`, `"X"`, `"IR"`, `"IRAM"`, `"Carolina"`, `"Some Obscure Region"`.

MUST return specific entity: `"United States"` → US, `"USA"` → US, `"Sao Paulo"` → BR-SP, `"California"` → US-CA, `"South Carolina"` → US-SC (cascade short-circuits at Pass 1), `"California Republic"` → US-CA (Pass 3 unambiguous), `"Califrnia"` → US-CA (Pass 4 unambiguous), `"Île-de-France"` → FR-IDF.

Locale-edge: Turkish dotted İ, Lithuanian Ė, German ß in city names, RTL Arabic country names, CJK characters — parity-pinned in the Step 3 fixture; outcomes are byte-identical across .NET + TS implementations. Where `.toLowerCase()` diverges from `ToLowerInvariant()`, implementations MUST use a custom case-fold table or Intl.Collator to align.

The full adversarial test fixture lives at `contracts/geo/fixtures/name-resolver-adversarial.fixture.json` (Step 3); the cases above document failure CLASSES — the fixture covers many more cases per class.
...
// BCP-47 fallback resolution lives as a Locale extension in DcsvIo.D2.Geo.Abstractions
...
1. **TS import path**: `@dcsv-io/d2-geo-default/countries` returns `CountryLite` (no embedded nav objects) vs `@dcsv-io/d2-geo-default/countries/full` returns `CountryFull extends CountryLite` (with embedded nav objects, all non-optional).
...
- `import { CountryLookup } from "@dcsv-io/d2-geo-default/countries"` — returns `CountryLite` (only ISO codes / FK code arrays; no embedded objects).
- `import { CountryLookup } from "@dcsv-io/d2-geo-default/countries/full"` — returns `CountryFull` (denormalized with all embedded navs populated and non-optional).
...
**What**: ship BOTH `DcsvIo.D2.Geo.Default`'s timezone catalog AND `DcsvIo.D2.Time`'s NodaTime tzdb wrapper. They serve different roles, both essential.

| Capability                                    | NodaTime tzdb (DcsvIo.D2.Time)            | Geo.Default timezone catalog         |
| --------------------------------------------- | ----------------------------------------- | ------------------------------------ |
...
**Bridge**: IANA identifier. Storage uses IANA id; Geo.Default provides UI catalog (selectors / display names / country grouping); DcsvIo.D2.Time provides runtime math (offset conversion / DST resolution / ambiguity handling).
...
- V2: KEEPS the catalog (in `DcsvIo.D2.Geo.Default`) as the UI / selector / display layer; ADDS NodaTime (in `DcsvIo.D2.Time`) as the runtime rules engine. Also adds: localized display names (11 langs), `Selectable` flag (filter niche zones from dropdowns), `Aliases` list (transparent old→new IANA id mapping).

---

### Decision 11 — Temporal model (DcsvIo.D2.Time — NEW Phase 1 lib, pulled forward from "deferred")

**What**: NEW `DcsvIo.D2.Time` lib in Phase 1. Wraps NodaTime + adds `IClock` + `SystemClock` + `TestClock` + EF Core value converter wiring (via Npgsql.NodaTime plugin) + custom types `ZonedInstant` + `LocalAnchoredEvent`.
...
- `IPostalCodeValidator` interface in Geo.Abstractions or `DcsvIo.D2.Location` (Planner recommends Location — postal validation is tied to address shape)
...
1. **Per-catalog sub-exports**: `@dcsv-io/d2-geo-default/countries`, `/subdivisions`, `/timezones`, `/currencies`, `/locales`, `/languages`, `/geopolitical-entities` — tree-shake-friendly entry points.
2. **Lite vs Full shapes** are determined by TS import path:
   - `@dcsv-io/d2-geo-default/{entity}` → returns `{Entity}Lite` (only ISO codes for nav refs, no embedded objects). Small browser bundle (~50-60% smaller).
   - `@dcsv-io/d2-geo-default/{entity}/full` → returns `{Entity}Full` (denormalized — embedded parent/child objects). Larger bundle.
...
**Deferred**: a `@dcsv-io/d2-geo-default-browser` slim variant — deferred unless browser bundle becomes a real problem in practice. The sub-exports + lite views should be enough.
...
- `@dcsv-io/d2-time` wraps **Temporal API** (Node 22+ native; polyfilled for older browsers via `temporal-polyfill` — ~30KB gzipped)
- `@dcsv-io/d2-geo-default` directly importable from SvelteKit SSR (`+page.server.ts` `load()` functions) — static data resolved at build time, NO gRPC calls for selector data
...
**Why direct SSR import of `@dcsv-io/d2-geo-default`**:
...
| DcsvIo.D2.Time exposing full NodaTime range                                                                                                                                                                    | Phase 1 wraps just the types we need; expose more as consumers demand                                 |
| TS `@dcsv-io/d2-geo-default-browser` slim variant                                                                                                                                                                      | Defer unless browser bundle becomes a real problem                                                    |
...
| **NEW subsection: Reference Data Philosophy**                                               | `docs/PATTERNS.md` (new subsection added in Step 6)               | Enshrines the user-stated principle: _"De-facto currencies are more important than politically-correct ones."_ Drives Country.Currencies M:M classification (LegalTender vs WidelyAccepted vs Tourist) — see Decision 6 / Country.Currencies. Same principle generalizes to other ref data: catalog reality, not aspirational reality.                                                                                                                                                                                                                                                                                                                                                                             |
...
### Decision 18 — Step ordering (for deliverable 0009-geo-libs)

8 steps, ordered for dependency satisfaction + early-feedback on the foundation libs. **Applies to 0009 only**. Deliverable 0008-geo-data-pipeline does NOT have prescriptive step ordering — it uses an iterative-spike approach (see 0008 PLAN at `docs/wip/0008-geo-data-pipeline/README.md`). 0009 cannot begin until 0008 ships + the go/no-go user-review gate passes.

| #   | Step                                                                                                                                                                                                                                                                                  | Notes                                                                                                                                               |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0   | Branch checkout `n/geo-libs` off clean `nova` (after 0008 ships)                                                                                                                                                                                                                      | Per the "Plans must include branch checkout as Step 0" feedback codified in memory. Base commit: clean `nova` head after 0008's squash-merge lands. |
| 1   | `DcsvIo.D2.Time` (NodaTime wrapper + IClock + SystemClock + TestClock + Npgsql.NodaTime EF config) + `@dcsv-io/d2-time` (Temporal wrapper) + parity tests                                                                                                                                     | Foundation; nothing else depends on it within the deliverable but Phases 4-5 will                                                                   |
| 2   | `DcsvIo.D2.Geo.Abstractions` (types + lookup interfaces + DeprecationInfo) + `@dcsv-io/d2-geo-abstractions`                                                                                                                                                                                   | Zero-dep foundation for Default + Location                                                                                                          |
| 3   | Consume spec files from 0008 (already at `contracts/geo/*.spec.json` — countries.spec.json, subdivisions.spec.json, currencies.spec.json, languages.spec.json, locales.spec.json, timezones.spec.json + Tier 2 specs) + build .NET SourceGen analyzer + TS emitter                    | Codegen infrastructure shared by Step 4; specs are INPUTS from 0008                                                                                 |
| 4   | `DcsvIo.D2.Geo.Default` (codegen'd lookups) + `@dcsv-io/d2-geo-default` + sub-exports for tree-shake                                                                                                                                                                                          | Consumes spec + codegen from Step 3                                                                                                                 |
| 5   | `DcsvIo.D2.Location` (3 VOs + composition function + IPostalCodeValidator interface + DefaultPostalCodeValidator) + `@dcsv-io/d2-location`                                                                                                                                                    | Depends on Abstractions only (not Default)                                                                                                          |
...
| 7   | Final-review (K=12 + Aggregator per process.md §3a-c) + Deliverable Completeness Checklist                                                                                                                                                                                            | Per CLAUDE.md MANDATORY block 3                                                                                                                     |
| 8   | SHIP → squash merge `n/geo-libs` → `nova`                                                                                                                                                                                                                                             | Per user direction + process.md §SHIP                                                                                                               |
...
**C# record** (`DcsvIo.D2.Geo.Abstractions/Country.cs`):
...
**TS interfaces** (Pattern B — separate Lite + Full per Decision 14; `@dcsv-io/d2-geo-abstractions/src/country.ts`):

```typescript
// LITE shape — from @dcsv-io/d2-geo-default/countries
...
// FULL shape — from @dcsv-io/d2-geo-default/countries/full
...
```json
{
  ...
  "currencies": [{ "iso4217AlphaCode": "USD", "level": "LegalTender" }],
...
- `import { CountryLookup } from "@dcsv-io/d2-geo-default/countries"` (lite) → `countryLookup.byAlpha2("US")` returns the record with FK codes populated (`primaryCurrencyISO4217AlphaCode = "USD"`, `geopoliticalEntityShortCodes = ["NA","NATO","UN",...]`); embedded nav props (`primaryCurrency`, `geopoliticalEntities`, etc.) stay undefined.
- `import { CountryLookup } from "@dcsv-io/d2-geo-default/countries/full"` (denormalized) → same API call returns the record with both FK codes AND embedded nav populated.
...
```typescript
// LITE — from @dcsv-io/d2-geo-default/subdivisions
...
// FULL — from @dcsv-io/d2-geo-default/subdivisions/full
...
```typescript
// LITE — from @dcsv-io/d2-geo-default/locales
...
// FULL — from @dcsv-io/d2-geo-default/locales/full
...
```typescript
// LITE — from @dcsv-io/d2-geo-default/timezones
...
// FULL — from @dcsv-io/d2-geo-default/timezones/full
...
```typescript
// LITE — from @dcsv-io/d2-geo-default/geopolitical-entities
...
// FULL — from @dcsv-io/d2-geo-default/geopolitical-entities/full
...
**C# record** (`DcsvIo.D2.Location/ValueObjects/Coordinates.cs`):
...
**TS interface** (`@dcsv-io/d2-location/src/coordinates.ts`):
...
**C# record** (`DcsvIo.D2.Location/ValueObjects/StreetAddress.cs`):
...
// @dcsv-io/d2-geo-default/countries → CountryLite (no embedded nav objects);
// @dcsv-io/d2-geo-default/countries/full → CountryFull (with all embedded navs populated non-optional).
```
...
```jsonc
{
  ...
  "geopoliticalEntities": [
    {
      ...
      "name": "North America",
      "type": "Continent",
      "countryISO31661Alpha2Codes": ["US", "CA", "MX", "..."],
    },
    {
      "shortCode": "NATO",
      "name": "North Atlantic Treaty Org.",
      "type": "MilitaryAlliance",
      "countryISO31661Alpha2Codes": ["US", "CA", "UK", "..."],
    },
    {
      "shortCode": "UN",
      "name": "United Nations",
      "type": "GovernanceAndCooperationAgreement",
      "countryISO31661Alpha2Codes": ["US", "CA", "..."],
    },
    {
      "shortCode": "G7",
      "name": "Group of Seven",
      "type": "GovernanceAndCooperationAgreement",
      "countryISO31661Alpha2Codes": ["US", "CA", "FR", "DE", "IT", "JP", "UK"],
    },
    {
      "shortCode": "G20",
    ...
    },
    {
      "shortCode": "FVEY",
      "name": "Five Eyes",
      "type": "SecurityCooperationAgreement",
      "countryISO31661Alpha2Codes": ["US", "CA", "UK", "AU", "NZ"],
    },
    {
      "shortCode": "QUAD",
      "name": "Quadrilateral Security Dialog",
      "type": "SecurityCooperationAgreement",
      "countryISO31661Alpha2Codes": ["US", "JP", "AU", "IN"],
    },
    {
      "shortCode": "CPTPP",
      "name": "Comprehensive & Progressive TPP",
      "type": "FreeTradeAgreement",
      "countryISO31661Alpha2Codes": ["..."],
    },
  ],
  ...
  "timezones": [
    ...
    },
    {
      ...
      "displayName": "Mountain Standard Time — Phoenix (no DST)",
      "currentStdOffsetMinutes": -420,
    ...
    },
    {
      ...
      "displayName": "Hawaii Time (no DST)",
...
### Example 2: lite return — TS `import { CountryLookup } from "@dcsv-io/d2-geo-default/countries"; countryLookup.byAlpha2("US")` (lite import; returns `CountryLite`)
...
**Memory cost** (paid once at lib import in `DcsvIo.D2.Geo.Default`):
...
import { CountryLookup } from "@dcsv-io/d2-geo-default/countries";
...
import { CountryLookup } from "@dcsv-io/d2-geo-default/countries/full";
...
import { CountryLookup } from "@dcsv-io/d2-geo-default/countries";
import { TimezoneLookup } from "@dcsv-io/d2-geo-default/timezones";
import { SubdivisionLookup } from "@dcsv-io/d2-geo-default/subdivisions";
...
import { GeopoliticalEntityLookup } from "@dcsv-io/d2-geo-default/geopolitical-entities";
// Bundle impact: ~3 KB gzipped (59 entities × ~600 bytes each, lite)

// All catalogs (composite — typical for an admin-tools page)
import * as Geo from "@dcsv-io/d2-geo-default";
...
| ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | -------------- |
| `@dcsv-io/d2-geo-default/countries` (lite)              | 250                                                                                                                                                        | ~40 KB      | ~12 KB         |
| `@dcsv-io/d2-geo-default/countries/full` (denormalized) | 250                                                                                                                                                        | ~120 KB     | ~25 KB         |
| `@dcsv-io/d2-geo-default/subdivisions` (lite)           | ~3,600                                                                                                                                                     | ~280 KB     | ~75 KB         |
| `@dcsv-io/d2-geo-default/timezones` (lite)              | ~600                                                                                                                                                       | ~50 KB      | ~12 KB         |
| `@dcsv-io/d2-geo-default/currencies` (lite)             | ~180                                                                                                                                                       | ~15 KB      | ~4 KB          |
| `@dcsv-io/d2-geo-default/locales` (lite)                | **~700 (full CLDR BCP-47 catalog; 18 selectable; rest non-selectable for Country.Locales M:M completeness + BCP-47 fallback + future expansion headroom)** | ~280-320 KB | **~75-100 KB** |
| `@dcsv-io/d2-geo-default/languages` (lite)              | **~180 (full ISO 639-1; 11 with `IsSupported=true`)**                                                                                                      | ~25 KB      | ~7 KB          |
| `@dcsv-io/d2-geo-default/geopolitical-entities` (lite)  | 59                                                                                                                                                         | ~10 KB      | ~3 KB          |
| **All** (composite via `@dcsv-io/d2-geo-default`)       | 5,150+                                                                                                                                                     | ~680 KB     | ~200 KB        |
...
**Subdivisions special case**: full ISO 3166-2 ships ~75 KB gzipped, the heaviest catalog. The narrowing helper `subdivisionLookup.ByCountry("US")` runs against the in-memory denormalized index — so a SSR `+page.server.ts` that needs just US states can `import { SubdivisionLookup } from "@dcsv-io/d2-geo-default/subdivisions"` and the bundle for the user receives only what the SSR `load()` returned (a 51-element array, ~3 KB serialized to the page payload). The ~75 KB never crosses the wire.
...
User's strategic framing (2026-05-17): _"it's reasonable / good to keep enemies of the US and neutral + high abuse countries out of our support range. frankly, i could see a scenario where we have to BLOCK india, russia, PR china, countries in the middle east / africa outright due to abuse / cyber attacks."_ This deferred-list reflects that strategy. See "Phase 3 carry-forward inventory" section below for the active Edge geo-security carry-forward (per-country block-list + rate-limit override + geographic risk scoring) that operationalizes this stance.
...
| §1.26 | Test Discipline                     | Inject `IClock` from `DcsvIo.D2.Time`; never call `DateTimeOffset.UtcNow` / `SystemClock.Instance.GetCurrentInstant()` directly. Evidence: `grep` returns zero direct calls. Why: tests can't deterministically control time without `IClock`; `TestClock` is the test seam.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
**What v2 does instead**: deploy-to-update model. Reference data is build-time-baked into `DcsvIo.D2.Geo.Default`. Any ref-data change requires a new lib release + consumer redeployment.
...
## DcsvIo.D2.Geo.Default catalog metadata (item 6b)
...
| DcsvIo.D2.Time exposing full NodaTime range (additional types)                                                 | Per-phase    | Consumer demands                                                                                                                                                     |
| `@dcsv-io/d2-geo-default-browser` slim variant                                                                         | Future       | Browser bundle becomes a real problem                                                                                                                                |
...
## Phase 2 (DcsvIo.D2.Contacts) — v1 carry-forward inventory
...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------- |
| `Contact` entity (11 fields: Id, CreatedAt, ContextKey, RelatedEntityId, IETFBCP47Tag, IANAIdentifier, ContactMethods, PersonalDetails, ProfessionalDetails, LocationHashId, Locale/Timezone navs) | `Geo.Domain/Entities/Contact.cs`                  | `DcsvIo.D2.Contacts/Entities/Contact.cs`                     | Carry forward all 11 fields + immutable-update pattern                    |
| `ContactMethods` VO                                                                                                                                                                                | `Geo.Domain/ValueObjects/ContactMethods.cs`       | `DcsvIo.D2.Contacts/ValueObjects/ContactMethods.cs`          | Carry forward                                                             |
| `EmailAddress` VO                                                                                                                                                                                  | `Geo.Domain/ValueObjects/EmailAddress.cs`         | `DcsvIo.D2.Contacts/ValueObjects/EmailAddress.cs`            | Carry forward                                                             |
| `PhoneNumber` VO                                                                                                                                                                                   | `Geo.Domain/ValueObjects/PhoneNumber.cs`          | `DcsvIo.D2.Contacts/ValueObjects/PhoneNumber.cs`             | Carry forward + E.164 validation                                          |
| `Personal` VO (9 fields incl. DateOfBirth + BiologicalSex)                                                                                                                                         | `Geo.Domain/ValueObjects/Personal.cs`             | `DcsvIo.D2.Contacts/ValueObjects/Personal.cs`                | Carry forward                                                             |
| `Professional` VO                                                                                                                                                                                  | `Geo.Domain/ValueObjects/Professional.cs`         | `DcsvIo.D2.Contacts/ValueObjects/Professional.cs`            | Carry forward                                                             |
| `BiologicalSex` enum (4 values)                                                                                                                                                                    | `Geo.Domain/Enums/BiologicalSex.cs`               | `DcsvIo.D2.Contacts/Enums/BiologicalSex.cs`                  | Carry forward                                                             |
| `GenerationalSuffix` enum (12 values)                                                                                                                                                              | `Geo.Domain/Enums/GenerationalSuffix.cs`          | `DcsvIo.D2.Contacts/Enums/GenerationalSuffix.cs`             | Carry forward                                                             |
| `NameTitle` enum (16 values)                                                                                                                                                                       | `Geo.Domain/Enums/NameTitle.cs`                   | `DcsvIo.D2.Contacts/Enums/NameTitle.cs`                      | Carry forward                                                             |
| `CreateContacts` handler                                                                                                                                                                           | `Geo.App/...`                                     | `DcsvIo.D2.Contacts/Handlers/C/CreateContacts.cs`            | Reimplement                                                               |
| `DeleteContacts` handler                                                                                                                                                                           | `Geo.App/...`                                     | `DcsvIo.D2.Contacts/Handlers/D/DeleteContacts.cs`            | Reimplement                                                               |
| `DeleteContactsByExtKeys` handler                                                                                                                                                                  | `Geo.App/...`                                     | `DcsvIo.D2.Contacts/Handlers/D/DeleteContactsByExtKeys.cs`   | Reimplement                                                               |
| `GetContactsByExtKeys` handler                                                                                                                                                                     | `Geo.App/...`                                     | `DcsvIo.D2.Contacts/Handlers/R/GetContactsByExtKeys.cs`      | Reimplement                                                               |
| `GetContactsByIds` handler                                                                                                                                                                         | `Geo.App/...`                                     | `DcsvIo.D2.Contacts/Handlers/R/GetContactsByIds.cs`          | Reimplement                                                               |
| `UpdateContactsByExtKeys` handler                                                                                                                                                                  | `Geo.App/...`                                     | `DcsvIo.D2.Contacts/Handlers/U/UpdateContactsByExtKeys.cs`   | Reimplement (immutable-update pattern)                                    |
| `ContactEviction` pub                                                                                                                                                                              | `Geo.App/...IPubs.ContactEviction`                | `DcsvIo.D2.Contacts/Pubs/ContactEviction.cs`                 | Reimplement via DcsvIo.D2.Messaging                                       |
| `ContactsEvicted` sub                                                                                                                                                                              | `Geo.Client/...ISubs.ContactsEvicted`             | `DcsvIo.D2.Contacts.Client/Subs/ContactsEvicted.cs`          | Reimplement for cache invalidation                                        |
| **`ApiKeyMappings` security control**                                                                                                                                                              | `Geo.App/GeoAppOptions.ApiKeyMappings`            | `DcsvIo.D2.Contacts/Options/ContactsOptions.cs`              | **CRITICAL — recreate or contact RPCs become wide-open**                  |
| `ContactExpirationDuration`                                                                                                                                                                        | `Geo.App/GeoAppOptions.ContactExpirationDuration` | `DcsvIo.D2.Contacts/Options/ContactsOptions.cs`              | Carry forward (default 4hr)                                               |
| `AllowedContextKeys` client-side allowlist                                                                                                                                                         | `Geo.Client/GeoClientOptions.AllowedContextKeys`  | `DcsvIo.D2.Contacts.Client/Options/ContactsClientOptions.cs` | Carry forward — defense-in-depth                                          |
| `ContactToCreateValidator`                                                                                                                                                                         | `Geo.Client/...`                                  | `DcsvIo.D2.Contacts/Validators/ContactToCreateValidator.cs`  | Carry forward                                                             |
| `CleanupOrphanedLocations` job + `JobLockTtlSeconds`                                                                                                                                               | `Geo.App/...ICommands.CleanupOrphanedLocations`   | `DcsvIo.D2.Contacts/Jobs/CleanupOrphanedLocations.cs`        | Carry forward — Contact ownership of orphan-creation justifies relocation |
| `IPhoneValidator` interface + Default impl                                                                                                                                                         | (Phase 2 per Decision 13)                         | `DcsvIo.D2.Contacts/Validators/IPhoneNumberValidator.cs`     | Add per Decision 13 pattern                                               |
| `IEmailValidator` interface + Default impl                                                                                                                                                         | (Phase 2 per Decision 13)                         | `DcsvIo.D2.Contacts/Validators/IEmailValidator.cs`           | Add per Decision 13 pattern                                               |
...
## Final attestation block (template — filled at deliverable 0009's Step 8 SHIP)
...
(Deliverable 0008-geo-data-pipeline carries its own attestation in its workspace README at its SHIP gate; 0009's attestation below is the Phase-1-completing attestation.)
...
**Awaiting user REVIEW + SHIP authorization (for deliverable 0009-geo-libs).** SHIP gate steps (per workflow.md §SHIP): commit the 0009 SHIP work + apply approved rules.md predicate additions + present this doc to user + copy snapshot to `docs/dev/deliverables/0009-geo-libs.md` + squash-merge `n/geo-libs` → `nova` + update V2.md §4 Phase 1 row to ✅ + archive `docs/v2/PHASE_1.md` → `docs/archive/PHASE_1_GEO_LIBS.md` when Phase 2 begins (not at this SHIP). Note: deliverable 0008-geo-data-pipeline ships separately ahead of 0009 (snapshot to `docs/dev/deliverables/0008-geo-data-pipeline.md` + squash-merge `n/geo-data-pipeline` → `nova` + user go/no-go gate).

---

## Archive trigger

Archive `docs/v2/PHASE_1.md` → `docs/archive/PHASE_1_GEO_LIBS.md` when Phase 2 (Contacts) begins, per the V2.md §10 lifecycle rule + the precedent set by `docs/archive/PHASE_0_WIPE.md`. The archived snapshot stays as a frozen reference; the per-step deliverable workspaces at `docs/wip/0008-geo-data-pipeline/` and `docs/wip/0009-geo-libs/` are left in-place for user to remove manually whenever they want.
