...
**TypeScript twin (PHASE_3 T1 / deliverable 0028):** full layout + behavioral mirror under `server/shared/typescript/caching/` — `@d2/caching-abstractions` · `@d2/caching-local-default` · `@d2/caching-distributed-redis` · `@d2/caching-tiered`. Shared invalidation channel default `d2:cache:invalidations` (everyone acts). Cross-language parity: [PARITY.md](PARITY.md) (behavioral runtime twin, same class as messaging/encryption).
