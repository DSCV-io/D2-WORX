...
- Every inbound sync hop re-validates the bearer JWT (RS256 signature against the **cached** JWKS key + claims + a session-liveness check) rather than trusting a pre-authenticated identity asserted by the transport. That per-hop CPU + liveness lookup is overhead a **transport-trust** model would avoid — internal services mutually authenticated by mTLS, passing a plaintext identity header that downstream accepts without re-validating the token. We deliberately don't do that: it moves the trust anchor from the token (bound to the issuer's signature at every hop) to the network, so one compromised or misconfigured internal service could forge any identity. The threat model accepts the (cache-warm) re-validation cost to keep every hop zero-trust; the mTLS fast path is not implemented. (Note: identity-via-JWT and the operational subset are both still *propagated* — what we decline is *trusting* a propagated identity without re-verifying its token.)
