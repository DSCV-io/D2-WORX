...
- The `factoryShape` enum must cover every existing `D2Result` factory signature; introducing a genuinely new signature is a canonical-schema change plus a generator change, not a spec-only edit.
