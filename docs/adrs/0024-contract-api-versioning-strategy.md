...
The committed protos were renumbered to `d2.keycustodian.v2alpha` — the proto package, the emitted `.NET` package (`D2.KeyCustodian.V2Alpha.*`), and the TS package (`@d2/keycustodian-v2alpha`) all carry the new name. The renumber ran through the normal codegen pipeline: the emitter derived the new package and C# namespace from the renumbered TypeSpec source, the emitted artifacts regenerated, and downstream references updated in the same change.
