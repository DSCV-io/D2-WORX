...
- **`aud=d2.internal`** — a single, broad internal audience that every internal service accepts. This value is declared once in the `auth-audiences` contract and surfaced as a named constant — never a raw literal scattered through the code — so changing it is a one-line contract edit that the mint side and every hop's audience check read from the same source.
