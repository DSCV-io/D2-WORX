...
The forwarded token's `aud` is `WellKnownAudiences.D2_INTERNAL_AUDIENCE` (value `"d2.internal"`) — a hand-declared constant in `D2.Shared.Auth.Abstractions`, the universal *receive* audience every hop validates. It is deliberately **not** an entry in `audiences.spec.json` (whose entries are token-exchange *targets*), so it is a protocol constant, not a spec-mirror DTO.
...
- **Operational-subset propagation on synchronous .NET-to-.NET hops is not yet wired.** The synchronous .NET transport middleware currently builds context from JWT claims only and does not yet read or forward the propagated operational subset on those hops (the identity half is already correct; the operational subset rides on the asynchronous path and the BFF transport today). Carrying the operational subset — including the new call-path field — across synchronous .NET-to-.NET hops is new plumbing and a follow-up for a later deliverable.
