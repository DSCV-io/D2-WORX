...
- `server/shared/dotnet/caching/abstractions/` — `ILocalCache.cs`, `IDistributedCache.cs`, `ITieredCache.cs`, the four building-block interfaces, `ICacheInvalidationBackplane.cs`, `InputFailures.cs`, `LocalCacheOptions.cs`, and the README (result-mapping table + "everyone acts" rationale).
...
- TypeScript twin cluster: `server/shared/typescript/caching/` — `@d2/caching-abstractions`, `@d2/caching-local-default`, `@d2/caching-distributed-redis`, `@d2/caching-tiered` (behavioral runtime twin of this ADR; package READMEs under each folder).
