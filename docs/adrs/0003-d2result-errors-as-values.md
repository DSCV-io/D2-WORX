...
**`ErrorCodes`** is codegen-emitted from `contracts/error-codes/error-codes.spec.json` (each code with a declared HTTP status). The same spec drives the TypeScript side via `tools/ts-codegen`. Hand-written constants are forbidden; cross-language drift is structurally impossible.
...
- `server/shared/dotnet/result/core/` — `D2Result.cs` + partials (`*.Factories.cs`, `*.Generic.Factories.cs`, `*.Generic.Monadic.cs`, `*.Booleans.cs`, `*.Combine.cs`, `D2ResultGuardExtensions.cs`, `InputError.cs`), and the committed `Generated/` `ErrorCodes.g.cs` + `D2ResultEnvelopeFieldNames.g.cs`.
- `server/shared/dotnet/result/core/README.md` — full factory table, bubble propagation, partial-success ladder, monadic API.
...
- `server/shared/typescript/result/src/` — `d2-result.ts`, `factories.ts`, `bubble.ts`, `tk-message.ts` (the TypeScript mirror).
