...
- [ ] Custom JWT claims namespaced with `d2:` prefix per V2.md §5.4 convention
...
- [ ] C# naming: `r_camelCase` (readonly), `_camelCase` (mutable), `sr_camelCase` (static readonly), `_UPPER_CASE` (private constants), `UPPER_CASE` (public constants). Primary-constructor handler params exempt per V2.md §6.1
...
- [ ] Multi-replica migration safety — startup migrator acquires PG advisory lock per V2.md §5.6 (only one replica migrates; others wait)
...
- [ ] Key rotation integration test passes (per V2.md §5.4 KeyCustodian) — required CI gate

## Documentation

- [ ] Every new handler / service / endpoint reflected in its `README.md`
- [ ] V2.md phasing table accurate
