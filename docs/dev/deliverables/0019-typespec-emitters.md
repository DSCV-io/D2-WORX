...
Also: add `@d2InProcess` (a 13th decorator) to the shipped `@d2/typespec-decorators` package.
...
- 0018 (`@d2/typespec-decorators`, shipped) = the `@d2*` vocabulary you annotate a `.tsp` with. **0019 = the generators that read those annotations and produce code.** 0019 depends on 0018.
- Emit mechanism is proven by the A/C/B/SC1 spikes: `$onEmit` + `navigateProgram` + `program.stateMap(KEY).get(op)` + string templates + `emitFile`. The spikes used stale stand-ins (`Outcome<T>`, `IAsyncResiliencePolicy`, `@d2Scope`); 0019 ports the **mechanism**, mapping to real shapes (`D2Result`, `D2.Shared.Resilience`, the shipped scope decorators). Spikes B + C already validated their emitters with ~9 tests each against injected seams (no host) — that's the precedent: validate against seams, not consumers.
...
| D2 | Leaf-vs-gRPC trigger = an explicit **`@d2InProcess`** decorator (13th), added to `@d2/typespec-decorators`. | user |
| D3 | Package home = `server/shared/typescript/typespec-emitters/` (pnpm workspace member; imports `@d2/typespec-decorators`). | user |
...
| D14 | **Façade interface signature is transport-neutral** — `ValueTask<D2Result<<Op>Output?>> <Op>Async(<Op>Input input, CancellationToken ct = default)`, NO `HandlerOptions?`. Required so the SAME `I<Module>InternalApi` backs both the in-process impl and a future gRPC-client impl (location transparency); `HandlerOptions` is a non-wire server concern. ⇒ a module's `Clients` project references `D2.Shared.Result` + `D2.Shared.Utilities` (the latter forced by the service-tree Tier-1 global-using injection in `server/services/Directory.Build.targets`; both are shared libs, ADR-0020-legal) — never handler-abstractions / Domain / App / Infra. | orchestrator (Step 6; D-b amended in 6a) |
...
7. **resilience pipeline emitter** — `ResilientPipeline<TKey,TValue>` + builder DSL + keyed DI from the `@d2Resilience` AST. *Seams:* real `D2.Shared.Resilience` + flaky fake outbound; retry/breaker/singleflight behavior.
...
- **Step 1 — `@d2InProcess` decorator.** Amend `@d2/typespec-decorators`: `extern dec` + state key + validator (proposed rule: requires `@d2ServedBy`; independent of `@d2GrpcMethod`) + tests + README. Re-run the package's full 100%-coverage gate; must not regress the existing 12 decorators.
- **Step 2 — Emitter package scaffold.** `server/shared/typescript/typespec-emitters/` workspace member: package.json (peer `@typespec/compiler`, dep `@d2/typespec-decorators`), tsconfig, vitest, the `$onEmit` entry + shared lib (scalar registry, name transforms, banner reuse, `emitFile` wrapper, a new `D2TSP*` diagnostics family). Smoke-emit only. **Spike the `TestServer`/in-memory-gRPC harness pattern here** (de-risks Steps 4/5/10).
...
- **Step 8 — resilience pipeline emitter.** Real `D2.Shared.Resilience` + flaky outbound; retry/breaker/singleflight. + integration test → ledger row.
...
- **R3 — stale spike code.** Port the *mechanism*, not the stand-ins. Map to real `D2Result` / `D2.Shared.Resilience` / shipped scope decorators. (rules.md §26.11 sweep.)
...
4. **gRPC-client validation (Step 9b finish).** The emitter source (`grpc-client-emitter.ts`) is
   ...
   `D2.Edge.Tests/TypeSpecGrpc/Generated/`, `GrpcClientTests.cs`, byte-gate constants in
...
|---|---|---|---|---|---|
| **C-1** | **Temporal scalars (DTO-emitter)** | Extend the scalar registry + `walkModel` with the full lossless temporal table (per temporal-mapping.md); emit the two zone-bearing composite DTOs; C# + TS round-trip fixtures + DST-adversarial + cross-language parity | — (DTO-emitter foundational) | Real `D2.Shared.Time` (`ZonedInstant`/`LocalAnchoredEvent` `Create` round-trip) + `@d2/time` + the shared `contracts/temporal/*.fixture.json` | New VALIDATION.md temporal rows; closes C5; resolves §E temporal-mapping decision (record the D-T choices) |
...
| **C-4** | **gRPC-client validation harness (Step 9b finish)** | Committed `…/TypeSpecGrpc/Generated/` client fixtures + `GrpcClientTests.cs` + byte-gate constants + VALIDATION.md gRPC-client row + `Clients.csproj` MUT-1; confirm the captured-envelope body (transport-fault retry; business `ValidationFailed` NOT retried, full fidelity) | C-3 if the fixture op carries a predicate; else independent (emitter source already built) | Real in-memory gRPC `TestServer` harness (already in `D2.Edge.Tests`) + real `D2.Shared.Result.Grpc` envelope mapper + real `D2.Shared.Resilience` keyed pipeline + fault-injecting service shim | Closes C1a; the gRPC-client ledger row + replace-triggers CB5/CB6 (channel address + one-time outbound auth) recorded |
| **C-5** | **`@d2Resilience` predicate EMISSION + gRPC-client consumes it** | Emit the C# + TS predicates from the AST; thread the C# predicate into the generated client's retry decision (sentinel + extended `IsTransient` per §5); byte-gate the predicate output; over-the-wire predicate test deferred to C-8 | C-3 (the decorator) + C-4 (the client it folds into) | Real `D2.Shared.Resilience` (the keyed pipeline + sentinel retry path) + the in-memory harness; cross-runtime C#↔TS predicate-behavior parity test | New VALIDATION.md predicate rows; closes the emitter half of C11 |
| **C-6** | **TS client emitters — browser REST + server gRPC (Step 9c)** | New `ts-client-emitter.ts` (or split) emitting per-op typed fns for BOTH surfaces per D18; dispatch wired in `emitter.ts`; vitest tests; VALIDATION.md rows | C-1 + C-2 (so emitted TS DTOs cover temporal/enum) + C-3/C-5 (TS predicate parity, if the op carries one) | `@d2/grpc-client` (`handleGrpcCall`/`unaryCall`/`isTransientGrpcError`/`d2ResultFromProto`) for server; `apiCall`/`executeFetch`/`gateway-response` for browser (see PREREQ RISK on the BFF seam, below) | Closes C10; replace-triggers CB7 (SSR channel) + CB8 (browser fetch substrate) recorded |
...
- **`I<Op>Handler` is module-internal** (always generated); the **façade is the only externally-visible surface** (in the thin `D2.Edge.<Module>.Clients` project siblings reference).
...
- **Step 6 — Exposed-op contract layer**: scaffold `D2.Edge.KeyCustodian.Clients`; DTO emitter places exposed DTOs there; generate `I<Op>Handler` (app) + façade interface (clients) + façade impl (app); the GetJwks overwrite. Gate: KC builds + tests pass.
...
- **Step's code change has corresponding test coverage?**
  YES — TS vitest 952 / 43 files / 100% Stmts/Branch/Funcs/Lines (src/**); .NET D2.Edge.Tests
...
- **Test suite passes at the most recent test run citation?**
  YES — `dotnet test D2.Edge.Tests` SOLO → 934 passed / 0 failed / 6 skipped (the 6 =
