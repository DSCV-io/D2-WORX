...
- .NET `D2.Shared.I18n`: `TK.Common.Errors.NOT_FOUND` is a `TKMessage`; raw key via `.Key`. Import `using D2.Shared.I18n;`.
- TS `@d2/i18n/keys`: `TK.common.errors.NOT_FOUND` value **is** the string. Import `import { TK } from "@d2/i18n/keys";`.
...
| --- | --- | --- | --- |
| 01 | .NET test project | 10 files: Validation/Location/HandlerRepo/Result tests | `D2.Shared.Tests` — build 0W + inspectcode 0W + dotnet test |
| 02 | TS shared packages | validation/default (3 tests) + `@d2/time` { dep add + `types.ts` prod (2) + `types.test.ts` (10) + README + `pnpm install` } — **15 conversions** (D6/Option 3) | type-check:test + vitest + eslint/prettier |
...
| .NET form | `.Be("x_y_Z")` → `.Be(TK.X.Y.Z.Key)`; match `using D2.Shared.I18n;` import style of files already using `TK.*`. |
| TS form | `.toBe("x_y_Z")` → `.toBe(TK.x.y.Z)`; add `import { TK } from "@d2/i18n/keys";`. |
