...
1. [Test Discipline](#1-test-discipline) — incl. §1.25 (aggregate counts ≠ test executed), §1.26 (live-DB integration tests for EF-mapping / anonymization)
...
9. [Architectural Layer Hygiene](#9-architectural-layer-hygiene) — incl. §9.35 (parallel persistence-write converter symmetry)
...
26. [Codegen Discipline (spec / proto / schema-derived types)](#26-codegen-discipline-spec--proto--schema-derived-types)
...
24. [Audit Evidence Discipline (meta — how to audit)](#24-audit-evidence-discipline-meta--how-to-audit)
    ...
    - [Predicates §24.0 – §24.24 (incl. §24.0h K=1 carve-out, §24.0i Sonnet-for-Auditors, §24.16 Plan-Audit-required, §24.19 uncommitted-deliverable working-tree, §24.20 tool-invisible convention lens, §24.21 full-solution gate-verify, §24.22 source-xmldoc step-ref scan, §24.23 Fixer no-workspace-IDs in own additions, §24.24 Auditor scope = git-verified authored surface)](#predicates--24-audit-evidence-discipline)
...
- **K=12 + Aggregator** — the canonical audit-round dispatch: 12 parallel cluster Auditors (each scoped to one §-range cluster, per process.md §3) + 1 Aggregator sub-agent (Opus per the [Sub-agent model policy per role](process.md#sub-agent-model-policy-per-role) canonical table) that merges the 12 partial outputs into the canonical journal big table. See §24.0h + §24.0i.
...
### Global usings (the two-tier-plus-safety-set policy)

- **5.26** Does the global-usings set follow the two-tier-plus-safety-set policy — and is the never-globalize safety set kept EXPLICIT so the dependency-direction law stays compiler-enforced? (Canonical: [ADR-0020](../adrs/0020-service-project-structure.md) "the global-usings policy".)
  - **Tier-1 — repo-wide** (central `<Using>` items in `server/Directory.Build.props`, **scoped to RUNTIME projects only**): `D2.Shared.Result` · `D2.Shared.Utilities.Extensions` + `.Attributes` + `.Enums` · `D2.Shared.I18n`. The `netstandard2.0` source-gen projects **opt out** — a `global using X;` for an assembly the project does not reference is a hard CS0246, so the Tier-1 items must NOT apply to projects outside the D2 runtime stack.
  - **Tier-2 — per layer** (`GlobalUsings.cs` per project): **domain** → `NodaTime` + `D2.Shared.Time` + the service's own `Entities`/`ValueObjects`/`Enums`/`Errors`; **app** → `D2.Shared.Handler` + `D2.Shared.Context.Abstractions`; **tests** → `Xunit` + `AwesomeAssertions`; **source-gen** → `Microsoft.CodeAnalysis` + `System.Collections.Immutable` + the polyfills. (`NodaTime` is Tier-2 not Tier-1 because it is the domain's #1 import but absent from pure-DTO projects — central globalization would force a NodaTime package ref into projects that never use it.)
  - **Never globalize (the safety set)** — these stay EXPLICIT in every file that uses them: `Microsoft.EntityFrameworkCore*`, `Microsoft.Extensions.DependencyInjection*`, `Microsoft.Extensions.Options*`, `Microsoft.Extensions.Logging*` / `Serilog*`, `Microsoft.AspNetCore.*`, `Microsoft.Extensions.Hosting`, `System.Security.Cryptography`, any vendor SDK.
  - **Why**: a global `using` for one of the safety-set namespaces would BLIND the compiler-enforced dependency-direction law (§9.36) — a domain file could silently take an EF / Options / DI / logging dependency with no visible signal. Keeping them explicit means a file's usings still reveal what it is coupled to. Globalizing only the universal-and-signal-free namespaces (`D2Result`, the utilities surface, `TK`) removes the most-repeated usings with zero lost signal.
  ...
  - Evidence: per service/runtime project → Tier-1 items present in central props (runtime-scoped); per layer → the Tier-2 `GlobalUsings.cs` matches the layer; grep the central props + every `GlobalUsings.cs` for the safety-set namespaces → expect zero (a safety-set namespace found globalized = FINDING). Cross-ref §9.36 (dependency-direction law).
...
- **9.31** For every domain entity with a `Status` field (or equivalent state-discriminator field) that admits multiple values, is there an EXPLICIT valid-transitions table (enum + transition predicate, OR a state-machine declaration) that the test suite asserts against?
  - **Scope**: any domain record / entity / aggregate that carries a `Status` / `State` / `Phase` discriminator field whose value affects downstream behavior (e.g. `OrderStatus = Pending | Confirmed | Shipped | Delivered | Canceled`; `NotificationDeliveryStatus = Queued | Dispatched | Delivered | Failed`).
  - **Required**: an explicit valid-transitions structure declared adjacent to the status enum — either (a) a transition predicate method `IsValidTransition(from, to) → bool`, (b) a typed transitions table (e.g. `static readonly Dictionary<(Status from, Status to), bool> Transitions`), OR (c) a state-machine declaration (Stateless, custom DSL). The test suite asserts every legal transition + asserts illegal transitions are rejected.
  - **Forbidden**: a `Status` field whose transition rules live ONLY in handler if-statements scattered across the codebase. That's implicit-state-machine drift — each new handler can introduce a transition the original author didn't anticipate, and no test enforces correctness.
  - **Evidence**: per domain entity with a `Status` field → (a) transitions structure cited at `file.cs:NN` + (b) test suite citation at `file.cs:NN` asserting legal + illegal transitions.
  - **Why**: silent state-machine drift is a recurring class of correctness bug — an entity gets stuck in an invalid state (e.g. `Status = Delivered` AND `Status = Failed` set on the same record by two parallel handlers; a `Refunded` order being re-`Shipped`). Explicit transitions caught at compile-time / test-time eliminate the class. Origin: prior audit-checklist surface ("Status state machine adherence").
  - **How**: when adding or modifying a domain entity with a `Status` field, declare the transitions structure adjacent to the enum + ship the test asserting against it in the same change.
...
- **9.36 (the Domain ← App ← Infra ← Api dependency-direction law is preserved on every edge)** For every project reference and every `using` introduced in a service project, does it respect the one-direction dependency law — domain depends on nothing but shared primitives; app depends on domain (+ abstractions + EF *types*) but never a concrete adapter; infra depends on app + domain (+ vendor SDKs); api depends on app + infra; **api is the ONLY project that may reference infra**?
  ...
  - **Why**: the dependency law is the invariant behind every structure rule ([ADR-0020](../adrs/0020-service-project-structure.md)). The compiler enforces it ONLY if the never-globalize safety set (§5.26) keeps EF/Options/DI/logging usings explicit — a global `using` for one of them would let a domain file silently take an EF or Options dependency with no visible signal. Vendor churn touching one layer, a transport-reusable orchestration layer, and an infrastructure-free testable domain ALL collapse if an edge points the wrong way.
  - **How**: when adding a `<ProjectReference>` or a `using` in a service project, check it against the law; the tie-breaker for an ambiguous placement is "which layer still compiles if I delete the layer below the candidate?". Evidence: per service project → enumerate `<ProjectReference>` edges + grep `using` for the forbidden namespaces in domain/app (`grep -rEn 'using (Microsoft\.EntityFrameworkCore|Microsoft\.Extensions\.(Options|DependencyInjection|Logging)|Serilog)' <domain/app scope>` → domain expects zero; app expects EF only via the port, zero Options/DI/logging in handler bodies beyond what `BaseHandler` provides) + confirm no project but api references infra. Cross-ref §9.8 / §11.29 (dep-graph parity), §5.26 (never-globalize safety set).
...
- **24.0i (Sub-agent model policy per role)** Is every sub-agent dispatch's model selection consistent with the canonical role-to-model policy table (Sonnet-default roles: Auditor / Plan-Auditor / Final-reviewer / Implementer / Fixer / Investigator; Opus-default roles: Orchestrator / Planner / Plan-amender / Aggregator)? Self-invoked Opus dispatch for a Sonnet-default role (any reasoning pattern: "this scope is too complex for Sonnet" / "I want extra rigor on this round" / "the Implementer's work is bigger than expected") is a §24.0i violation unless the dispatch meets the documented Sweeping carve-out criteria below OR the user has explicitly approved per-occurrence.
  - **Evidence**: per `Agent` invocation in the orchestrator log → the invocation's `model` selector matches the canonical table below. Sonnet-default roles carry an explicit `model: "sonnet"` override (or equivalent runtime selector). Opus-default roles inherit the parent's default model OR carry an explicit `model: "opus"` override; no `1m`-suffix variant is required (Opus 4.x defaults to 1M context on Max plans — the suffix is redundant + risks rotting). Absence of the required selector on any dispatch is a §24.0i violation; the round MUST be re-run with the correct dispatch model. The sub-agent's return self-attestation (per the Self-documentation requirement below) provides the orthogonal evidence channel.
  - **Why** (role-by-role rationale):
    - **Orchestrator (Opus)** — main-thread decision-making + delegation + trust-but-verify discipline (per [process.md §4 Orchestrator verification of Sonnet sub-agent outputs](process.md#orchestrator-verification-of-sonnet-sub-agent-outputs)). Cannot outsource the spot-check / re-run / adversarial-challenge discipline that catches Sonnet-side hallucinations or short-circuits. Sonnet on the orchestrator would collapse the verification asymmetry the policy depends on.
    - **Planner (Opus)** — the Plan section is the canonical contract every downstream sub-agent reads. Plan quality is the single highest-leverage low-volume artifact in the framework: a missed gate-check in the Plan cascades into Implementer + Auditor + Fixer cycles to catch it post-hoc. Opus-class reasoning at Plan-authoring time prevents the cascade.
    - **Plan-amender (Opus)** — writes to the same canonical Plan artifact the Planner authored; same high-leverage low-volume reasoning. Mid-execution amendments must be coherent with the original Plan's framing, locked decisions, and downstream dispatch briefs; Sonnet here risks introducing internal inconsistency the next Implementer builds against.
    - **Aggregator (Opus)** — merges 12 cluster Auditor partials + performs cross-cluster sister-sweep + cross-cutting verification. The merge alone exceeds smaller-context budgets (12 partials × full big-table chunk + findings list each); Opus 4.x defaults to 1M context on Max plans which provides sufficient budget. Cross-cluster reasoning across 12 thematic perspectives benefits from Opus-class capability.
    - **Auditor / Plan-Auditor / Final-reviewer (Sonnet)** — pattern-matching against `rules.md` predicates + grep verification + file:line citations. This is Sonnet-shaped work — bounded predicate scope per cluster, structured output format (the partial-file template), no synthesis required (the Aggregator owns synthesis). Opus is over-specified; the orchestrator's Opus context budget is better spent on synthesis (Aggregator) and high-leverage planning (Planner / Plan-amender). K=12 Auditor dispatch is the highest-volume sub-agent invocation pattern in the framework (12 parallel × multiple rounds × multiple steps per deliverable); Opus here blocks Opus availability for the roles where capability actually matters.
    - **Implementer / Fixer (Sonnet)** — bounded code/test authorship per a specific brief (Implementer) OR mechanical application of pre-specified fix scope (Fixer). The Plan / Aggregator / orchestrator have already done the hard reasoning; the Implementer / Fixer executes against a tight contract. Sonnet handles this well. **Sweeping carve-out applies** (see below) for the unusual cases where the brief's scope is too large for Sonnet to hold coherently.
    - **Investigator / Research (Sonnet)** — bounded-scope investigation tasks returning structured reports (file paths, grep counts, line citations, summaries). Sonnet's bread and butter — focused queries with concrete deliverables. The orchestrator consumes the report and routes; no cross-cutting synthesis needed in the Investigator itself.
    - **Cost-quality trade-off**: Opus is materially more expensive per token + scarcer in the user's weekly budget. The policy's value lives in spending Opus where its capability actually moves outcomes (synthesis + high-leverage planning + trust-but-verify) and using Sonnet where its capability already saturates the task (pattern-matching + bounded execution + structured reporting). Misallocating Opus to Sonnet-shaped work doesn't just waste budget — it depletes Opus availability for the roles where Sonnet would visibly under-perform.
    - **Verification discipline shift**: when the orchestrator dispatches to Sonnet sub-agents, it takes on additional verification responsibility (per [process.md §4 Orchestrator verification of Sonnet sub-agent outputs](process.md#orchestrator-verification-of-sonnet-sub-agent-outputs)) — spot-sample partial evidence, re-run gate samples, adversarially challenge "all green" reports, re-read changed files for high-stakes work. The trust-but-verify discipline IS the structural compensation for the cost-quality trade-off; without it, Sonnet dispatch risks shipping unverified outputs.
  - **How**: every `Agent` tool invocation's parameters match the canonical table:
    - Sonnet-default roles (Auditor / Plan-Auditor / Final-reviewer / Implementer / Fixer / Investigator): include `model: "sonnet"` (literal string, lowercase) — or the runtime-equivalent Sonnet selector.
    - Opus-default roles (Planner / Plan-amender / Aggregator): include `model: "opus"` (literal string, lowercase) — or inherit the parent orchestrator's default if the parent is already Opus. No `1m`-suffix variant is mandated; current Opus models default to 1M context on Max plans which the Aggregator's 12-partial merge requires.
    - **Sweeping carve-out** (Implementer / Fixer Opus escalation) — see sub-block below.
    - **Self-documentation requirement** (every return self-attestation) — see sub-block below.
  - **When**: applies to every sub-agent dispatch in every phase (PLAN / EXECUTE / FINAL-REVIEW), every audit round (per-step AND final-review), every Plan-Audit round, every Fixer dispatch. There is no scope-based exception. K=1 Auditor dispatches (when user-approved per §24.0h) ALSO carry `model: "sonnet"` by default — the model selection is independent of K.
  - **Combined K=1 + Opus authorization** (Auditor-only): when the user wants BOTH K=1 AND Opus on the same Auditor round, the bypass authorization message MUST name BOTH explicitly (e.g., `K=1 + Opus approved for this round` or equivalent). A bare `K=1 approved` authorizes K=1 with Sonnet (the default); a bare `Opus approved for Auditors` is not a valid combined authorization (the §24.0h K count must also be explicit). Per §13.14 the user names the SPECIFIC predicates / parameters being bypassed; the orchestrator does not infer the combined form from partial approval.
  - **Boundary vs §24.0h**: §24.0h governs the K count (parallelism); §24.0i governs the model selector (capability per role). The two are independent dispatch-shape gates that compose explicitly — a K=12 Auditor batch with `model: "sonnet"` per Auditor satisfies both; a K=1 Auditor with `model: "opus"` requires per-occurrence approval of BOTH bypasses.
  - **Boundary vs Sweeping carve-out**: the carve-out below is the ONE codified Sonnet-default → Opus escalation that does NOT require per-occurrence user approval. Any Sonnet-default role escalation that does NOT meet the carve-out criteria DOES require explicit per-occurrence user approval per §13.14.
  - Cross-ref §24.0h (K=1 discipline), [process.md §3 Sub-agent model policy per role](process.md#sub-agent-model-policy-per-role) (canonical table — single source of truth for the role-to-model mapping), [process.md §4 Orchestrator verification of Sonnet sub-agent outputs](process.md#orchestrator-verification-of-sonnet-sub-agent-outputs) (trust-but-verify discipline that compensates for Sonnet dispatch).

  **Sweeping carve-out** (Implementer / Fixer Opus escalation — codified bypass requiring no per-occurrence user approval):

  An Implementer or Fixer dispatch qualifies for Opus escalation when it meets ≥1 of the following criteria. The orchestrator's dispatch brief MUST cite the triggering criterion + a brief justification; the sub-agent's return self-attestation MUST echo the same citation (per the Self-documentation requirement below).
  ...
  **Citation requirement** — every Opus dispatch under this carve-out includes in the orchestrator's dispatch brief (visible to user) the exact form:

  > `Opus carve-out criterion {1|2|3|4}: <brief justification — e.g., "atomic 67-file rename across .NET + TS spans criterion 1 + criterion 3">`

  And the sub-agent's return self-attestation (per the Self-documentation requirement below) MUST echo the same citation verbatim. The dual-citation gives retroactive auditability — after N deliverables, a sweep can identify which Opus dispatches under the carve-out could've been Sonnet (the justification was weak) and which Sonnet dispatches needed re-do (the justification was missing). The self-learn loop tunes the carve-out criteria over time.

  **Out-of-scope for the carve-out** — Auditor / Plan-Auditor / Final-reviewer / Investigator escalations to Opus require explicit per-occurrence user approval per §13.14. The carve-out is Implementer / Fixer ONLY because those two roles are the ones where bounded-brief execution can occasionally exceed Sonnet's bounded-context comfort zone; Auditor-shaped work (predicate walking + grep verification + structured output) does NOT exceed Sonnet's comfort zone at any K count or scope, so no automatic carve-out is justified there.
  ...
  **Self-documentation requirement** (every sub-agent return summary):
  ...
  - **Evidence**: every sub-agent return in the orchestrator's dispatch log opens with the two-line attestation block. Sonnet-default-role returns that include only `Model: <sonnet-id>` (no carve-out reason) are valid. Opus-default-role returns that include only `Model: <opus-id>` (no carve-out reason) are valid. Sonnet-default-role returns that dispatched to Opus MUST include both lines — the carve-out reason is REQUIRED, not optional. Absence of the attestation block is a §24.0i compliance failure; the orchestrator MUST re-prompt the sub-agent for the attestation before treating the return as valid.
  - **Orchestrator dispatch-log requirement**: the orchestrator's per-step journal MUST record per dispatch — (a) the dispatched model, (b) the role being filled, (c) if Opus dispatched from a Sonnet-default role: the carve-out criterion invoked + the verbatim justification. This gives retroactive auditability for self-learn loops: after N deliverables, the orchestrator (or a Final-reviewer) can sweep dispatch logs and identify which Sonnet dispatches needed re-do (the role-to-task fit was wrong) vs which Opus dispatches could've been Sonnet (the carve-out justification was weak). The self-learn loop tunes the policy over time.
  - **Why dual-channel attestation (dispatch brief + return self-attestation)**: the dispatch brief is the orchestrator's pre-commitment to a model choice; the return self-attestation is the sub-agent's post-execution confirmation that it ran on the model the orchestrator selected. The two together close the verification loop — if the runtime silently routed an Opus-dispatched brief to a Sonnet model (or vice versa), the mismatch surfaces. Single-channel attestation (orchestrator-only OR sub-agent-only) leaves a verification gap.
...
- **24.12 (self-audit compliance)** Does the sweep that walks rules.md INCLUDE walking §24 itself against the journal it writes into? In other words, does the latest big table have rows for §24.0, §24.0a-i, §24.1-§24.24 (incl. §24.13.1-4 and §24.13.3a-d) — each one PASS-cited or FINDING'd against the journal file itself?
...
- **24.16 (Plan-Audit required before Implementer dispatch)** For every step's PLAN that introduces new types, new patterns, OR has scope >50 files, does the orchestrator dispatch a K=12 Plan-Audit (cluster-partitioned over the same §-ranges as code audits per [process.md §3 Auditor cluster partition](process.md#auditor-cluster-partition-canonical-k12)) + Aggregator (Opus per the [Sub-agent model policy per role](process.md#sub-agent-model-policy-per-role) canonical table) BEFORE the Implementer is dispatched? Does the Plan-Audit's findings get addressed by a Plan-amender sub-agent (Opus per the same policy table, scoped to journal Plan-section edits) before the Implementer brief is written?
...
- **24.20 (tool-invisible convention audit lens)** Does every audit round READ each modified `.cs` / `.ts` file in scope to scan for conventions that NEITHER `dotnet build` NOR `jb inspectcode` enforce — and FLAG any violation as a FINDING?
  - **The three mandatory tool-invisible lenses** (every Auditor MUST scan for all three by reading; a gate-green build does NOT imply convention-clean source):
    ...
    3. **`var` for locals where the type is evident** — carve-outs: `const` locals, types load-bearing for overload resolution, non-inferable return types. Neither tool enforces `var` preference; auditors must read the local-declaration sites.
...
- **24.21 (gate-verify MUST build the tests project AND run full-solution `jb inspectcode`)** Does the gate-verification step BUILD the tests project (not only the single feature csproj) AND run `jb inspectcode server/D2.slnx` at full-solution scope (not a per-lib or per-project scope)?
  ...
  - **Required inspectcode scope**: `jb inspectcode server/D2.slnx --severity=WARNING --format=Text --no-build` at FULL SOLUTION scope. Per-lib `jb inspectcode <feature.csproj>` hides findings in test files and in other in-solution projects that consume the feature — a lib-scoped run can report zero findings while 40+ test-file findings accumulate invisibly.
  - Evidence: gate-verify journal entry cites the literal command invoked — expect either `dotnet build server/D2.slnx` or `dotnet build <tests-csproj>` AND `jb inspectcode server/D2.slnx ...`. A gate-verify entry citing only a single-project build or a lib-scoped inspectcode is a §24.21 violation.
  - **Why**: deliverable 0014 ran `jb inspectcode` on the lib csproj only per-step; 43 test-file findings accumulated invisibly (mostly §5.25a redundant-`!`-after-NotBeNull recurrence) and were caught only by the K=12 final-review's full-solution sweep. First-pass test files with missing `using Xunit;` never compiled; the single-project build passed while test coverage was silently zero. Both gaps are closed by the two required scopes.
  - **How**: in the Implementer's pre-flight evidence section AND in any gate-verify step, document both commands: (a) `dotnet build server/D2.slnx` (or tests-csproj build) confirming test files compile, (b) `jb inspectcode server/D2.slnx --severity=WARNING --format=Text --no-build --output=inspectcode.log && cat inspectcode.log` confirming full-solution zero findings.
...
- **25.2** Does production code use `NodaTime` (.NET) / `Temporal` (TS) types exclusively for temporal values — never BCL `DateTime` / `DateTimeOffset` / `TimeSpan` (.NET) or JS `Date` (TS)?
  ...
  - **§25.2.b — spec-gen-driven wire-boundary types carve-out**: `DateTimeOffset?` is permitted on spec-gen-driven destination-assembly wire-boundary types (`IRequestContext`, `IAuthContext`, and the generated `MutableRequestContext`) where (i) the `.spec.json` declares the field as `DateTimeOffset?` for JSON interop with the inbound JWT / HTTP header pipeline, AND (ii) the generated xmldoc on the property mandates conversion to `NodaTime.Instant` at the consumer boundary before any temporal computation. This carve-out is strictly scoped to the spec-generated context interfaces — hand-written code that introduces new `DateTimeOffset` fields does NOT inherit this carve-out.
