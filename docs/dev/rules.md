...
| `D2.Shared.Auth.Outbound`                      | `ITokenExchangeClient` (RFC 8693) for the cases that genuinely re-mint — the Edge boundary mint + the deliberate exceptions (cross-trust-domain, narrowing, async scope reduction, impersonation); NOT a per-hop default. Internal hops forward the boundary-minted token unchanged (ADR-0022). Don't hand-roll OAuth flows. Workload identity is mTLS, not a forwarded service-identity token (ADR-0023) — the `client_credentials` service-identity surface (`IServiceIdentityClient`, `ServiceIdentityCallCredentials`) is superseded and built-but-unwired pending removal.                                                                                                                                                                                                                                                                                                                                                                                  |
