...
- **5.26** Does the global-usings set follow the two-tier-plus-safety-set policy — and is the never-globalize safety set kept EXPLICIT so the dependency-direction law stays compiler-enforced? (Canonical: [ADR-0020](../adrs/0020-service-project-structure.md) "the global-usings policy".)
  ...
  - Evidence: per service project → Tier-1 items present in `server/services/Directory.Build.targets` (service-scoped); per shared lib → explicit usings only, no Tier-1 globals (grep `server/shared/dotnet` for the Tier-1 namespaces in `GlobalUsings.cs` → expect zero); per layer → the Tier-2 `GlobalUsings.cs` matches the layer; grep every `GlobalUsings.cs` for the safety-set namespaces → expect zero (a safety-set namespace found globalized = FINDING). Cross-ref §9.36 (dependency-direction law).
