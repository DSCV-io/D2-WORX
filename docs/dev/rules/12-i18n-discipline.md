...
**Predicate index:** §12.1–§12.9 · 9 predicates.
...
- **12.2** Are backend handler messages (`D2Result.messages`) and input errors (`D2Result.inputErrors`) using translation keys from the dual-home message catalogs (`public/contracts/messages/` and/or `private/contracts/messages/`, not hardcoded strings)?
...
- **12.4** When adding translation keys, are they added to ALL present locale files in the owning dual-home catalog (`public/contracts/messages/*.json` and/or `private/contracts/messages/*.json`, kept in sync within that home)?
  - Evidence: `ls public/contracts/messages/*.json private/contracts/messages/*.json | xargs jq 'keys' | sort | uniq -c` (per home) → key sets identical across locales in that home. Run Paraglide compile from `private/services/web/` for frontend keys.

- **12.5** Are translation keys referenced via `TK.*` constants from `@dcsv-io/d2-i18n` / `DcsvIo.D2.I18n` (instead of bare TK key strings)? **Applies to TEST assertions as well as production source.** Carve-outs where the bare literal IS the API or the test (keep bare; audit treats as `⚪ N/A`, not a finding):
  - **(a) `D2Result` factory defaults** — bare strings are the API (`@dcsv-io/d2-result` is zero-dependency).
