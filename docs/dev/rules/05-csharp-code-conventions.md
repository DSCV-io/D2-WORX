...
- **5.2** Are all `TryParse` patterns using `D2.Shared.Utilities.Extensions` (`str.TryParseTruthyNull(out Guid? r)` / `str.TryParseTruthyNull<TEnum>(out var r)`)?
  ...
  - **Applies**: hand-written code AND codegen emitter output — when MutableEmitter / TKEmitter / ScopesEmitter emit code touching strings or Guids, it should call these extensions.
...
- **5.6** Are extension methods using C# 14 extension-members syntax (`extension(T target) { ... }`) instead of the old `this T` parameter style?
  - **Carve-out**: extension methods on nested generic async receivers (e.g. `ValueTask<D2Result<T>>`) and fluent async chains may use the old `this T target` style when the C# 14 block form fails to resolve (CS1061 on the nested-generic receiver) or triggers spurious CA2012 "ValueTask awaited multiple times" warnings on fluent intermediates. Document the carve-out with a one-line comment at the file head citing this predicate — a permitted exception to §7.16's "default to no comments" rule because the compiler-bug rationale is exactly the "WHY non-obvious" §7.16 carve-out criterion. Reference instance: `server/shared/dotnet/result/core/D2ResultAsyncExtensions.cs`.
  - **Carve-out application criterion** (mechanical, not "I tried it once"): the carve-out applies ONLY when BOTH (a) the C# 14 block form was attempted in the same file AND (b) the build surfaced CS1061 OR CA2012 specifically against the block-form extension's receiver. Generic CS1061s from unrelated bugs (missing using, wrong receiver, typo) do NOT trigger the carve-out. The comment cites the specific error code seen.
...
- **5.7** Are all concrete classes / records / exceptions / attributes marked `sealed`?
  ...
  - **Why**: enables JIT devirtualization on virtual / interface call sites and signals "not an extension point." Unsealing later is cheap; sealing retroactively is not.
...
- **5.8** Are single-line `if`/`while`/`for`/`foreach` bodies WITHOUT braces, and multi-line bodies WITH braces?
  - **Rule**: visually multi-line bodies (the body wraps, or a constructor / method call breaks across lines) ALWAYS get braces, regardless of statement count.
  - **Why**: a multi-line body without braces lets the next sibling statement visually merge with the body — the C dangling-`else` footgun, a real source of refactor bugs.
  - **Two acceptable brace-less forms — use one or the other, no in-between**:
    - **Form A — single line**: `if (cond) return foo;` — entire if + body on one source line. No padding required (the standard guard-clause pattern).
    - **Form B — two-liner with `if` on its own line**: permitted ONLY with blank lines BOTH above AND below the if-block. The brace-less body needs visual breathing room.
...
- **5.8a** Does every C# method body place a single blank line **before AND after** every control-flow block and every multi-line statement?
  - **Scope**: hand-authored `.cs` files. Does NOT apply to generated files (`*.g.cs`, `Generated/`) or EF migration files.
  ...
  - **Consecutive simple single-line statements** (sequential assignments, simple `return`s / method calls each fitting on one line) do NOT require padding between them — group them naturally.
  - **StyleCop interaction** (never violate these while adding padding):
    - SA1505 — no blank line after an opening brace.
    - SA1507 — no double blank lines anywhere (single only).
    - SA1508 — no blank line before a closing brace.
    - SA1510 — no blank line immediately BEFORE a `catch` / `finally` / `else` / `else if` / `do…while`-`while` continuation keyword. The "blank line before every block" requirement does NOT extend to continuation keywords: a `catch` is part of the same `try`, an `else` part of the same `if`, the `while` closes the `do…while` above it. Padding still applies before the OPENING `try` / `if` / `do` and after the WHOLE statement closes — just never between a block and its own continuation keyword.
    - SA1516 — member separator (blank line between type members) is pre-existing; per-method body padding adds blanks WITHIN a body and does not conflict.
  - **Cross-ref**: §5.8 (brace rules) is the sibling — Form-B already mandates blank lines above AND below a brace-less two-liner `if`; §5.8a generalizes that to every control-flow block and multi-line statement inside a method body.
  - **Why**: control-flow blocks and wrapped statements create visual noise when packed against sequential code; blank lines on both sides make the boundary between "decision / complex expression" and "simple flow" scannable in one pass. Dense padding is also a signal the method does too much (extract).
  - **How** — gold-standard example (from KeyCustodian `CompromiseKeyHandler`, deliverable 0022):
    ```csharp
    ...
    ```
    Note: the `await db.Keys…` chain is a multi-line statement → blank line before AND after. Each `if` → blank line before AND after. Simple single-liners (`var kidResult = …`) do NOT get padding against each other.
  ...
  - *Provenance: deliverable 0022 (blank-line readability convention from the post-step user review; SA1510 caveat from the S4 audit catching catch-after-try padding).*
...
- **5.10** Is `namespace` declared BEFORE `using` directives in every `.cs` file? (File-scoped `namespace X;` on line N, blank line, then `using` block.)
...
- **5.29** Does every concrete handler implementation file (a non-abstract `<Op>Handler` deriving `BaseHandler<…>` / `BaseRepoHandler<…>`) declare the three file-scoped `using` aliases `H` / `I` / `O` and use them in all five canonical slots?
  - **The triad** (file-scoped `using` directives, after the file-scoped `namespace …;` per §5.10):
  ...
  - **`I` here is the INPUT-TYPE alias — NOT the `I`-prefix interface-naming rule.** Do not flag `using I = <Op>Input;` as a §7 interface-naming violation — `I` is a deliberate one-letter alias for the input type, mirroring `O` for output and `H` for handler. (Auditor false-positive guard.)
  ...
  - **Aliases are PER-FILE, never global** — each op needs a different `H`/`I`/`O`, so globalizing them would collide. Optional convenience aliases for frequently-referenced dependency namespaces (`ReadRepo = …` / `CreateRepo = …`) are permitted but not part of the mandatory triad.
  ...
  - **Why**: a handler's type args, implemented interface, and `ExecuteAsync` signature repeat the input/output/interface FQNs up to five times (dominating the declaration for generated transport DTOs); the three-letter triad makes the signature scan as "this is the H for I→O." Established v1 shape reinstated.
  - **Evidence**: per concrete handler file → confirm the three aliases are present AND used in all five slots; confirm TSelf is spelled out (not aliased); confirm the aliases are file-scoped (absent from `GlobalUsings.cs`). A handler spelling out the FQNs instead of the aliases, missing one alias, or aliasing TSelf = FINDING.
...
- **5.12** Are collection expressions (`[a, b, c]` / `[]`) used instead of `new T { ... }` / `new[] { ... }` / `Array.Empty<T>()`?
  - **Required when** target type is `IEnumerable<T>` / `IReadOnlyList<T>` / `IList<T>` / `T[]` / `Span<T>` (or any constructible collection). Compiler picks the best concrete type (often a stack-allocated span or single allocation) and call sites read at half the noise.
...
- **5.18** Do all `public` types / methods have XML doc comments?
  - **Format**: `<summary>`, `<param>`, `<returns>`, `<exception>` as appropriate. Wrap onto multiple lines for line-length compliance.
  - **Synchronously-invoked callback parameters** (`onX`, `Action<>` / `Func<>` ctor args invoked inside `lock` / on the calling thread) MUST document throw-behavior in their `<param>` block — specifically, what happens to the upstream exception if the callback throws. Default platform behavior is "the thrown exception REPLACES the upstream" — a buggy logger inside the callback can silently swap a meaningful "TimeoutException from upstream X" for "InvalidOperationException from logger", making outage diagnosis painful. Document loudly so callers wrap in their own try/catch.
...
- **5.20** Do `[GeneratedRegex]` patterns classify into the right backtracking bucket and apply timeout discipline accordingly?
  - **Bucket 1 — no backtracking → NO timeout.** Single greedy quantifier with no following pattern (`\s+`); single char-class match with no quantifier (`[^\d]`); quantifier whose char class is disjoint from the next required token (`\w+\}` — `\w` can't match `}`).
  - **Bucket 2 — linear backtracking AND input upstream-bounded → NO timeout.** Greedy quantifier followed by an overlapping required token, but each backtrack is O(1) and total attempts grow at most linearly (e.g. `[^@\s]+\.[^@\s]+`). Document the linear-time guarantee + the input-length assumption in the pattern's XML doc.
  - **Bucket 3 — super-linear backtracking → set tight `matchTimeoutMilliseconds` (10–25 ms) AND pre-warm the JIT.** Nested quantifiers (`(a+)+`, `(a*)*`); alternation with overlap (`(a|aa)+`); polynomial / exponential backtracking. Pre-warm via `static readonly bool sr_jitWarmedUp = WarmUpHelper();` so the first user-visible call doesn't pay JIT cost inside the timeout window.
  - **Document the bucket** in the pattern's `<summary>`. A pattern change promoting Bucket-1/2 → Bucket-3 needs a timeout + pre-warm added in the same edit.
...
- **5.21** Does `dotnet build server/D2.slnx` produce zero StyleCop (SA\***\*), CS\*\*** warnings, null-ref warnings? Never suppress with `#pragma warning disable`, `!` (for silencing), or analogous.
  - **Container coordination (cross-ref §8.2)**: host `dotnet build server/D2.slnx` collides with `dotnet watch` containers via the shared `obj/` mount and crashes geo/gateway/signalr. Before running this gate while Compose is up, EITHER stop the active .NET containers (`docker compose stop <containers>`) OR run the build INSIDE a container. Re-start after the gate passes.
...
- **5.25** Does production code that emits codegen'd member names (Serilog diagnostic-property keys, OTel span/metric-tag keys, JSON field names mirroring an interface property, telemetry counter labels, AMQP header names mirroring a domain property) use `nameof(SourceOfTruthType.Member)` rather than raw string literals?
  - **EXEMPTION**: spec-pinning tests that explicitly assert "this exact name exists on the wire" KEEP literal strings — the literal IS the pin. Same exemption for constants whose literal value IS the wire format (JWT claim-type constants, OAuth scope constants, AMQP exchange names) — spec-anchored, never `nameof`-derived.
  ...
  - **Why**: raw literals defeat compile-time rename safety. When the source-of-truth member is renamed (`IRequestContext.SessionId` → `.UserSessionId`), every raw literal `"SessionId"` silently drifts to the WRONG wire value while still compiling — Loki / Tempo queries see a partial-data outage with no compile-time signal. `nameof(IRequestContext.UserSessionId)` makes the rename break the build at every emission site.
  - **How**: emit structured-log properties, span/metric tags, JSON fields — any wire-format key mirroring a domain member — via `nameof(IInterface.Member)`. Rename test: "if I rename the source-of-truth member tomorrow, do I want this site to break the build?" Yes → `nameof`; no → literal (spec-pinning tests stay literal).
...
- **5.25a** After a `.Should().NotBeNull()` assertion (AwesomeAssertions), is the null-forgiving operator `!` absent from any immediately following member access on the same variable?
  - **Required**: use `x.Member` — NOT `x!.Member`. After `.Should().NotBeNull()`, AwesomeAssertions flows a non-null post-condition that the C# compiler AND `jb inspectcode` both recognize; the `!` is therefore redundant and `jb inspectcode` flags it.
  - **Why**: matches the §5.21 / §5.22 zero-warnings-both-tools mandate. Redundant `!` operators specifically appear in sub-agent-written test code after `NotBeNull()`; this predicate makes the violation explicitly auditable.
  - **False-positive carve-out — AwesomeAssertions 9.x does NOT ship `[NotNull]`**: its `.Should().NotBeNull()` returns `AndConstraint<T>`, not the original value, and carries no `[NotNull]` post-condition. C# nullable flow analysis does NOT infer non-null on the original variable — the compiler still considers `x` nullable, so `x!.Member` on the next line is COMPILER-REQUIRED, not redundant. (FluentAssertions 6+ DOES ship `[NotNull]`, making the `!` there genuinely redundant.) When the library is AwesomeAssertions, `x!.Member` after `.Should().NotBeNull()` is NOT a §5.25a violation — flag N/A with this rationale, do not dispatch a Fixer.
  - Evidence: `grep -rEn '\.Should\(\)\.NotBeNull\(\)' <test scope>` → per hit, confirm the following access does NOT carry `!`; if it does, check whether the library is AwesomeAssertions (carve-out applies) or FluentAssertions (violation).
...
- **5.26** Does the global-usings set follow the two-tier frequency-driven policy — no duplicates of SDK ImplicitUsings or Tier-1 entries, no unused globals flagged by inspectcode? (Canonical: [ADR-0020](../adrs/0020-service-project-structure.md) "the global-usings policy".)
  - **Tier-1 — service-project scope** (central `<Using>` items in `server/services/Directory.Build.targets`): `D2.Shared.Result` · `D2.Shared.Utilities.Extensions` + `.Attributes` + `.Enums` · `D2.Shared.I18n`. Service projects (`server/services/`) reliably reference the full D2 runtime stack; the `netstandard2.0` condition excludes source-gen shells. **Shared libs (`server/shared/dotnet/`) are excluded** — `D2.Shared.I18n` is split across three assemblies and the Tier-1 libs form a dependency chain; a blanket runtime-wide global produces hard CS0246 in libs below the full stack. Shared libs keep explicit usings.
  ...
  - **Hard constraints** (regardless of frequency): (a) never duplicate a namespace already covered by SDK ImplicitUsings (`System`, `System.Collections.Generic`, `System.Threading`, `System.Threading.Tasks`, etc.); (b) never duplicate a Tier-1 entry; (c) never globalize in a shared lib (`server/shared/dotnet/`).
  ...
  - Evidence: per `GlobalUsings.cs` → (a) compare each entry against the SDK ImplicitUsings default set + Tier-1 entries — any overlap = FINDING (duplicate); (b) `jb inspectcode` → zero "using directive is not required" warnings on `GlobalUsings.cs` (an entry never resolved = unused global = FINDING); (c) per reference graph → confirm `<ProjectReference>` edges respect the one-direction law (§9.36) — a global using in a domain project for `Microsoft.EntityFrameworkCore` with no EF csproj reference is a §9.36 csproj violation, not a global-using one. Cross-ref §9.36.
...
- **5.27** Is `[Required]` NEVER used on a non-nullable struct property (it is a no-op), with a real range/custom validator used instead?
  ...
  - **Why**: belt-and-suspenders — the options validator catches config errors at boot; the domain VO catches every path. `[Required]` on a struct is the trap that passes review because it *reads* as validation. Origin: an as-built `RotationPolicyOptions` carried `[Required] public TimeSpan Cadence` — a silent no-op.
  - Evidence: `grep -rEn '\[Required\]' <options scope>` → per hit, confirm the property is a reference type (nullable struct or class); a non-nullable struct property carrying `[Required]` = FINDING. Cross-ref §23 (configuration hygiene), [ADR-0020](../adrs/0020-service-project-structure.md) (options pipeline).

- **5.28** (Prettier-clean commits — CODE files; Markdown is EXCLUDED) Are all of a commit's touched Prettier-handled CODE files Prettier-clean? The Prettier scope is `.ts` / `.js` / `.mjs` / `.cjs` / `.json` / `.svelte` / `.html` / `.css` / `.yaml` / `.yml`. The `.husky/pre-commit` hook runs `prettier --check` on the staged subset of that glob and BLOCKS a commit whose staged code files are not clean.
  - **Required**: before committing, the touched code files pass `prettier --check` (respecting `.prettierignore`). Run `pnpm format` (write) or `pnpm format:check` first. Never `--no-verify` past the hook. Generated output (`**/*.g.ts`, `Generated/`, machine-emitted fixtures) is `.prettierignore`d — formatting is the emitter's responsibility.
  - **Markdown carve-out — NEVER Prettier a `.md` file**: `**/*.md` is in `.prettierignore`, and `md` is dropped from the `format` / `format:check` globs + the hook glob. Prettier's Markdown emphasis-normalization (`*` ↔ `_`) cannot distinguish an emphasis marker from an underscore in an identifier or an asterisk in a glob, so on `--write` it CORRUPTS this identifier-heavy docs tree — `SCREAMING_SNAKE` → `SCREAMING*SNAKE`, `*.json` → `_.json`, `PHASE_3.md` → `PHASE*3.md` — and `--check` then calls the garbled output "clean" because it is stable. No Prettier option disables emphasis-normalization. Markdown is governed by §11, NOT Prettier.
  - **Why**: a clean code-formatting baseline keeps diffs reviewable; the mechanical hook prevents drift re-accumulation. The Markdown carve-out exists because a repo-wide `prettier --write` silently mangled ~121 identifier-heavy Markdown files before it was caught — the corruption passes `--check`, invisible to the gate, surfaced only by a content read.
  - **How**: code files → `pnpm format` + the hook. Markdown → §11 + manual editing; if `prettier` is ever about to touch a `.md`, stop.
